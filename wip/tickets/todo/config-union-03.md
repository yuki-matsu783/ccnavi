...
rationale: |
  設計 `wip/design/config-union.md` の §25.2 と §25.4 を実装する。`CCNAVI_PROJECT_HOME`
  ...
  旧 `CCNAVI_PROJECT_RULES` の warn、`--explain` の rules の層ごとの全件と `--explain --json` の
...
allow:
  ...
  - match: Write|Edit
    glob: "tests/*"
  # `--explain --json` に `layers` を足すと、拡張のフィクスチャと鍵集合を突き合わせる
