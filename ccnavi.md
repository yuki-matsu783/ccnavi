...
> | チケットは `.current-ticket.md` 1 本、承認は台帳 jsonl の最後の行（§9、§17.5） | チケットは `.ccnavi/proposals/` に複数、親子とフェーズを持ち、作業ツリー（git worktree）ごとに 1 本が効く。承認はチケットごとの写し。§24 にまとめた。§9 と §17 は 1 本のときの形として残す |
>
...
| 何 | 場所 | 誰が書く | git |
|---|---|---|---|
| 提案（親も子も） | 親の作業ツリーの `.ccnavi/proposals/<状態>/<識別子>.md`。状態は `todo` / `doing` / `done` / `cancelled` の 4 つの置き場 | `todo/` は親が書く。置き場を動かすのは保護済みスクリプトだけ | 親のブランチにコミット |
| 承認済みの写し | 親の作業ツリーの `.ccnavi/tickets/<識別子>.md` | `ccnavi --approve`（人）。コミットと push は `ccnavi-approve.sh` | 親のブランチにコミット |
| 閉じた写し | 親の作業ツリーの `.ccnavi/tickets/closed/<識別子>.md` | hook が提案の `done/` `cancelled/` を見て動かす | 親のブランチにコミット |
| フェーズの印 | 親の作業ツリーの `.ccnavi/tickets/phases/<親>/<N>.pending` / `.requested` / `.reviewed` / `.skipped` | レビュースクリプト、`ccnavi --reviewed`、hook | 親のブランチにコミット |

**写しは git で運ぶ。** 承認した人の機械にだけ在る形だと、A が承認して B の機械で作業し A がレビューする、
という普通の流れが成り立たない。B の機械には写しが無いので、承認は「されなかったこと」になる。だから写しも
印も提案と同じブランチ（親チケットのブランチ）に乗せ、`ccnavi-approve.sh` がコミットして push する。
B の側はセッションの頭に `ccnavi-fetch.sh` が fast-forward で取り込む。

代償は 3 つ。**承認は push するまで効かない**（オフラインでは他の機械に届かない）。**承認の記録が
リポジトリの履歴に残る**ので、写しを消しても過去のコミットから復元できる。**ccnavi が入っていない機械の
エージェントが偽の写しを push できる**（配らない形ではその機械の外に影響が出なかった）。1 つめと 2 つめは
運用で飲み、3 つめを塞ぐなら承認に署名を足すことになる（今は入れない）。

**権威は親のツリーの版 1 つ。** 子の作業ツリーは親のブランチから切るので、そこにも同じ写しが checkout
される。読むのは親のツリーの側だけで、提案の `dedupe` と違い「権威の側に無ければ全部残す」には倒さない。
倒すと、親のツリーで閉じたチケットが、子に残った切った時点の写しから開いたものとして復活する。
親のツリーがその識別子を開閉どちらでも持っていないときだけ、見つかった側を残す（親の作業ツリーを作る前に
承認した写しを落とさないため）。
...
置き場は 2 本の設定で差し替える。`CCNAVI_TICKETS`（提案、既定 `.ccnavi/proposals`）と
`CCNAVI_APPROVED`（写し、既定 `.ccnavi/tickets`）。どちらも各ツリーのルートからの相対で、
そのツリーの git が追跡する。4 つの状態と `closed/` `phases/` はそれぞれの下に固定。`CCNAVI_TICKET` と `CCNAVI_LEDGER` は廃止し、`--lint` が「もう効かない」と言う。
...
写しを守るのは組み込み（`builtin-guard-approved-tickets`）で、ルールファイルには書かせない。書かせると
消せることになる。置き場はプロジェクトごとに違うリポジトリの中にあるので、ルールファイルに綴りを固定できない
（プロジェクトのルールと同じ理由。§25.6）。綴りから当てる形を組み立てられないときだけ `--lint` が error を出す。

`.claude/` ではなく `.ccnavi/` に置くのは、プロジェクトに `.claude/` があると Claude Code がそこのスキルを
読み込み、`--lint` が迷い子として拾うから（§25.2）。

main の作業ツリーはチケットを持たない。
...
- **識別子は子が `<親>-<2 桁連番>`。** 親の識別子が名前空間になり、別の親の子と衝突しない。親の識別子は人が決める（issue 番号など）。連番はその親の `.ccnavi/proposals/` 全体（4 つの置き場）の最大 + 1。作業ツリーは `.claude/worktrees/i0050-03/` のように、読んで親が分かる
...
計画中に親が触るのは `.ccnavi/proposals/` だけで、そこはルールで守る場所ではない。
...
| Write / Edit | 通す。次のフェーズの計画（`.ccnavi/proposals/`）はレビュー前に進めてよい |
...
同じ拡張のリスク管理画面は、配点（`risk.yml`、§17.3）の閾値と項目を直す。守るものはルール設定画面と同じ。
編集中の配点は一時ファイルに書いて `--lint --risk` で検証し（hook はこのフラグを渡さないので、保存していない
配点が実運用に効く道は無い）、拡張は点を数えない。数えるのは `risk.py` が子を閉じるときだけで、拡張に
差分を数える写しを持たせると「何点になるか」が 2 か所に置かれる。配点はワークスペースに 1 本（§25）なので、
保存を止める条件はどのツリーの `doing` でも。ファイルが無ければ組み込みと同じ値で作る（数え方は変わらない）。

同じ拡張のフェーズ管理画面は、フェーズの種類（`phases.yml`、§24.15.1）を直す。守るものはリスク管理画面と同じ。
編集中の種類は一時ファイルに書いて `--lint --phases` で検証し（hook はこのフラグを渡さない）、拡張は子の範囲が
上限に収まるかもレビューの要否も判定しない。それは `--approve` と着手・閉じるときに実行ファイルが言う。
`--lint` は提案（作業ツリーに在る親。承認済みでも提案が残っていれば）の計画をその種類で照合するので、計画が指す種類を
消す保存はそこで止まる（承認済みの写しは定義が読めるかまで。写しの計画との食い違いは次の承認と着手で実行ファイルが言う。
チケット制御が disable なら照合は走らない）。実行ファイルは PyYAML（YAML 1.1）で、拡張の `yaml` は 1.2 なので、`yes` `on` `1:30`
`0755` のような語は裸で書くと型が変わる。両画面とも、その綴りは引用符で囲んで書く。種類は
ワークスペースに 1 本（§25）で、承認・着手・閉じるときに読まれるので、保存を止める条件はどのツリーの `doing` でも。
ファイルが無ければ README の例を雛形として作る。組み込みの既定は持たない（§24.15.1 と同じ理由。既定を組み込むと、
意図せずレビューの要否が決まる）ので、雛形の `scope` はそのプロジェクトの置き場に直す前提。同じ id を 2 つ書くと
`yaml.safe_load` が後ろで黙って上書きするので、そこは拡張が保存の前に止める。
...
- 保護済みスクリプトが `.ccnavi/tickets/` へ書く道が、組み込みの既定のシェル書き込み検知に当たらないか
...
| 進行状態は共有ルート 1 か所、判定記録はツリーごと | §24.2、§14 | 写しと印は親のツリー、記録は各ツリー |
...
| 層 | 何を表す | 場所 | 取りうる値 |
|---|---|---|---|
| 提案 | チケット 1 本の進み具合 | 親の作業ツリーの `.ccnavi/proposals/<状態>/` | `todo` / `doing` / `done` / `cancelled`（置き場） |
| 写し | そのチケットの範囲が効いているか | 親の作業ツリーの `.ccnavi/tickets/` | 無し（未承認）/ 開（`<識別子>.md`）/ 閉（`closed/<識別子>.md`） |
| 印 | フェーズがレビューのどこにいるか | 親の作業ツリーの `.ccnavi/tickets/phases/<親>/` | `N.pending` / `N.skipped` / `N.requested` / `N.reviewed` の有無 |
...
- 同じ識別子が 2 つの置き場にあると、スクリプトは動かさない（「複数の場所にある」）。子の作業ツリーに写った親の
  `.ccnavi/proposals/` は数えない。権威は親のツリーの側
...
| 遷移 | 動かす者 | 条件 |
|---|---|---|
| 無し → 開 | `ccnavi --approve`（人）。写しを配るのは `ccnavi-approve.sh` のコミットと push | 提案が `cancelled/` 以外にあり、写しが開にも閉にも無い。子は親の写しが開いていて、親の部分集合で、深さ 2 段 |
| 開 → 閉 | `ccnavi-ticket.sh done` / `cancel` の直後。取りこぼしは次の PostToolUse の `sync` | 写しが記録した提案の場所（`source_tree` の `.ccnavi/proposals/`）で、提案が `done/` か `cancelled/` にある |
...
- 写しへ写る欄はスクリプトの欄だけ（`started_at` `completed_at` `base_sha` `cancelled_at` `cancel_reason`）。
  範囲は承認した時点のまま
- 承認したあと、写しをコミットするまでは子の作業ツリーに渡らない。順序は「承認 → 写しをコミット →
  子の作業ツリーを作る → `start`」。コミットの前に作業ツリーを作ると、その子には範囲が効かない
- 親の作業ツリーを消すと写しが読めなくなる（権威がそこにあるため）。片付けの順（子をマージ → `done` →
  ツリーを消す）を守れば、消す時点で全部閉じている
...
`wip/`。提案の置き場から導くのはやめ、綴りを固定した）が追跡から消えていること、未コミットが無いこと、
push 済みであること。未コミットの検査は `.ccnavi/` を数えない（印は hook が書くので、そこを数えると
「レビューを頼む前に印をコミットしろ」と言い続けることになる）。
...
骨は 3 つ。**道具はワークスペース、設定はプロジェクト、git はツリーごと**。**Write / Edit は行き先のプロジェクトのルール、Bash は全プロジェクトの和**。
**プロジェクトを決めるのは人の承認で、判定はその値を見ない**。
...
| ワークスペースのルール | `.claude/ccnavi/rules.yml`（`CCNAVI_RULES`、今と同じ） | ワークスペース |
| プロジェクト | `projects/<名前>/`（`CCNAVI_PROJECTS`、既定 `projects`、ワークスペースルートからの相対） | ワークスペースでは無視。プロジェクト自身の git |
| プロジェクトのルール | `projects/<名前>/config/rules.yml`（`CCNAVI_PROJECT_RULES`、既定 `config/rules.yml`、 git プロジェクトルートからの相対） | プロジェクト |
| 作業ツリー | `.claude/worktrees/<識別子>/`。プロジェクトから切る | 管理外 |
| 提案 | そのツリーの `.ccnavi/proposals/<状態>/<識別子>.md`。プロジェクト向けの提案はプロジェクトの git が持つ | プロジェクト |
| 承認済みの写し、閉じた写し、フェーズの印 | そのツリーの `.ccnavi/tickets/`。写しに `project:` が入る | プロジェクト |
| 記録、控え、セッションの状態 | ワークスペースの `.claude/ccnavi/`。今と同じ | 管理外 |
| git ラッパの記録 | ワークスペースの `logs/<プロジェクト>/`。ワークスペース自身は `logs/` | 管理外 |
| フェーズの種類、リスクの配点 | ワークスペースに 1 本。プロジェクトごとに変えるのは今回入れない | ワークスペース |
...
**プロジェクトのリポジトリに書くのは、人が合意したもの全部。** ルール、提案、承認済みの写し、フェーズの印。
以前は「書くのはルールだけ」としていた。提案を public のリポジトリに出すとワークスペースの運用の痕跡が残る、
という理由だった（REQ-MLT-14）。判断を裏返したのは、A が承認して B の機械で作業する流れでは、その痕跡こそが
共有したいものだから（§24.2）。痕跡は feature ブランチに乗り、マージリクエストの squash merge で 1 コミットに畳まれる。

git ラッパの記録（`logs/`）は今までどおりワークスペース側に残す。こちらは判断の記録ではなく、その機械で
何を打ったかでしかない。
...
作業ツリーの中に checkout されている `config/rules.yml` は読まない。読むのは git プロジェクトルートの版 1 本。ブランチで
変えたルールがそのブランチの判定に効く形は、人が見ていないルールが効く形になる。

**Bash は全部の和。** ワークスペースのルールと、全プロジェクトのルールをタイプごとに連結して当てる。`deny` `ask` `allow` の順は変わらない。
どのプロジェクトに `cwd` があっても、`cd` を挟んでも、同じ判定になる。

代償は allow の共有 1 つ。プロジェクト A だけが許したい `npm test` はプロジェクト B に居ても通る。`deny` と `ask` は増える側なので
安全に倒れ、緩むのは allow の共有だけ。逆に「プロジェクト A に居るときだけ止める」は書けない。要るなら採らなかった形に戻るが、
Bash の allow はどのプロジェクトでも似た形になるので、まず要らないと見ている。

**識別子にはプロジェクトの名前を添える。** 記録の `rules` と文面の `source` は `lib:source` のように書く。プロジェクトどうしで同じ `id` が
あっても衝突しない。ワークスペースのルールは今までどおり裸の `id`。

**読めないプロジェクトのルールは組み込みに落ちる。** そのプロジェクトへの書き込みは組み込みの既定で判定し、Bash の和からはそのプロジェクトを外す。
外したことは記録の `fallback` にプロジェクトの名前付きで残し、`--lint` が言う。ワークスペースのルールが読めないときの挙動は今と同じ。

読み込みはプロジェクトの数だけ増える。1 本 200 行の YAML で数 ms なので、実行前の判定の期限の中に収まるが、`ms` の欄で見る（§25.10）。
...
- **提案の置き場は `wip/<repo>/tickets/`。** 親と子は同じ `repo` なので同じ場所に並ぶ。ワークスペース自身の作業は
  `project:` を書かず、ワークスペースのツリーのまま

### 25.6 守るもの

プロジェクトのルールファイルを中核ファイルに加える。Write / Edit の拒否、シェルからの書き込みの拒否、控えと復元の 3 つとも、
今 `.claude/ccnavi/rules.yml` に掛けているものをそのまま `projects/<名前>/config/rules.yml` に掛ける。
プロジェクトのルールは行き先の判定に使うので、ここが書けると、エージェントが自分のルールを緩めて次の呼び出しから効く。

ワークスペース側の中核（hook の登録、実行ファイル、保護済みスクリプト）は今と同じ。プロジェクトの中には無いので、プロジェクトを編集している
エージェントの手が届かない。
...
git プロジェクトルートで見た変更に当てるのはそのプロジェクトのルール、作業ツリーならそのツリーに結び付くチケットの範囲とプロジェクトのルール。
Bash の和はここでは使わない。実行後に見るのはファイルの変更で、行き先が分かっている。
...
置き換わる（`rules.load`）。プロジェクトのルールファイルでも同じで、置き換わる先はそのプロジェクトではなく
ワークスペースルート。sh はそこにしか無いから。
...
`--lint` が言うこと。`projects/` がワークスペースの `.gitignore` に入っているか。各プロジェクトのルールが読めるか。プロジェクトが `.claude/` を
持っていないか。作業ツリーの切り元が写しの `project:` と合うか。`project:` の無いプロジェクト向けチケットが無いか。

`--explain` が示すこと。プロジェクトの一覧と、それぞれのルールファイルが読めるか、タイプごとの件数。

記録に `tree` と `project` を足す。`rules` はプロジェクトの名前付き。dry-run でまず `project` の分布を見て、Bash の和で増えた `ask` と
`deny` を数えてから `enable` に切り替える。
...
- プロジェクトごとのフェーズの種類とリスクの配点。ワークスペースの 1 本を使う
- プロジェクト A に居るときだけ効く Bash のルール
- 2 段以上の深さに置いたプロジェクト、ワークスペースの外に置いたプロジェクト
- `cwd` と `cd` の追跡、引数の語の走査
- 作業ツリーの中に checkout されたルールファイル
