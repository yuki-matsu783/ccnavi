...
承認は「人の合意」そのものなので、エージェントが自分で出せてはいけない。経路は 2 つあり、
守り方が違う。

| 経路 | 誰が打つ | 守り |
|---|---|---|
| `ccnavi --approve` | 人が端末で | 標準入力が端末であること（`cli._from_terminal`）。hook の中や `echo y \|` から来ていないことの、いちばん安い証拠 |
| `ccnavi --approve --yes <識別子,…>` | VS Code の拡張が、人がオーバーレイで押したときに | 組み込みの deny（`builtin-guard-ticket-approval`）が、Bash / PowerShell からの `--yes` を止める。加えて、見せた束と今の束が同じであることを実行ファイルが求める |

`--yes` に端末を求めないのは、拡張の子プロセスが端末を持たないから。端末の壁を残したまま
拡張から承認しようとすると、`CCNAVI_GUARD_TICKET_APPROVAL=disable` で壁ごと切ることになり、
`--reviewed` と `accept` の守りまで一緒に落ちる。壁を切るのではなく、承認だけを別の守りに移した。

束を見るだけの `--approve --preview` は、写しを置かないのでエージェントにも通す。免除するのは
`--preview` の付いた `--approve` だけで、`--yes` は免除しない。免除の範囲はコマンド 1 本まで
（Bash は shellread の区切り、PowerShell は生の文字列なので `;` `&` `|` と改行を区切りと見る）。
ここを緩く書くと、後ろのコマンドに `--preview` を足すだけで承認が通る。

この守りは §17.6 と同じ限界を持つ。`CCNAVI_MODE=dry-run` の間は deny が止めないので、
エージェントは `--yes` を打てる。dry-run は「判定を言うだけ」のモードで、承認の経路だけを
例外にはしない。例外を作ると、dry-run で確かめている判定と本番の判定が別物になる。

### 17.8 承認されたことをモデルに伝える

人がボードで承認しても、モデルはそれを知らない。人が「承認した」と打つまで後工程に入れない、
という形は、承認の経路を拡張へ移した意味を半分に減らす。そこで hook が伝える。

- 控えは `<state>/approved-<session>-<agent>.json`。中身は「識別子 → 印」で、印は写しの
  `approved_at` と `revised_at`。改版（§24.15.5）は写しを書き換えるだけで識別子を増やさないので、
  識別子だけを比べても新しい合意だと分からない
- 起点はそのセッションの最初の hook。それ以前の写しは伝えない。`SessionStart` は控えが無ければ
  起点を作るだけで、控えがあれば触らない（compact の前に置かれた承認は、compact の後にも 1 度伝える）
- 伝えるのは `UserPromptSubmit` と `PreToolUse`。どちらでも `additionalContext` で届く
- 閉じた写しも見る。承認の直後・次の hook の前に子が閉じると、開いた写しだけを見る形では
  その承認が誰にも伝わらないまま控えに吸われる
- 控えが読めるのに壊れているときは、「無い」と同じには扱わない。伝えていない承認ごと現状を
  起点にして黙るより、もう一度伝えるほうがよい
...
人の判断のうち、承認は拡張の中で完結する。ボードのオーバーレイが
`--approve --preview --json` で束を見せ、人が押したら `--approve --yes <識別子,…> --json` を
子プロセスで打つ（README「承認の JSON」）。`--yes` は端末を求めない。端末を求める壁（§17.7）は
「エージェントが人の合意を自分で出せない」ための壁で、拡張は人が押したことを知っている側に
居る。壁の代わりに 2 つを求める。見せた束と今の束が同じであること（人が見ていないものを
承認しない）と、エージェントが `--yes` を打てないこと（組み込みの deny）。

`accept` は端末のまま。sh がレビューのスレッドを取ってきてから判断する経路で、拡張に移すには
その結果を JSON で返す契約が別に要る。`wrapup` はボードに出さない。

承認されたことは hook がモデルに 1 度だけ伝える（§17.8）。拡張は同じ文を通知から人に渡し、
人が進行中のセッションに貼るか、新しいセッションを開く。走っている Claude Code のセッションへ
拡張から文を送り込む公開の口は無いので、そこは人の 1 手を残す。
...
**プロジェクトのリポジトリに書くのはルールだけ。** 提案と git ラッパの記録は、今は各ツリーのルートからの相対に置いているが、
プロジェクトの中に置くと public のリポジトリにワークスペースの運用の痕跡が出る。どちらも「ワークスペースルートからの相対で、プロジェクトの名前を挟む」に
変える。プロジェクトが無いとき（`projects/` が無いか空）は今の置き場のままで、この章の前と同じに動く。
