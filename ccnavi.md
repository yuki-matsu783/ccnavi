...
> 親子とフェーズを持つ形は §24 にある。書式も §24 で rules.yml と同じタイプに改めた。
...
以前は、チケットが宣言した範囲の広さで承認時に点を付け、40 以上で識別子の手入力を求めていた（二段階承認）。
これはやめた（2026-09-10）。宣言の広さは親がチケットを書くときに `human_review.reason` で言えばよく、
守りたいのは「計画のときに軽いと思った作業が、やってみたら大きな変更になっていたのに、レビューを通らずに進む」
のほうだった。宣言は実績を保証しない。だから点は、子を閉じるときにその子の差分で数える（`risk.py`）。

**測るとき・測るもの。** 子の `ticket done` で、その子の作業ツリーの `base_sha..HEAD`（コミット済み。
未コミットは `done` の前提で無い）を `git diff --numstat` と `--name-status` で数える。記録は
`phases/<親>/<子>.risk.json`（点・段階・加点の内訳・測れなかった項目・HEAD）。フェーズの点は子の最大値。

**配点は人が持つ。** `.claude/ccnavi/risk.yml`（guard の内側）。エージェントが配点を書けると、自分のリスクを
自分で決められる。無ければ組み込み（下の定量 4 項目）。壊れていれば組み込みに落ち、`--lint` と閉じたときの出力が言う。

| 系統 | 当て方 | 測る者 | 測れなかったとき |
|---|---|---|---|
| 定量（組み込み） | `lines_over: N`（追加＋削除の行数）/ `files_over: N` / `deleted_over: N` / `glob`（触ったファイルが当たるごとに加点。`max` で上限） | exe が差分から | — |
| 定量（スクリプト） | `script: <path>`。置ける場所は `.claude/ccnavi/` か `.claude/scripts/` の下（guard の内側）だけ | exe が `sh <path>` を走らせる。cwd は子の作業ツリー、`CCNAVI_BASE_SHA` / `CCNAVI_HEAD` / `CCNAVI_TICKET` / `CCNAVI_PARENT`、標準出力は整数か `{"points": N, "message": "…"}`、期限 30 秒 | その項目の `points` を加点し、内訳に「測れなかった」と書く。0 にすると壊れたスクリプトがリスクを消す |
| 定性（サブエージェント） | `judge: <問い>` | 判断はサブエージェント、記録は親。`done` が問いと差分の要約を `state/risk-judge-<子>.md` に書き、判定が揃うまで閉じない。親は `ccnavi-ticket.sh judge <子> <項目> yes\|no --reason` で記録し、`phases/<親>/<子>.judge.json` に HEAD 付きで残る。HEAD が動けば判定は古く、取り直し | 判定が無い＝閉じられない |

組み込みの既定は、行数 300 超 +25、ファイル 10 超 +15、`.github/**` +35（上限 35）、消したファイル 3 超 +20。

| 段階 | 既定の閾値 | 何をするか |
|---|---|---|
| LOW | 0 | 何もしない |
| MEDIUM | 20 | フェーズの終わりの文面と `--explain` に点を出す |
| HIGH | 40 | **宣言に関わらず、そのフェーズは人間レビューが要る扱い**（`Phase.review_required` が真になり、ゲートが閉じる。`skipped` の印は置かれない） |
| CRITICAL | 70 | HIGH と同じ。記録の段階が CRITICAL になる |

段階の名前は固定で、閾値だけ `levels` で動かす。承認画面と記録が名前を読むので、増減させると読み手が揃わない。
実績で上書きするのは厳しい側にだけ。実績が小さくても、宣言のレビュー要を下げることはしない。

**レビュアーへの見せ方。** `request` の依頼文の先頭に「このレビューのリスク: 58 (HIGH) — 行数が多い（412 行 > 300）、…」を
機械が書く（`review._covered_header`）。閉じたときの出力とフェーズの終わりの文面にも同じ行が出る。レビュアーは
「なぜこのフェーズにレビューが要ることになったか」を依頼文で読める。

### 17.4 二段階承認（廃止）

宣言の広さで数える点と、40 以上で識別子の手入力を求める二段階承認は 2026-09-10 に消した。承認画面は範囲の差分・
レビュー要否・計画だけになった。理由は §17.3 の冒頭。
...
置き場は 2 本の設定で差し替える。`CCNAVI_TICKETS`（提案、既定 `wip/tickets`、各作業ツリーのルートからの相対）と
...
frontmatter は rules.yml と同じタイプ（`deny` / `ask` / `allow`）で書く。`version` を上げる。
今効くのは Write / Edit 系のパスのタイプだけで、`match` に Bash を書いた項や `tools` は「効かない」と名指しで警告する。
上限は 20 件のまま。
...
範囲の前置はその作業ツリーのルートから解く。main の直下ならチケットは無く、ルールだけで判定する。
...
| イベント | すること |
|---|---|
| SubagentStart | 承認済みで閉じていない子の一覧（識別子・作業ツリー・範囲）を `additionalContext` で渡す。渡すのは cwd の作業ツリーに関わる子だけ（親のツリーならその親の子、子のツリーならその子自身）。main とチケットの無い作業ツリーからの起動には何も渡さない。止められないイベントなので判定はしない |
...
`--explain --json` は、`--explain` が言うことのうちチケットに関わる部分を機械可読で出す（REQ-DIA-06）。
読み手は VS Code の拡張 `vscode-extension/ccnavi-board/`（ボード）。形は README「ボードの JSON」に定め、
同じ例を拡張のフィクスチャと Python のテストの両方が見る。ここで守るのは 2 つ。

- **答えは 1 か所で出す。** ゲートの開閉（§24.7）、承認待ち（§24.5 の束）、作業ツリーの有無（§24.4）は、
  判定と承認が使う関数（`phase.phases_of`、`approval.waiting`、`tree.worktrees`）をそのまま呼んで載せる。
  拡張は提案や印を自分で解釈しない。解釈を 2 か所に置くと、ボードが「開いている」と言うゲートで
  判定が止まる、という形になる
...
拡張は glob も regex も自分で当てない（REQ-DIA-03）。編集中のルールは一時ファイルに書いて
`--rules` で渡す。画面が書くのはルールファイルだけで、`settings.json` は読むだけ。保存の前に
`--lint` を通し、作業中のチケット（`doing`）がある間は保存しない。hook は呼び出しのたびに
ルールを読み直すので、走っているセッションの途中で判定が変わる形を作らないため。

ルール設定画面はプロジェクトのルール（`projects/<名前>/config/rules.yml`、§25.4）も対象にできる。
対象ごとに 1 パネル。編集中の内容は `--project-rules-file <名前>=<パス>`（REQ-DIA-09）で渡す。
`--rules` がワークスペースのルールを差し替えるのと同じことを、プロジェクト 1 つに対して行うオプションで、診断
（`--test` / `--test-samples` / `--lint` / `--explain`）でだけ効き、hook からの判定では無視する。
保存していないルールが実運用の判定に効く道を作らないため。守る対象（§25.6）も差し替えを見ない。
保存を止める条件は、プロジェクトのルールならそのプロジェクトの `doing` だけ。プロジェクトのルールは
そのプロジェクトへの書き込みと Bash の和にしか効かず、他のプロジェクトで進んでいるセッションの書き込みの判定は
変えないため（Bash の和は変わる。そこは受け入れる）。

同じ拡張のプロジェクト管理画面は、§25 の置き場（`projects/`）を人が整えるための入口。`--explain --json` の
`trees` を並べ、`--lint --json`（README「lint の JSON」、REQ-DIA-08）の苦情をプロジェクトごとに添える。拡張が
自分で見るのは、origin（ローカルの git を読み取り専用で起こす）、ルールファイルと `.claude/` の有無、`.gitignore` の
本文、ワークスペース直下を深さ 2 まで歩いて見つける「プロジェクトになっていない `.git`」だけで、何がプロジェクトかを
自分で決め直すことはしない。
`git clone` はワークスペースルートで統合ターミナルへ送る。`ccnavi-git.sh` は `clone` を「利用者に依頼する」として
通さないので、その依頼を受けた利用者がこの画面から打つ形になる。認証の対話はターミナルで人が行い、完了は
`projects/<名前>/.git` が現れたことで知る。
URL は https / ssh / scp 風の 3 つの形だけで、資格情報入りは通さない（ターミナルの履歴と `.git/config` に平文で残る）。
名前は ASCII に絞る（`project:`、`wip/<名前>/`、`logs/<名前>/` にそのまま使う）。同じリポジトリを既に clone
していれば、origin を比べるための鍵（scheme・ユーザ・ポート・`.git` を落とした `host/path`）で止める。
clone 後の設定（`.gitignore` に置き場を足す、`config/rules.yml` にワークスペースのルールを写す）は、人が
ボタンを押したときだけ書く。写すときは文面の `sh .claude/scripts/` を `sh {root}/.claude/scripts/` に置き換える（§25.8）。
`fetch` / `pull` も生の git をターミナルへ送る。プロジェクトを外す操作は入れない。作業ツリーと写しが残ったまま
ディレクトリだけ消してしまう事故につながるし、消したいならエクスプローラで消せる。
...
| SubagentStart の一覧（§24.9） | 写し、作業ツリー | cwd の作業ツリーが親ならその親の開いた子、子ならその子。作業ツリーの有無と、先行が閉じた写しにあるかを添える。main なら無し |
...
## 25. 複数のリポジトリを 1 つのワークスペースの下で扱う

§24 までは、Claude Code が開いているワークスペースルートが git リポジトリ 1 本のルートでもある前提で書いてある。
この章はその前提を外す。要求は requirements.md の REQ-MLT。

呼び名は 2 つ。**ワークスペースルート**は Claude Code を起動した場所（`CLAUDE_PROJECT_DIR`）。
**git プロジェクトルート**は `.git` がある場所で、ワークスペース自身、`projects/` の下の各プロジェクト、
作業ツリーのそれぞれがこれを持つ。この 2 つは文書とコードの全体で同じ意味に揃えてある。

### 25.1 何を解くか

道具（hook の登録、実行ファイル、保護済みスクリプト、スキル、CLAUDE.md）は public に出したくないワークスペースで、
private のリポジトリに置く。直したいプロジェクトは別々のリポジトリで、ワークスペースの下に clone して Claude Code に作業させる。
ルールはプロジェクトごとに違い、プロジェクトの git で育てる。1 つのセッションが複数のプロジェクトを横断し、issue からプロジェクトを決める。

今の作りでは 5 つが成り立たない。前の 3 つはルートとルールファイルの数の話、後ろの 2 つは設定 3 本の持ち方の話。
...
- ワークスペースのルールがプロジェクトのファイルに効かない。どのツリーでも効いてほしい deny（hook の守り、認証情報、main への直書き）を
  プロジェクトごとに写して育てることになり、写し忘れたプロジェクトが穴になる
- フェーズの種類とリスクの配点がワークスペースに 1 本しか無い。種類の `scope` は作業ツリーのレイアウトに縛られるので、
  レイアウトの違う 2 つ目のプロジェクトで破綻する

骨は 4 つ。**道具はワークスペース、設定はプロジェクト、git はツリーごと**。**設定 3 本（ルール・フェーズの種類・リスクの配点）は
「共通層 + そのツリーの層」の和で判定する。足すだけ、上書き無し、厳しいほうが勝つ**。**Write / Edit / NotebookEdit は共通層 + 行き先の 1 層、
Bash は共通層 + 全部の層**。**プロジェクトを決めるのは人の承認で、判定はその値を見ない**。

採らなかった形も書いておく。Bash の呼び出しがどのプロジェクトのものかを `cwd` と `cd` の追跡と引数の語の走査で当てる案は、
当てるための仕掛けが「どのルールファイルを引くか」の 1 点にしか効かず、コマンドの副作用は結局実行後の監視が拾う。
仕掛けの量と得るものが釣り合わないので、当てるのをやめて和を取る。
...
| 提案 | ワークスペースの `wip/<プロジェクト>/tickets/<状態>/<識別子>.md`。ワークスペース自身の作業は今までどおり `wip/tickets/<状態>/` | ワークスペース |
| 承認済みの写し、閉じた写し、フェーズの印 | ワークスペースの `.claude/ccnavi/tickets/`。今と同じ。写しに `project:` が入る | 管理外 |
| 記録、控え、セッションの状態 | ワークスペースの `.claude/ccnavi/`。今と同じ | 管理外 |
...
§25.1 のまま）と、3 本をそれぞれ別の環境変数で動かす形（動かす理由が無く、つまみが増えるだけ）。

**読むのは常に git プロジェクトルートに checkout されている版。** ブランチは問わない。作業ツリーの中の `.ccnavi/` は読まない（REQ-MLT-04）。
ワークスペース自身の層も同じで、読むのはワークスペースルートの `.ccnavi/config/` であり、ワークスペースから切った作業ツリーの中の写しは読まない。
...
**プロジェクトの置き場は 1 段に固定する。** `projects/` の直下で `.git` を持つディレクトリだけをプロジェクトと数え、名前がそのままツリーの
名前になる。ワークスペースの直下を歩いて探す形にしないのは、列挙が速いことと、何がプロジェクトかで迷わないため。`projects/` はワークスペースの
`.gitignore` に入れる。入っていなければ `--lint` が warn で言う。

**プロジェクトのリポジトリに書くのは層の傘だけ。** 提案と git ラッパの記録は「ワークスペースルートからの相対で、プロジェクトの名前を挟む」に置く。
プロジェクトの中に置くと public のリポジトリにワークスペースの運用の痕跡が出る。プロジェクトが無いとき（`projects/` が無いか空）は
今の置き場のままで、自身の層への移行（§25.12）を除いてこの章の前と同じに動く。

### 25.3 ツリー

ツリーは 3 種になる。ワークスペースルート、 git プロジェクトルート（`projects/<名前>`）、作業ツリー（`.claude/worktrees/<識別子>`）。
`Tree` は `repo` を持ち、ワークスペースなら空、プロジェクトならその名前、作業ツリーなら切り元のプロジェクトの名前になる。

作業ツリーの確かめ方は §24.4 の相互参照のまま。ただし `.git` ファイルの `gitdir:` が指す先は、ワークスペースではなく
プロジェクトの `.git/worktrees/<識別子>` になる。どのプロジェクトから切られたかは、`gitdir:` の指す `.git` の親で分かる。
ワークスペースから切った作業ツリー（ワークスペース自身の作業）も今までどおり数える。

`tree_of` は最長一致のまま。候補が 3 種に増えるだけで、`.claude/worktrees/x/src/a` がワークスペースルートに畳まれない理由も同じ。
上限はワークスペースルートで、外に行き先があれば `None`。列挙はファイルシステムだけで行い、git を起こさない。
...
`additionalContext*` / `additionalContext*File` / タイプ）が一致する定義は、同じルールの写しとみなし、後ろの層のものを捨てる。`--lint` が
info で言い、`--explain` は残った 1 本だけ出す。Bash の和でも同じ。見本を写して始めたプロジェクトが共通層と同じ行を持つのは普通の形で、
それを衝突と呼ぶと本当の衝突が埋もれる。
...
層の名前付きで残し、`--lint` が error で言う。組み込みの既定へは落とさない（§25.2）。
...
読み込みは層の数だけ増える。1 本 200 行の YAML で数 ms なので、実行前の判定の期限の中に収まるが、`ms` の欄で見る（§25.10）。

#### 25.4.1 フェーズの種類の合成

**どの層を足すかは親の写しの `project:` で決まる。** 人が承認した値で、子は親から継ぐ。空ならワークスペース自身の層。作業ツリーの切り元は
`project:` と合うことを着手時に確かめてある（REQ-MLT-13）ので、種類を選ぶのに申告の値を使っても、判定が申告に依存する形にはならない。
...
どちらの `review:` が効いているかを人が読めない。記録の `fallback` に層の名前を残すのは配点の側だけ（§25.4.2）。種類の衝突は承認が
...
**どの層を足すかはフェーズの種類と同じ。** 子を閉じるときに、親の写しの `project:` の層を共通層に足す。

**項目は連結、閾値はキーごとに小さいほう。** 共通層の項目の後ろに層の項目を並べる。同 `id` は error（今の 1 本の中の「重複している」を
層をまたいで適用）。全欄一致なら写しとして後ろを捨てる。`levels` は `medium` / `high` / `critical` のそれぞれで、**書かれた鍵だけが参加し**、
...
frontmatter に `project:` を足す。親だけが書き、子は親から継ぐ。値は `projects/` の名前。

```yaml
---
version: 2
ticket: i0050
project: lib                # 親だけ。projects/ の名前
issue: 50
...
---
```

- **決めるのは人。** 承認画面が `project:` を出し、`projects/` に無い名前なら承認しない。issue からプロジェクトを探すのは
  エージェントと sh の仕事で、各プロジェクトの `origin` をローカルの git config で読み、URL と突き合わせて候補を出す。
  1 件に絞れなければ人に聞く。決まった値は写しに残る
- **判定は申告を見ない。** 書き込みは行き先のツリーで判定され、そのツリーに結び付くチケットの写しの `project:` と
  ツリーの `repo` が違えば止める（`DENY_TICKET_REPO_MISMATCH`）。エージェントがプロジェクトを思い違えても、
  違うプロジェクトに書いた時点でそこのルートで判定されて止まる。検知の正しさにガードが依存しない
- **作業ツリーはプロジェクトから切る。** `.claude/worktrees/<識別子>` を `projects/<repo>` の worktree として作る。
  切るのは親が git ラッパの `worktree add` で行い、プロジェクトの中で打つ（ラッパは `cwd` の git を使う）。
  `ticket start` は、その作業ツリーの切り元が写しの `project:` と合うことを確かめてから動かす
- **提案の置き場は `wip/<repo>/tickets/`。** 親と子は同じ `repo` なので同じ場所に並ぶ。ワークスペース自身の作業は
  `project:` を書かず、`wip/tickets/` のまま

### 25.6 守るもの

selfguard の中核（ルールファイルの外で、組み込みで、控えと復元まで持つ対象）は次のとおり。
...
| 作業ツリーの中の写し | 上の各ファイルのうち追跡されているものの、**その切り元から切った**作業ツリーの中の写し |
...
**予約名のプロジェクトの 3 本も守る。** 判定の層としては数えない（§25.4）が、ファイルは控えて戻す。`--lint` が名前を変えるよう言っている
...
控えと復元の対象ではなかった。ルールで止める形は、ルールの並びと書き方で緩みうるし、作業ツリーの写しは統合で main へ入る道を持つ。
中核へ入れて、シェルからの書き込みは組み込みで止め、書けても戻す。名指しのツールから共通層の 3 本を守るぶんは、今も共通層のルールの
1 行（`/ccnavi-config` への案内付き）に任せる。そこは `deny` を 1 行書けば済み、書いたことが読める場所に残る。組み込みで名指しのツールを
止めるのは、置き場が設定で動く実行ファイルと層の傘の 2 つだけ。

**プロジェクトから切った作業ツリーの写しを足すのも既存の穴の修正。** 以前は作業ツリーを切り元無しで列挙していたので、プロジェクトから切った
作業ツリーは列挙されず、写しの相対も「ワークスペースルートから」で組んでいた（`projects/lib/...`）ため、作業ツリーの中にその綴りは無かった。
切り元付きに列挙し、写しの相対を切り元の git プロジェクトルートから組む（REQ-MLT-08）。

**`.ccnavi/scripts/` は中核に入れない。** スクリプトは配点から `script:` で参照されて初めて効き、参照が無ければ何もしない。配点が中核で
守られていれば、新しい参照を足すことはできない。残る道は「既に参照されているスクリプトの中身を書き換える」で、これは次の組み込み deny と
`CCNAVI_RESTORE_IF_DENY` で止めて戻す。中核の控えと復元まで持たせると、ファイル数が定まらないものを呼び出しのたびに読んで比べることになり、
実行前の判定に張った期限に効く。数が定まっているのは設定 3 本のほうで、そこが中核と中核でないものの境になる。
...
ワークスペースルート。sh はそこにしか無く、拒否の文面が案内する綴りはそこを指すから。重複の判定（§25.4）も置き換えた後の欄で比べる。
共通層とプロジェクトの層に同じ `{root}/...` を写した定義は、置き換え先が同じなので写しとして一致する。書いた綴りで比べると、
同じ場所を指す 2 本を別物として両方効かせることになる。

フェーズの種類と配点の欄にはパスの綴りを書く場所が `script:` しか無く、`script:` は §25.4.2 の解決先で決まるので `{root}` は使わない。
...
作業ツリーの切り元が写しの `project:` と合うか）に次を足す。`.ccnavi/config/` が無いことは言わない。無いのは正常で、言うと本当に言うべきものが埋もれる。
...
**`--approve` の終了コード。** 束の一部が落ちたら 1。通ったぶんの写しは置き、落ちたものは名指しする。成功で終わると、端末を見ていない側
（スクリプト、CI）は全部通ったと読む。
...
- Bash ツールの環境に `CLAUDE_PROJECT_DIR` が来るか。**来ない（実測済み）。** 案内は `{root}` で書く（§25.8）
...
- VS Code 拡張の追従。拡張のプロジェクト管理画面とルール設定画面は、今も旧の置き場（`CCNAVI_PROJECT_RULES`、既定 `config/rules.yml`）を
  読み書きし、`--explain --json` の `layers[]` も読まない。代償として、拡張で開いて保存したプロジェクトのルールは判定に効かない
  （`--lint` が旧の置き場として warn で言う）。フェーズの種類と配点の画面も足さない。拡張とその文書は別に直す
...
- `.ccnavi/scripts/` を selfguard の中核にする形（§25.6）。中身のハッシュを突き合わせる形も入れない
...
- 層の数だけ読み込みが増える。1 本 200 行の YAML で数 ms なので期限の中に収まるが、`ms` の欄で見る（§25.10）
...
2. 新しい実行ファイルを組んで、ワークスペースルートの配り先へ人が配る
...
旧ファイルの削除は人が行う。ルールの案内用の 1 行と selfguard の写しの保護が先に当たるので、エージェントには消せない。
