...
呼び名を 2 つ決めておく。**ワークスペースルート**は Claude Code を起動した場所（`CLAUDE_PROJECT_DIR`）。
**プロジェクトルート**は `.git` がある場所で、ワークスペース自身、`projects/` の下の各プロジェクト、作業ツリーの
それぞれがこれを持つ。前の章までの「プロジェクトルート」は、この章のワークスペースルートのこと。
...
| 何 | 場所 | git |
|---|---|---|
| hook の登録、実行ファイル、保護済みスクリプト、スキル、CLAUDE.md | ワークスペースルート（ワークスペースルート） | ワークスペース |
...
| プロジェクトのルール | `projects/<名前>/config/rules.yml`（`CCNAVI_PROJECT_RULES`、既定 `config/rules.yml`、プロジェクトルートからの相対） | プロジェクト |
...
ツリーは 3 種になる。ワークスペースルート、プロジェクトルート（`projects/<名前>`）、作業ツリー（`.claude/worktrees/<識別子>`）。
...
`tree_of` は最長一致のまま。候補が 3 種に増えるだけで、`.claude/worktrees/x/src/a` がワークスペースルート に畳まれない理由も同じ。
...
そのプロジェクトルート にある `config/rules.yml`。ワークスペースのツリーならワークスペースのルール。ルールは今までどおり解いた先の絶対パスに
当てるので、書き方は変わらない。プロジェクトのルールに `*/schema/*` と書けば、プロジェクトルート でも、そこから切った作業ツリーでも当たる。

作業ツリーの中に checkout されている `config/rules.yml` は読まない。読むのはプロジェクトルート の版 1 本。ブランチで
...
`git status` をツリーごとに起こす。見るのはワークスペースルート と、そのターンで書き込みのあったプロジェクトルート と作業ツリー。
触ったツリーはセッションの控え（`session/<id>.trees`）が既に持っているので、それを使う。基準（ターンの始まり）と
「もう伝えた」の控えはツリーごとに分ける。差し戻しはそのツリーの git で行う。報告はツリーの名前を頭に付ける。

プロジェクトルート で見た変更に当てるのはそのプロジェクトのルール、作業ツリーならそのツリーに結び付くチケットの範囲とプロジェクトのルール。
