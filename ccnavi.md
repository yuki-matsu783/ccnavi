...
| 何 | 場所 | 誰が書く | git |
|---|---|---|---|
| 提案（親も子も） | 親の作業ツリーの `wip/tickets/<識別子>.md` | 親 | 親のブランチにコミット |
| 承認済みの写し | main の `.claude/ccnavi/tickets/<識別子>.md` | `ccnavi --approve`（人） | 管理外 |
| 閉じた写し | main の `.claude/ccnavi/tickets/closed/<識別子>.md` | hook が `status: done` を見て動かす | 管理外 |
| レビュー済みの印 | main の `.claude/ccnavi/tickets/phases/<親>/<N>.reviewed` | レビュー確認スクリプト、または `ccnavi --reviewed` | 管理外 |

置き場は 2 本の設定で差し替える。`CCNAVI_TICKETS`（提案、既定 `wip/tickets`、各作業ツリーの根からの相対）と
`CCNAVI_APPROVED`（写し、既定 `.claude/ccnavi/tickets`、main の根からの相対）。`closed/` と `phases/` は写しの下に固定。
`CCNAVI_TICKET` と `CCNAVI_LEDGER` は廃止し、`--lint` が「もう効かない」と言う。
...
version: 1
ticket: PROJ-1234-a
parent: PROJ-1234        # 子だけ。親は書かない
phase: 2                 # 子だけ。同じ親の同じ番号が 1 つの束
status: open             # 親が done にすると閉じる
title: 設定画面の分割
rationale: |
...
`status: done` は閉じる印。範囲が消える向きなので承認無しで効く（§24.6）。
...
相互参照でも確かめる。前置だけだと、同じ名前のただのディレクトリを作業ツリーと読み違える。
...
閉じるのは親だけ。`.claude/scripts/` に置いた保護済みスクリプトが提案の frontmatter に `status: done` を書く。
PreToolUse の Bash 入力に `agent_id` が付いていたら、このスクリプトの呼び出しを止める。
スクリプトがやるのは印を書くことだけで、作業ツリーの削除は親のマージ手順（git ラッパ）に任せる。

hook が `status: done` を見たら写しを `closed/` へ動かす。hook は書けるので、エージェントに `.claude/` を書かせずに済む。
再開は人が写しを戻す。

作業ツリーが消えているだけの子は「効かない」だが「閉じた」ではない。閉じ方はスクリプト 1 本に寄せる。

### 24.7 フェーズと門

同じ親の同じ `phase` の子が全部 `status: done` になったとき、そのフェーズは終わり。
最後の子を閉じた呼び出しの PostToolUse で、`additionalContext` に「ターンを終えて利用者にレビューを頼め。
指摘があれば次の子を切れ」を返す。

それでも進もうとしたら門が止める。門は cwd の作業ツリーで親を引く。`cwd` が `.claude/worktrees/<親>/` の中なら
その親のフェーズ状態を見る。別の親は別の門を持ち、main や無関係のツリーからの呼び出しに門は無い。

| 呼び出し | 門の中 |
|---|---|
| `Agent`（サブエージェントの起動） | 止める。reason に利用者への問い合わせを入れる |
| Bash | 止める。ただし閉じるスクリプト・レビュー確認スクリプト・git ラッパは通す |
| Write / Edit | 通す。次のフェーズの計画（`wip/tickets/`）はレビュー前に進めてよい |

git ラッパを通すのは、親が門の中でも子の成果をマージして片付ける必要があるため。ラッパは push を通さないので、
門の中で通しても外へ出るものは無い。

Stop の exit 2 は「続行させる」道具で、「ターンを終えて人に聞け」を強制する手段は hook に無い。
だから先に文で言い、進んだら止める、という 2 段になる。

`cwd` は `cd` 1 回で外れる。門はそこで緩む向きに外れうるが、外れた先で起動したサブエージェントの書き込みは
行き先で止まるので、致命傷にならない。書き込みは行き先、起動とシェルは `cwd`、と鍵を使い分ける。

### 24.8 レビュー確認

門を開けるのは保護済みのレビュー確認スクリプト。親が Bash で打つ。エージェントはスクリプトを書き換えられず、
スクリプトはリモートの実物しか見ないので、打たせても門は緩まない。

1. 親ブランチの名前から MR / PR を引く（GitLab は `source_branch`、GitHub は `head`）。無ければ門は開かない
2. 未解決のスレッドを数える。0 なら `phases/<親>/<N>.reviewed` を置く
3. 残っていれば印を置かず、未解決の一覧を返す

未解決を残したまま進める判断は人が端末で `ccnavi --reviewed <N> --accept-unresolved` を打つ。
`--approve` と同じ対話の形で、残っているスレッドを見せてから y/N。チャットの「進めてよい」は hook が見られず、
エージェントが「人が許した」と主張して進める形は、止められた側が門を開ける形になる。

GitLab と GitHub はリモート URL のホストで見分ける。トークンは `GITLAB_TOKEN` / `GITHUB_TOKEN`。
`gh` や `glab` は無い前提で HTTPS API を直接叩く。どちらが要るかは `--lint` が言う。

同じ番号のフェーズに子を足せる。足した子が承認された時点で、その番号の印は消す。
再び全部閉じたら門がまた閉じ、レビューをもう一度頼む。印は「その時点の子が全部見られた」以上の意味を持たない。
...
| SubagentStop | サブエージェントの作業ツリーに範囲外の変更が残っていれば、1 回だけ差し戻す。Stop の「ターンで変わった保護領域を報告する」と同じ形。未コミットの有無は見ない（共有状態に触る検査は親の Stop） |
...
- ブランチの親子とマージ先。確かめるのは作業ツリーの名前と識別子の一致だけ。切り方と戻し方は親の手順が持つ
