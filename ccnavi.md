...
§24 までは、Claude Code が開いているプロジェクト根が git リポジトリ 1 本の根でもある前提で書いてある。
...
- 根が 1 つ。相対パスの解決、実行後の監視、作業ツリーの列挙が、すべてプロジェクト根 1 か所を向く。
...
| 何 | 場所 | git |
|---|---|---|
| hook の登録、実行ファイル、保護済みスクリプト、スキル、CLAUDE.md | 資産の根（プロジェクト根） | 資産 |
