...
識別子を並べた `ccnavi --approve <識別子>...` はその分だけの束（VS Code 拡張が絞り込みで見えている承認待ちを渡す）。
絞りは束を狭めるだけで、絞らない束で落ちるものを通さない。承認待ちに無い識別子が混じれば何も承認しない。
親の改版が承認待ちなのに束に無い子も何も承認しない（外すと写しの旧計画で検証されるため）。親を外した子は「親が承認されていない」で落ちる。
落ちた親は親子を引く池に入れない。残すと、親の承認を通らずに子の写しができる。
...
frontmatter に `project:` を足せる。値は `projects/` の名前で、提案を置いた場所と同じでなければならない。
真はあくまで置き場で、この欄は照合のためにある（書かなくてよい）。

```yaml
---
version: 1
ticket: i0050
project: lib                # 置き場と同じ名前。省ける（置き場が決める）
issue: 50
...
---
```

- **決めるのは置き場。** プロジェクトは提案を置いた場所が決める（`wip/<名前>/tickets/` ならその名前、作業ツリーの中なら
  その切り元、ワークスペースの `wip/tickets/` なら空）。`scan` がそれを入れ、写しに残す。frontmatter の `project:` は
  宣言ではなく照合で、置き場と違えば承認しない。親と子は同じ置き場に並ぶので、承認が子に継がせる段は無い。
  承認画面はプロジェクトと提案の道を出し、人はそこで、書き込みが向かうリポジトリを見て承認する。
  issue からプロジェクトを探すのはエージェントと sh の仕事で、各プロジェクトの `origin` をローカルの git config で読み、
  URL と突き合わせて候補を出す。1 件に絞れなければ人に聞く
...
- **提案の置き場は `wip/<repo>/tickets/`。** 親と子は同じ `repo` なので同じ場所に並ぶ。ワークスペース自身の作業は
  `wip/tickets/` のまま。`projects/` に無い名前の置き場（`wip/<名前>/tickets/`）は走査されないので、
  そこに提案を置いても画面にもボードにも出ない。`--lint` がその置き場を名指しする
...
持っていないか。作業ツリーの切り元が写しの `project:` と合うか。走査されないチケットの置き場（`projects/` に無い名前の `wip/<名前>/tickets/`）が
残っていないか。
