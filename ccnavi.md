...
> 食い違いには 2 種類あり、扱いが違う。
>
> **まだ作っていないだけのもの。** チケット、承認ゲート、レイヤ合成、
> 実行後の監視、診断コマンド。この文書のとおりに作ってよい。
>
> **判断が変わったもの。この文書が古い。**
>
> | この文書の記述 | 現在の判断 |
...
> 転記注記: T-2 と T-5 の対処箇所は原文のとおりに写しているが、文書内の実際の規定はそこに無い。T-2 の「上位レイヤ単独ツリーでの判定」は §11.1 の順 3 にあり、§11.1 の本文自身が「順 3 の判定を Layer 1 ツリー単独で行う理由（T-2 への対処）」と述べている（§11.3 は具体例の節）。T-5 の上限クランプは §8.6 にあり、§21 の #14 も §8.6 を挙げる（§8.7 は「⑥⑦⑧その他」）。原文の参照先の食い違いをそのまま残している。
...
> 転記注記: この章と節の見出しは原文と一致していない。原文は「6. 設定レイヤと合成モデル」「6.1 レイヤ定義」である。また原文の表は Layer 0-A の行（`.claude/settings.json` / 人間（PR）/ 極低 / 最高（Claude Code が強制））から始まるが、以下の表は 0-B から始まっており 1 行足りない。この章は本 issue の取り込み範囲の外（既存の記述）なので本文には手を入れず、原文との差だけを記す。
...
> 転記注記: この見出しの下にあるはずの判定結果の表が既存の記述に無い。secrets/keys のくり抜き・migrations の緩和・`"/"` の広域について、それぞれどう判定されるかが読めない状態になっている。このページは提供された写真にも写っていないため補えない。この節は本 issue の取り込み範囲の外（既存の記述）なので本文には手を入れていない。
...
クォート保全には実装上の限界が 1 つある。`shlex` は引用された `<<` と素の `<<` を
同じ文字列で返し、どちらだったかを問い合わせる手段を公開していない。そのため
`grep -n "<<" README.md` はヒアドキュメントの始まりと区別が付かず、閉じない本文として
解析不確定に倒れ、②の deny に当たる。
...
> 転記注記: この表は原文でも 4 行で、付録 B.3 がキャッシュ ✗ とする `IMPL_TICKET_UNAPPROVED`（チケット未承認による allow → ask 降格）を含まない。付録 A.4 の早見表にも該当する発生源の行が無い。原文の食い違いをそのまま残している。§17.5 の実行時検証は未承認チケットで全承認キャッシュを即時破棄すると定めており、B.3 の ✗ はそれと整合する。
...
> 転記注記: スコア 40 ちょうどの扱いが原文の中で揺れている。上のレベル表は「40–69 HIGH」で 40 を含むが、§17.4 の本文と §8.7 の `high_risk_threshold: 40` のコメントは「超える場合」「このスコア超」で 40 を含まない。§16.5 の運用方針は `risk_score >= 40` と書く。どれも原文のとおりに写している。
...
> 転記注記: 「検知層」列の丸数字は原文でも 2 つの体系が混ざっている。#1・#3 の ⑥（DB 破壊キーワード検知）と #8・#9 の ②（sandbox_root 境界検査）は §5.1 Stage 1 の ①〜⑦ を指し、#4 の ③（コマンド別パス権限）と #6・#7 の ①（トークン分解）は §12.3 の ①〜⑤ を指す。#2 の「②⑥」は 1 セルの中で両方の体系にまたがる。原文の写真と照合しており、転記の誤りではない。
...
| L-7 | 引用された `<<` を素の `<<` と区別できない | `grep -n "<<" README.md` のように、ヒアドキュメントに言及するだけのコマンドが deny になる | 許容する。止まる側に倒れ、文面が解析不確定であることを名乗る。対象をファイルへ逃がせば回避できる。§12.3 ① |
...
> 転記注記: 上の「作業中」「やってはいけないこと」の 2 つの箇条書きは、原文でもコードブロックの枠の外に置かれている（枠は `- [ ] 既存の振る舞いが変わっていないこと` で閉じる）。`.current-ticket.md` のテンプレートの一部ではなく、テンプレートに続く解説として写している。
