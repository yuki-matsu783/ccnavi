...
## 25. 複数のリポジトリを 1 つの資産の下で扱う

§24 までは、Claude Code が開いているプロジェクトルートが git リポジトリ 1 本の根でもある前提で書いてある。
この章はその前提を外す。要求は requirements.md の REQ-MLT。

### 25.1 何を解くか

道具（hook の登録、実行ファイル、保護済みスクリプト、スキル、CLAUDE.md）は public に出したくない資産で、
private のリポジトリに置く。直したい対象は別々のリポジトリで、資産の下に clone して Claude Code に作業させる。
ルールは対象ごとに違い、対象の git で育てる。1 つのセッションが複数の対象を横断し、issue から対象を決める。

今の作りでは 3 つが成り立たない。

- 根が 1 つ。相対パスの解決、実行後の監視、作業ツリーの列挙が、すべてプロジェクトルート 1 か所を向く。
  資産が git 管理下にあればその git だけを見て対象の変更は見えず、無ければ `top_level` が上へ抜けるか止まる
- ルールファイルが 1 本。対象ごとのルールを置く場所が無い
- 対象に `.claude/` を置く形は採れない。Claude Code がそこのスキルを読み込み、`--lint` が迷い子として拾う

骨は 3 つ。**道具は資産、設定は対象、git はツリーごと**。**Write / Edit は行き先の対象のルール、Bash は全対象の和**。
**対象を決めるのは人の承認で、判定はその値を見ない**。

採らなかった形も書いておく。Bash の呼び出しがどの対象のものかを `cwd` と `cd` の追跡と引数の語の走査で当てる案は、
...
| 何 | 場所 | git |
|---|---|---|
| hook の登録、実行ファイル、保護済みスクリプト、スキル、CLAUDE.md | 資産の根（プロジェクトルート） | 資産 |
| 資産のルール | `.claude/ccnavi/rules.yml`（`CCNAVI_RULES`、今と同じ） | 資産 |
| 対象 | `projects/<名前>/`（`CCNAVI_PROJECTS`、既定 `projects`、資産の根からの相対） | 資産では無視。対象自身の git |
| 対象のルール | `projects/<名前>/config/rules.yml`（`CCNAVI_PROJECT_RULES`、既定 `config/rules.yml`、対象の根からの相対） | 対象 |
| 作業ツリー | `.claude/worktrees/<識別子>/`。対象から切る | 管理外 |
| 提案 | 資産の `wip/<対象>/tickets/<状態>/<識別子>.md`。資産自身の作業は今までどおり `wip/tickets/<状態>/` | 資産 |
| 承認済みの写し、閉じた写し、フェーズの印 | 資産の `.claude/ccnavi/tickets/`。今と同じ。写しに `repo:` が入る | 管理外 |
| 記録、控え、セッションの状態 | 資産の `.claude/ccnavi/`。今と同じ | 管理外 |
| git ラッパの記録 | 資産の `logs/<対象>/`。資産自身は `logs/` | 管理外 |
| フェーズの種類、リスクの配点 | 資産に 1 本。対象ごとに変えるのは今回入れない | 資産 |

**対象の置き場は 1 段に固定する。** `projects/` の直下で `.git` を持つディレクトリだけを対象と数え、名前がそのままツリーの
名前になる。資産の直下を歩いて探す形にしないのは、列挙が速いことと、何が対象かで迷わないため。`projects/` は資産の
`.gitignore` に入れる。入っていなければ `--lint` が warn で言う。

**対象のリポジトリに書くのはルールだけ。** 提案と git ラッパの記録は、今は各ツリーの根からの相対に置いているが、
対象の中に置くと public のリポジトリに資産の運用の痕跡が出る。どちらも「資産の根からの相対で、対象の名前を挟む」に
変える。対象が無いとき（`projects/` が無いか空）は今の置き場のままで、この章の前と同じに動く。

### 25.3 ツリー

ツリーは 3 種になる。資産の main、対象の main（`projects/<名前>`）、作業ツリー（`.claude/worktrees/<識別子>`）。
`Tree` は `repo` を持ち、資産なら空、対象ならその名前、作業ツリーなら切り元の対象の名前になる。

作業ツリーの確かめ方は §24.4 の相互参照のまま。ただし `.git` ファイルの `gitdir:` が指す先は、資産ではなく
対象の `.git/worktrees/<識別子>` になる。どの対象から切られたかは、`gitdir:` の指す `.git` の親で分かる。
資産から切った作業ツリー（資産自身の作業）も今までどおり数える。

`tree_of` は最長一致のまま。候補が 3 種に増えるだけで、`.claude/worktrees/x/src/a` が資産の main に畳まれない理由も同じ。
上限は資産の根で、外に行き先があれば `None`。列挙はファイルシステムだけで行い、git を起こさない。

### 25.4 ルール

**書き込み系（Write / Edit / MultiEdit / NotebookEdit）は行き先で 1 本に決まる。** 行き先のツリーの `repo` が対象なら、
その対象の main にある `config/rules.yml`。資産のツリーなら資産のルール。ルールは今までどおり解いた先の絶対パスに
当てるので、書き方は変わらない。対象のルールに `*/schema/*` と書けば、対象の main でも、そこから切った作業ツリーでも当たる。

作業ツリーの中に checkout されている `config/rules.yml` は読まない。読むのは対象の main の版 1 本。ブランチで
変えたルールがそのブランチの判定に効く形は、人が見ていないルールが効く形になる。

**Bash は全部の和。** 資産のルールと、全対象のルールを区画ごとに連結して当てる。`deny` `ask` `allow` の順は変わらない。
どの対象に `cwd` があっても、`cd` を挟んでも、同じ判定になる。

代償は allow の共有 1 つ。対象 A だけが許したい `npm test` は対象 B に居ても通る。`deny` と `ask` は増える側なので
安全に倒れ、緩むのは allow の共有だけ。逆に「対象 A に居るときだけ止める」は書けない。要るなら採らなかった形に戻るが、
Bash の allow はどのプロジェクトでも似た形になるので、まず要らないと見ている。

**識別子には対象の名前を添える。** 記録の `rules` と文面の `source` は `lib:source` のように書く。対象どうしで同じ `id` が
あっても衝突しない。資産のルールは今までどおり裸の `id`。

**読めない対象のルールは組み込みに落ちる。** その対象への書き込みは組み込みの既定で判定し、Bash の和からはその対象を外す。
外したことは記録の `fallback` に対象の名前付きで残し、`--lint` が言う。資産のルールが読めないときの挙動は今と同じ。

読み込みは対象の数だけ増える。1 本 200 行の YAML で数 ms なので、実行前の判定の期限の中に収まるが、`ms` の欄で見る（§25.10）。

### 25.5 チケット

frontmatter に `repo:` を足す。親だけが書き、子は親から継ぐ。値は `projects/` の名前。

```yaml
---
version: 2
ticket: i0050
repo: lib                # 親だけ。projects/ の名前
issue: 50
...
---
```

- **決めるのは人。** 承認画面が `repo:` を出し、`projects/` に無い名前なら承認しない。issue から対象を探すのは
  エージェントと sh の仕事で、各対象の `origin` をローカルの git config で読み、URL と突き合わせて候補を出す。
  1 件に絞れなければ人に聞く。決まった値は写しに残る
- **判定は申告を見ない。** 書き込みは行き先のツリーで判定され、そのツリーに結び付くチケットの写しの `repo:` と
  ツリーの `repo` が違えば止める（`DENY_TICKET_REPO_MISMATCH`）。エージェントが対象を思い違えても、
  違う対象に書いた時点でそこの根で判定されて止まる。検知の正しさにガードが依存しない
- **作業ツリーは対象から切る。** `.claude/worktrees/<識別子>` を `projects/<repo>` の worktree として作る。
  切るのは親が git ラッパの `worktree add` で行い、対象の中で打つ（ラッパは `cwd` の git を使う）。
  `ticket start` は、その作業ツリーの切り元が写しの `repo:` と合うことを確かめてから動かす
- **提案の置き場は `wip/<repo>/tickets/`。** 親と子は同じ `repo` なので同じ場所に並ぶ。資産自身の作業は
  `repo:` を書かず、`wip/tickets/` のまま

### 25.6 守るもの

対象のルールファイルを中核ファイルに加える。Write / Edit の拒否、シェルからの書き込みの拒否、控えと復元の 3 つとも、
今 `.claude/ccnavi/rules.yml` に掛けているものをそのまま `projects/<名前>/config/rules.yml` に掛ける。
対象のルールは行き先の判定に使うので、ここが書けると、エージェントが自分のルールを緩めて次の呼び出しから効く。

資産側の中核（hook の登録、実行ファイル、保護済みスクリプト）は今と同じ。対象の中には無いので、対象を編集している
エージェントの手が届かない。

### 25.7 実行後の監視

`git status` をツリーごとに起こす。見るのは資産の main と、そのターンで書き込みのあった対象の main と作業ツリー。
触ったツリーはセッションの控え（`session/<id>.trees`）が既に持っているので、それを使う。基準（ターンの始まり）と
「もう伝えた」の控えはツリーごとに分ける。差し戻しはそのツリーの git で行う。報告はツリーの名前を頭に付ける。

対象の main で見た変更に当てるのはその対象のルール、作業ツリーならそのツリーに結び付くチケットの範囲と対象のルール。
Bash の和はここでは使わない。実行後に見るのはファイルの変更で、行き先が分かっている。

### 25.8 保護済みスクリプトと案内

sh は資産にしかない。対象の中に `cwd` があるとき、`sh .claude/scripts/ccnavi-git.sh` は届かない。拒否の文面が案内する
綴りを `"$CLAUDE_PROJECT_DIR/.claude/scripts/ccnavi-git.sh"` に変える。Bash ツールの環境にこの変数が来るかは実測（§25.10）。
来ないなら、`settings.local.json` の `env` に `CCNAVI_HOME` を機械ごとの絶対パスで置き、そこを案内する。

git ラッパの記録は資産の `logs/<対象>/` に書く。ラッパは `cwd` の git で動くので、対象の作業は対象に入ってから始める。
これは CLAUDE.md の運用に書く。判定はそれに依存しないが、レビューの依頼と確認が `origin` を `cwd` から引くので、
そちらはこの運用が前提になる。

### 25.9 診断と記録

`--lint` が言うこと。`projects/` が資産の `.gitignore` に入っているか。各対象のルールが読めるか。対象が `.claude/` を
持っていないか。作業ツリーの切り元が写しの `repo:` と合うか。`repo:` の無い対象向けチケットが無いか。

`--explain` が示すこと。行き先のツリーとその `repo`。書き込みなら読んだルールファイル 1 本、Bash なら和に入った対象の一覧。

記録に `tree` と `repo` を足す。`rules` は対象の名前付き。dry-run でまず `repo` の分布を見て、Bash の和で増えた `ask` と
`deny` を数えてから `enable` に切り替える。

### 25.10 実装時に実測するもの

- Bash ツールの環境に `CLAUDE_PROJECT_DIR` が来るか。来なければ §25.8 の `CCNAVI_HOME`
- 対象の数に対する PreToolUse の `ms`。対象 5 本で期限の半分を超えるなら、ルールの読み込みに mtime の控えを足す
- Windows で対象から切った作業ツリーの `.git` ファイルの `gitdir:` の綴りと区切り
- `projects/` を資産の `.gitignore` に入れたとき、Claude Code が対象の中の CLAUDE.md を読むか。読まれてよいが、読まれるなら
  資産の CLAUDE.md と矛盾しないように書く
- `cwd` が対象の中にあるとき、hook の `${CLAUDE_PROJECT_DIR}` が資産の根のままか

### 25.11 見ないもの、入れないもの

- 対象ごとのフェーズの種類とリスクの配点。資産の 1 本を使う
- 対象 A に居るときだけ効く Bash のルール
- 2 段以上の深さに置いた対象、資産の外に置いた対象
