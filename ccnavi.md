...
対象ごとに 1 パネル。編集中の内容は `--project-rules-file <名前>=<パス>`（REQ-DIA-09）で渡し、
`--rules` がワークスペースのルールにすることを、プロジェクト 1 つに対して行う。この旗は診断
（`--test` / `--test-samples` / `--lint` / `--explain`）でだけ効き、hook からの判定では捨てる。
保存していないルールが判定に効く道を実行時に持たないため。守る対象（§25.6）も差し替えを見ない。
保存を止める条件は、プロジェクトのルールならそのプロジェクトの `doing` だけ。プロジェクトのルールは
そのプロジェクトへの書き込みと Bash の和にしか効かず、他のプロジェクトのセッションの書き込みの判定は変えないため
（Bash の和は変わる。そこは受け入れる）。

同じ拡張のプロジェクト管理画面は、§25 の置き場を人が育てるための入口。`--explain --json` の `trees` を
並べ、`--lint --json`（README「lint の JSON」、REQ-DIA-08）の苦情をプロジェクトごとに添える。拡張が自分で見るのは
origin（ローカルの git を読み取り専用で）、ルールファイルと `.claude/` の有無、`.gitignore` の本文、
ワークスペース直下を深さ 2 まで歩いて拾う「ccnavi が数えない `.git`」だけで、何がプロジェクトかは出し直さない。
`git clone` はワークスペースルートで統合ターミナルへ送る（`ccnavi-git.sh` は `clone` を「利用者に依頼する」で
通さない。その利用者の手がこの画面）。認証の対話はターミナルで人が行い、完了は `projects/<名前>/.git` の出現で拾う。
URL は https / ssh / scp 風の 3 形だけで、資格情報入りは通さない（履歴と `.git/config` に平文で残る）。
名前は ASCII に絞る（`project:`、`wip/<名前>/`、`logs/<名前>/` にそのまま使う）。同じリポジトリを既に clone
していれば origin の鍵（scheme・ユーザ・ポート・`.git` を落とした `host/path`）で止める。
clone 後の整合（`.gitignore` に置き場を足す、`config/rules.yml` にワークスペースのルールを写す）は、人が
ボタンを押したときだけ書く。写すときは文面の `sh .claude/scripts/` を `sh {root}/.claude/scripts/` にする（§25.8）。
`fetch` / `pull` も生の git をターミナルへ送る。プロジェクトを外す操作は持たない（作業ツリーと写しが残ったまま
消せる事故の出口になる。エクスプローラで消せる）。
