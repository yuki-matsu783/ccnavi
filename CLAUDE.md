...
- **チケットが結び付いた作業ツリーでも、そのまま書ける。** 実行前の判定はチケットの範囲を
  `tmp/` に当てない。範囲に `tmp/*` を足す必要は無い
- **外れるのはルートの直下 1 段の `tmp/` だけ。** 綴りの大文字小文字も区別する。
  `TMP/`・`tmpX/`・`docs/tmp/`・`tmp` という名前のファイルは普通の作業対象で、範囲の
  外なら止まる。`.gitignore` が追跡から外す範囲と一致させてある
- **`tmp/` が追跡されているリポジトリでは、下書きが範囲外として報告される。** 実行後の
  監視とサブエージェント終了時の検査は `tmp/` を外さない。報告が出たら、下書きを消すの
  ではなく、そのリポジトリの `.gitignore`（または `.git/info/exclude`）に `/tmp/` が
  入っているかを見る。`ccnavi --lint` が同じことを警告で言う
- **作業ツリーがまだ無いときは、セッションの scratchpad を使う。** ワークスペースルートで
  設定の下書きを作る場面がこれにあたる。ルート直下は `main-tree` が止めるので、そこに
  `tmp/` を作っても書けない。scratchpad の綴りはセッションの Environment 欄が持つ
...
| 下書きと使い捨て | その作業ツリーの `tmp/`（追跡しない）。作業ツリーが無いときはセッションの scratchpad |
