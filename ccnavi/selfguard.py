...
この 3 つは追跡されているので、`.claude/worktrees/<名前>/` の中にも同じものが入る。
この作業ツリー側の設定は、今この瞬間には誰にも読まれない。hook の登録を読むのはセッションを起こした
...
class Target:
    ...
    # 突き合わせは大きさと更新時刻で行う。
    heavy: bool = False
    # top は、この対象を git から戻すときに渡す作業ツリーのルート。作業ツリー
    # 側の設定だけが持つ。空なら root の側から戻す。main の git に
    # `.claude/worktrees/...` を聞いても、そこは `.gitignore` の中なので
    # 何も持っていない。持っているのは、その作業ツリー自身の git。
    top: str = ""
...
def _worktree_copies(
    ...
    中に対応するものは無いので、そこは対象から落ちる。プロジェクトごとの
    ルールファイルも同じ扱いで、root の下に在るぶんだけ作業ツリー側の設定を守る。
    """
