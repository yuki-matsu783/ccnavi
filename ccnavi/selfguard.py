...
コアの控えと復元まで持たせない理由はもう 1 つある。ccnavi ディレクトリの下のファイル数は定まらない。
...
このワークツリー側の設定は、今この瞬間には誰にも読まれない。hook の登録を読むのはセッションを
...
def project_home_clause(project_home: str) -> str:
    ...
    ある場合の綴り。区切りはどちらの綴りにも当て、名前がそこで終わる形（ccnavi ディレクトリごと
    消す・退かす）にも当てる。
...
def _home_name(project_home: str) -> str:
    ...
    前後の区切りは落とし、区切りを含む綴りはそのまま 1 つの節にする。
    """
...
class Target:
    ...
    # 持つのは 2 通り。ワークツリー側の設定と、プロジェクトの層の設定。main の git に
...
def _layer_home(root: str, projects_dir: str, origin: str, layer: str) -> str:
    ...
    名前を引けないものはワークスペースルートに寄せる。そこから切ったワークツリーに
...
def _worktree_copies(
    ...
    在るだけではワークツリーと呼ばず、`.git` ファイルと切り元の登録の相互参照が
    両向きに揃ったものだけを数える。参照実装の写しのような、ただの
    ディレクトリを守りに行かないため。git は起こさないので、呼び出しごとに
    通っても外部プロセスは増えない。

    切り元付きで列挙する。ワークツリーはワークスペースからもプロジェクトからも
    切れて、中に入っているワークツリー側の設定は切り元が追跡しているものだけになる。
    ...
    設定の置き場は設定で動く。切り元の外を指しているなら、ワークツリーの中に
...
def report(outcomes: list[Outcome]) -> str:
    ...
    if any(outcome.target.copy for outcome in outcomes):
        # ワークツリー側の設定が混じっている。今この瞬間の判定は変わらないので、
        # 「何も起きていないのに戻された」と読まれる。時間差で効く道であることを
        # 言わないと、次に同じ手が繰り返される。
        lines.append(
            "ワークツリー側の設定は、その場では誰も読みませんが、統合すれば main の "
            "hook の登録とルールになります。"
        )
