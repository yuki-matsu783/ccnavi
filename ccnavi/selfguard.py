...
def _folded(clause: str) -> str:
    ...
    ここで囲むのは場所の綴りだけだが、組み込みの守りも `rules._build` を通るので、
...
def common_shell_clause(root: str, path: str) -> str:
    ...
    ADR-0052 で env（`CCNAVI_RULES` など）は廃止したが、フラグは診断のために残した。
...
def _worktree_copies(
    ...
    元リポジトリを添えて列挙する。ワークツリーはワークスペースからもプロジェクトからも
