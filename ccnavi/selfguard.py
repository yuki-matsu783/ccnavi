...
それでも守るのは、ワークツリー側の設定が統合先へ入る道を持っているから。ワークツリーで書き換えて
...
def guard_shell_regex(
    ...
    止まらなくなる。2 か所で組むと、片方だけが弱いほうへずれる。
    """
...
def common_shell_clause(root: str, path: str) -> str:
    ...
    env（`CCNAVI_RULES` など）は効かず、置き場を指せるのは診断のためのフラグだけ
...
def common_layer_regex(root: str, common_files: tuple[str, ...]) -> str:
    ...
    設定は統合で統合先の設定になる道を持つ。ワークスペースルートより上で閉じないと、
