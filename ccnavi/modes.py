...
EXIT_BLOCK = 2  # 判定を書けなかったので拒否にする
# 問われたことへの「いいえ」。`--approve --preview --verify` が、承認の対象に入らないと
# 答えるときに返す。1 と分けてあるのは、1 が「使い方か設定の誤り」だから。同じ値にすると、
# 打ち方を間違えた回と提案が落ちる回が区別できず、読む側（エージェント）は、直すものが
# 無いのに提案を直しに行く。
EXIT_ANSWER_NO = 3
...
def fail_closed(mode: str) -> int:
    ...
    enable は呼び出しを止める。達せなかった判定が許可になってはならないから。
