...
下書きの置き場（`tmp/`）は、git が追跡しないので範囲を当てない。
...
def is_scratch_place(rel: str) -> bool:
    ...
    `tmp/` は `.gitignore` が追跡から外す置き場で、下書き・再現用のスクリプト・調べた
    ...
    **綴りの大文字小文字は区別する。範囲の照合（`_fold`）とは逆にしてある。** 外してよい
    理由が「追跡されない」ことにあり、追跡から外しているのは `.gitignore` の `/tmp/` で、
    その照合は Linux では区別するため。区別せずに外すと、Linux の `TMP/` が「追跡される
    のに範囲を当てない場所」になり、承認した範囲の外の変更が統合先へ乗る道ができる。
    区別する側に倒せば、どの機械でも除外は追跡から外れる範囲より狭いままで、狭いぶんは
    範囲の判定が止めるだけで済む。

    ルートの直下 1 段だけを見る。`docs/tmp/` は普通の作業対象で、`.gitignore` も外さない
    （`/tmp/` の先頭の `/` はツリーのルートに掛かる）。`tmp` という名前のファイルも置き場
    ではない（末尾の `/` はディレクトリにしか当たらない）。`tmpX/` も置き場ではない。
    """
