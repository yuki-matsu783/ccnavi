...
**VS Code 拡張はルールだけ層に追従した。** プロジェクト管理画面とルール設定画面は `--explain --json` の `layers[]` から置き場を取り、
自身の層とプロジェクトの層の `.ccnavi/config/rules.yml` を開く。旧の置き場 `config/rules.yml` が残っていれば画面が「読まれていない」と出す。
フェーズ管理画面とリスク管理画面は今も共通層の 1 本だけを開く（設計 §11.11）。
