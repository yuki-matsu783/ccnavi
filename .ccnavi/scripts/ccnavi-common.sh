...
# ディレクトリの `.ccnavi/scripts/` だけでは足りない。層の傘の下には配点が呼ぶスクリプトの
...
# 根になる。ところが `.claude/ccnavi/state/`（控え）と `logs/` は追跡外で作業ツリーには
# 無いので、それらが見つからなくなる（承認済みチケットは今は `.ccnavi/tickets/` にあり、
# git がツリーごとに運ぶ）。道具のうち
