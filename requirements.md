...
| REQ-APV-06 | ユビキタス | ccnavi は、承認を人が押したことの証拠を求めること。端末から求められた承認は標準入力が端末であることを、画面から求められた承認は見せた束と現在の束が一致することを条件とし、一致しなければ何も承認しないこと。エージェントがシェルから承認の経路を起動することは、どちらの形でも拒むこと |
| REQ-APV-07 | イベント駆動 | チケットが承認されたとき、ccnavi は、そのことを次にモデルへ渡す機会に 1 度だけ伝えること。伝える対象は、その文脈が知る前に置かれた写しを除いた、新しく置かれた写しと内容の変わった写しとすること |

REQ-APV-02 の「前回承認されたチケット」は、子チケットでは親チケットを指す。
子は親の部分集合なので、新たに書ける領域は親の分だけで、子の承認画面が示す差分は
親からどれだけ絞ったかになる。

REQ-APV-06 の「画面から求められた承認」は VS Code の拡張を指す。拡張の子プロセスは端末を
...
REQ-APV-07 は、承認の場所が端末から画面へ移ったことで要る。端末で承認していたときは、
...
| REQ-DIA-10 | イベント駆動 | 承認の束の機械可読な提示を求められたとき（`--approve --preview --json`）、ccnavi は、束・承認画面の本文・承認の対象にしない提案とその理由・読めない提案を JSON で返し、写しを置かないこと。束は承認が使うのと同じ関数で求めること |
...
その下の置き場に clone した別々のリポジトリをプロジェクトと呼ぶ。Claude Code はワークスペースを開く。ルールはプロジェクトごとにあり、
プロジェクトの git で育てる。設計は §25。
...
| REQ-MLT-03 | ユビキタス | ccnavi は、書き込み系ツールの判定に、行き先のツリーが属するプロジェクトのルールファイル（git プロジェクトルートからの相対、既定 `config/rules.yml`）を用い、プロジェクトに属さないツリーにはワークスペースルートのルールファイルを用いること |
| REQ-MLT-04 | ユビキタス | ccnavi は、プロジェクトのルールファイルとして git プロジェクトルートにあるものだけを読み、作業ツリーの中にあるものを読まないこと |
| REQ-MLT-05 | ユビキタス | ccnavi は、Bash の判定に、ワークスペースルートのルールと全プロジェクトのルールをタイプごとに合わせたものを用い、呼び出し元の作業ディレクトリによらず同じ判定を返すこと |
| REQ-MLT-06 | 状態駆動 | プロジェクトのルールファイルが読めない間、ccnavi は、そのプロジェクトへの書き込みを組み込みの既定で判定し、Bash の合成からそのプロジェクトを外し、外したことを記録と診断で示すこと |
| REQ-MLT-07 | ユビキタス | ccnavi は、記録と文面で、当たったルールの識別子にプロジェクトの名前を添えること |
| REQ-MLT-08 | ユビキタス | ccnavi は、プロジェクトのルールファイルを設定ファイルの自己防衛の対象（書き込みの拒否、シェル書き込みの拒否、控えと復元）に含めること |
...
| REQ-MLT-15 | ユビキタス | ccnavi は、プロジェクト置き場が無いか空のとき、この節の前と同じ判定・置き場・記録を保つこと |
| REQ-MLT-16 | ユビキタス | `--lint` は、プロジェクト置き場が無視されているか、各プロジェクトのルールが読めるか、プロジェクトが `.claude/` を持たないか、作業ツリーの切り元と `project:` が合うかを言うこと |
| REQ-MLT-17 | ユビキタス | `--explain` は、プロジェクトの一覧と、それぞれのルールファイルが読めるか、タイプごとの件数を示すこと |
...
| 10 | ワークスペース 1 つとプロジェクト 2 つで、プロジェクトをまたぐ入力を通す | プロジェクト A と B への書き込み、プロジェクトの外と 2 段深い場所への書き込み、プロジェクトの中に `cwd` を置いた Bash と親に置いた Bash と `cd` を挟んだ Bash、片方のプロジェクトだけが持つ `allow` `ask` `deny`、同じ `id` を持つ 2 つのプロジェクト、読めないプロジェクトのルール、プロジェクトのルールファイルへの Write とシェル書き込み、`project:` の違う作業ツリーへの着手、置き場に無い `project:` の承認、ワークスペースルートと作業ツリーで起きた実行後の変更、プロジェクト置き場が無い入力 | REQ-MLT-01〜17 |
