...
| REQ-MLT-02 | ユビキタス | ccnavi は、書き込みのプロジェクトが行き着く先の属するツリー（ワークスペースルート、プロジェクト、作業ツリー）を最長一致で決め、行き先がワークスペースルートの外にあるとき判定を持たないこと |
| REQ-MLT-03 | ユビキタス | ccnavi は、書き込み系ツールの判定に、行き先のツリーが属するプロジェクトのルールファイル（プロジェクトルートからの相対、既定 `config/rules.yml`）を用い、プロジェクトに属さないツリーにはワークスペースルートのルールファイルを用いること |
| REQ-MLT-04 | ユビキタス | ccnavi は、プロジェクトのルールファイルとしてプロジェクトルート にあるものだけを読み、作業ツリーの中にあるものを読まないこと |
...
| REQ-MLT-08 | ユビキタス | ccnavi は、プロジェクトのルールファイルを設定ファイルの自己防衛のプロジェクト（書き込みの拒否、シェル書き込みの拒否、控えと復元）に含めること |
...
| REQ-MLT-17 | ユビキタス | `--explain` は、行き先のツリーとそのプロジェクト、読んだルールファイル、Bash なら合成に入ったプロジェクトの一覧を示すこと |
