...
| REQ-CMN-02 | 常時 | ccnavi は、判定しない・警告・ブロックの 3 つの動作モードを持ち、モードの指定が無いときブロックで動作すること |
| REQ-CMN-03 | 状態 | 警告モードである間、ccnavi は、ブロックモードと同じ判定を行い、その判定を記録と応答の両方で伝えたうえで、ツールの実行を止めないこと |
| REQ-CMN-04 | 常時 | ccnavi を停止させる指定は、ccnavi を起動した側の環境からのみ効き、プロジェクト内のファイルに書いても効かないこと |
| REQ-CMN-05 | 異常時 | もしモードとして解釈できない値を与えられたならば、ccnavi は、その値を示したうえで、ブロックモードで動作すること |
...
| REQ-CMN-09 | 状態 | 戻す働きが予行と設定されている間、または警告モードである間、ccnavi は、作業ツリーのファイルを戻さず、戻していたはずの対象を示すこと |
...
| REQ-PRE-05 | 常時 | ccnavi は、自身の設定・承認済みのチケットの写し・レビュー済みの印を、エージェントの操作によって書き換えさせないこと |
...
複数あるので、チケット 1 本ごとの写しがそれを満たす（ADR-0023）。
...
| REQ-DIA-06 | 事象 | チケットの状態の機械可読な一覧を求められたとき（`--explain --json`）、ccnavi は、提案・写し・印・ゲートの開閉・作業ツリーの有無・承認待ちを、判定と同じ関数で求めた値として JSON で返し、その際にネットワークへ出ないこと |
...
| REQ-TKT-24 | 常時 | ccnavi は、提案の置き場と承認済みの写しの置き場を設定で差し替えられ、写しの置き場が保護されていないときはそれを診断で示すこと |
| REQ-TKT-25 | 事象 | 設定の検証を求められたとき、ccnavi は、チケットの無い作業ツリー・識別子と名前の一致しない作業ツリー・識別子の形が規約に合わない子・同じ識別子が複数の置き場にあるもの・先行が閉じていないのに作業中の子・読まれない場所に置かれた写し・無い認証情報・作業ツリーでも主でもないのに設定ディレクトリを持つディレクトリを示すこと |
...
| REQ-TKT-31 | 事象 | フェーズの最後の子を閉じようとしたとき、種類の成果物が親の作業ツリーに在って追跡されていなければ、ccnavi は、閉じるのを拒み、無いものを示すこと |
...
| REQ-TKT-39 | 常時 | ccnavi は、チケット制御（提案の承認・写しの範囲・フェーズのゲート・サブエージェントの制限・状態とレビューの操作）を使うかを、ルールによる判定とは別の 1 つの設定（`enable` / `disable`、既定 `enable`）で切り替えられること。それ以外の値は `enable` として動き、設定の検証が error として示すこと。写しの置き場を空にすることでは切り替わらないこと |
...
| REQ-MLT-03 | 常時 | ccnavi は、書き込み系ツールの判定に、行き先のツリーが属するプロジェクトのルールファイル（git プロジェクトルートからの相対、既定 `config/rules.yml`）を用い、プロジェクトに属さないツリーにはワークスペースルートのルールファイルを用いること |
| REQ-MLT-04 | 常時 | ccnavi は、プロジェクトのルールファイルとして git プロジェクトルートにあるものだけを読み、作業ツリーの中にあるものを読まないこと |
| REQ-MLT-05 | 常時 | ccnavi は、Bash の判定に、ワークスペースルートのルールと全プロジェクトのルールをタイプごとに合わせたものを用い、呼び出し元の作業ディレクトリによらず同じ判定を返すこと |
| REQ-MLT-06 | 状態 | プロジェクトのルールファイルが読めない間、ccnavi は、そのプロジェクトへの書き込みを組み込みの既定で判定し、Bash の合成からそのプロジェクトを外し、外したことを記録と診断で示すこと |
| REQ-MLT-07 | 常時 | ccnavi は、記録と文面で、当たったルールの識別子にプロジェクトの名前を添えること |
| REQ-MLT-08 | 常時 | ccnavi は、プロジェクトのルールファイルを設定ファイルの自己防衛の対象（書き込みの拒否、シェル書き込みの拒否、控えと復元）に含めること |
...
| REQ-MLT-11 | 常時 | ccnavi は、チケットの `project:` を人の承認で確定し、置き場に無い名前を承認しないこと |
| REQ-MLT-12 | 常時 | ccnavi は、書き込みの行き先のツリーが属するプロジェクトと、そのツリーに結び付く写しの `project:` が違うとき止めること。エージェントが申告したプロジェクトを判定に使わないこと |
| REQ-MLT-13 | 常時 | ccnavi は、作業ツリーの切り元のプロジェクトを相互参照で確かめ、写しの `project:` と違う作業ツリーを着手させないこと |
| REQ-MLT-14 | 常時 | ccnavi は、プロジェクトごとの提案の置き場と git ラッパの記録をワークスペースルートの下に置き、プロジェクトのリポジトリの中に書かないこと |
| REQ-MLT-15 | 常時 | ccnavi は、プロジェクト置き場が無いか空のとき、この節の前と同じ判定・置き場・記録を保つこと |
| REQ-MLT-16 | 常時 | `--lint` は、プロジェクト置き場が無視されているか、各プロジェクトのルールが読めるか、プロジェクトが `.claude/` を持たないか、作業ツリーの切り元と `project:` が合うかを言うこと |
| REQ-MLT-17 | 常時 | `--explain` は、プロジェクトの一覧と、それぞれのルールファイルが読めるか、タイプごとの件数を示すこと |
