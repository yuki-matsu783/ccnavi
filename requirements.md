...
| REQ-RSK-02 | 常時 | ccnavi は、点の配点を保護された設定ファイルから読み、無ければ組み込みの配点で、壊れていれば組み込みに落ちてそのことを示すこと。リスクレベルの名前は固定で、境目の点だけを設定で変えられること |
...
| REQ-MLT-36 | 常時 | ccnavi は、実行後の監視と控えと復元で、プロジェクトから切った親のワークツリーの中の変更のうち、中身が共通層の対応するものと同じで、かつその親の印がその相対パスと中身の指紋を名指ししているものだけを、着手が写したものとして戻さず報告しないこと。コミットに入った分はコミットされた中身で見ること |
| REQ-MLT-37 | 異常時 | もし着手で上書きした設定をまだ知らせていない親を閉じようとしたならば、ccnavi は、閉じずに、利用者が端末で上書きを見たことを残す経路を案内し、その経路をエージェントの呼び出しからは通さないこと |
...
印をその親に限るのは、相対パスと指紋がプロジェクトをまたいで同じになり、どこかの印で足りるとすると、どのワークツリーでも共通層の中身へ戻す書き込みが外れるため。
コミットに入った分をコミットされた中身で見るのは、ディスクで見ると、好きな中身でコミットしてからディスクだけ戻す形が外れるため。
REQ-MLT-37 があるのは、レビューの無い親（計画が無い、全部 `review: none`、締めた）が REQ-MLT-35 の知らせを通らずに統合先へ入るため。
...
| リスクスコアの配点表と境目の点 | 「点とそのリスクレベルが出ること」と「実績で測ること」までを要求とする。配点は設定で動く |
