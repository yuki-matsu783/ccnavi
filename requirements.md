...
自己防衛（REQ-SLF-07）が同じ綴りで両方を守るため。置き場を引数で動かせると、スクリプトが探す場所と
守る場所を、置いた人ごとに合わせ直すことになる。置き場を動かす引数を渡されたときは、黙って無視せずに
何も書かずに断る。無視すると、打った人は指した場所に置いたつもりで進む。
...
REQ-RUL-09 が「ルートの綴りを書かずに」を求めるのは、ワークスペースルートの綴りが
機械ごとに違うため。直書きしたルールは、書いた人の機械でしか効かない。

REQ-RUL-10 は REQ-RUL-08 より強い。REQ-RUL-08 は「報告すること」までで、
飛ばした結果できた穴には触れていない。ワークスペースの外を指す条件が抜けると、
守られなくなる範囲が「外の全部」に広がるので、報告だけでは足りない。
ここだけ強くするのはその範囲の広さによる例外で、ほかの壊れ方は REQ-RUL-08 のままである。
...
ccnavi が判定を持っていないため。持っていない判定を確認として返すと、判断できる相手が
居る権限モードでもすべての呼び出しが人のところで止まる。判断する相手は classifier とは限らず、
Claude Code 自身の権限の仕組みもそれにあたる。そこへ確認を上乗せしても判断する者は
増えず、同じ呼び出しで 2 度聞かれるだけになる。
...
誰も答えないまま通ることになる。REQ-PRE-08 の除外をプロジェクトの設定に開いてあるのは、
人が意図して確認を捨てたモードに別の確認層を建てても、残る選択が「設定を切るか ccnavi を
切るか」になるため。開けた場合でも REQ-PRE-04 の除外は残る。委ねた事実を記録に残すのは、
設定のどこが薄いかを数える手立てが、委ねた回の記録以外に無くなるため。
...
1 往復させるか、CLAUDE.md に書いて全体に効かせるかしかなくなる。報告するだけのモードでも届けるのは、
判定を適用するモードに切り替えて初めて読まれる文を残さないため。

REQ-PRE-07 の確認の記憶はこれに反しない。あれは人が明示的に許可した事実を根拠にしており、
呼び出しの出どころを根拠にしていない。

REQ-PRE-13 が要るのは、ルールの多くがコマンドの先頭の語に固定して書かれ、`env rm …` のように実行役のコマンドを
前に置くだけで外れるため。外れた形は確認に落ちるか、読み切れる形なら権限モードに委ねられ（REQ-PRE-10）、
モードによっては尋ねずに通る。サブエージェントの禁止には確認の段が無いので、そのまま通る。ルールごとに実行役の
コマンドを書かせると同じ穴を 1 本ずつ塞ぐことになるので、読む側で中で実行されるコマンドを取り出して当てる。
取り出すのは ccnavi が組み込みで持つ実行役のコマンドだけで、一覧はルールファイルから足せない。足せるなら消すこともでき、
守りの根拠が守られる側に置かれる（REQ-SLF-04 と同じ向き）。

REQ-PRE-16 が要るのは、ルールも守りも当てる先が語の綴りで、`cd` で入ってから書くと行き先の綴りから
ディレクトリの名前が消えるため。`cd .claude && echo x > settings.json` は hook の登録そのものへの書き込みだが、
`settings.json` の語にはその名前が残らない。移った先から見た綴りを**足す**形にするのは、書かれた綴りに継ぎ足すと、
いま当たっているもの（コマンドの頭に固定して書かれたルール、サブエージェントの禁止の `sh …ccnavi-ticket.sh start`）が
外れるため。足すだけなら、行き先を読み違えても当たるはずのものが当たらなくなることはない。

同じ理由で、行き先が読めないときに「読み切れなかった」扱い（REQ-PRE-04）へ倒さない。読み切れない呼び出しは
生の文字列に当てるので、コマンドの頭に固定して書かれた守りが当たらなくなり、`cd - && rm -f <守られた場所>` の
ように、書かれた綴りでは止まる形が止まらなくなる。読めないのは居場所であって、コマンドではない。

REQ-PRE-14 は REQ-PRE-13 の向きを限る。中で実行されるコマンドは元の形の判定を消さずに当て先を足すだけなので、
止める側に限れば、取り出し方を読み違えても当たるはずのものが当たらないだけで、元の形より緩くはならない。
許す側に当てると逆になる。`sudo -u me cat …` は元の形では確認に落ちるが、中の `cat …` に読み取りの許可が当たって通り、
止めている間でも通すスクリプトを `env sh …` で実行する形は、止めているところを抜ける。

REQ-PRE-15 が実行役のコマンドとともに示すことを求めるのは、元の形だけを見た読み手には、どこが当たったのかが
分からないため。ルールは `rm` について書かれていて、`env` については何も言っていない。中で実行されるコマンドで
当たった理由には、読み切れなかったので生の文字列に当てたという断り（REQ-HNT-04）を付けない。当てた先は読み直した
コマンドで、断りを付けると、書いた覚えのない文字列を読み手に探させることになる。
...
**ccnavi 自身の副命令が書いたぶんは、REQ-PST-01 の検証から外れる。** 承認済みチケットと
フェーズのマーカーの置き場は、多くのプロジェクトで書き込み拒否と宣言されるが、そこへ書くのは
人だけではなく `ticket start` / `finish` / `cancel` と `review request` / `confirm` / `ready` でもある。
外さないと、ccnavi が自分の手順を自分の違反として報告し、自動復元が有効なら自分で戻して手順が
進まない。**外すかどうかは内容で決める**（ADR-0075）。副命令がまだ書いていない欄が足されただけ、
範囲を宣言しないファイル、1 対 1 で対になった閉じる向きの移動——この 3 つに限り、検証の対象から
外れる。承認された作業範囲そのもの（チケットが宣言する allow / ask / deny、親、フェーズ）が
変わったものは、どの経路から書かれても今までどおり検証される。REQ-APV-05 / REQ-APV-07 と
REQ-RSK-01 が「エージェントが書けない場所」と言うときの第 1 線は実行前の判定で、この面は
引数に現れない経路に対する第 2 線になる。

**見る範囲と戻す範囲は違う。** REQ-PST-01 が見るのは拒否と確認の両方で、REQ-PST-05 が
戻すのは拒否のぶんだけ。確認と宣言した領域は「人が 1 度見る場所」であって「書くな」では
ないので、戻すと、人が確認に答えた編集をあとから無かったことにする。承認された作業範囲の
外も同じで、そこはルールが何も言っていない場所なので、戻す根拠が宣言の側に無い。
履歴に入った変更も戻さない（取り消すには履歴を書き換えることになる）。
...
位置が振り分けのスクリプトを指すときは、そのスクリプトが起動する実行ファイルの置き場も守る対象に含める。
スクリプトだけを守ると、実体を差し替えても判定が入れ替わったことに気付かない。
...
REQ-APV-13 が終了コードで返すのは、読む相手がエージェントだから。文だけで返すと、
通ったかどうかの判断がその場の読みに委ねられる。見せる枝（REQ-DIA-10）と分けてあるのは、
そちらを読む VS Code のボードが 0 以外を失敗として扱うためで、承認の対象にしない提案が
1 件あるだけで一覧を出せなくなる形にはしない。

REQ-APV-13 が「承認が落とさないものでは落とさない」のは、確かめと承認の答えが割れると、
確かめるほうが先に捨てられるため。落とさないのは範囲の超過（判定が切り詰めるだけで、承認は
止まらない）と、読めない提案（走査は絞る前の全ツリーを見るので、他のセッションの書きかけ 1 本で
落ちる）。どちらも本文では名指しする。承認待ちが 1 件も無いときだけは承認と答えが違う
（承認は「することが無い」で通り、確かめは「いいえ」）。頼むものが無い状態を、確かめの
成功にはしない。

REQ-APV-14 が「判定を変えずに」と言うのは、この文が判定ではないから。ルールの表に 1 行足すと、
承認待ちの置き場が「設定が言及している場所」になり、REQ-PRE-08 と REQ-PRE-10 の倒し方を
通らなくなる。さらに、判定は強いタイプから見て最初に当たった段で決まるので、置き場に確認や
拒否を宣言しているプロジェクトには文が届かない。承認の流れをいちばん気にしているところにだけ
届かない、という向きになる（ADR-0059）。1 度に限るのは、提案を書くたびに同じ文を積むと
コンテキストを食うため。

REQ-APV-07 の「画面から求められた承認」は VS Code の拡張を指す。拡張の子プロセスは端末を
持たないので、端末であることを証拠にできない。代わりに「人に見せたものと、これから承認する
ものが同じ」ことを証拠にする。人が見ていない提案が承認の対象に紛れ込んでいれば承認しない。
エージェントを締め出すのは、この経路でも組み込みの拒否（シェルからの起動）で行う。

3 つめの「置き場に在ること」は、端末もボードも持たない人のための経路（ADR-0058）。手元に
ccnavi を置けず、ホストの画面でファイルを動かすことしかできない運用でも承認の意思を出せる。
証拠はその置き場にエージェントが書けないことそのもので、動かせたのは書ける権限を持つ人だけ
になる。この経路は承認の画面を通らないので、承認のときにしか当たらなかった検査
（REQ-TKT-06、REQ-MLT-11）は判定の側が当てる。当たったチケットは範囲を当てる前に止まり、
どれが止まっているかは検証（REQ-DIA-06 の `blocked`）が言う。

REQ-APV-08 が要るのは、画面で承認すると承認とセッションが離れるため。端末で承認するなら、
同じ人が同じ画面で次の指示を打つので、承認したことは人が伝える。画面で承認したときに
伝えないと、人が「承認した」と打つまで作業が止まる。

### 2.9.1 実績で測るリスク — REQ-RSK

| ID | 型 | 要求 |
|---|---|---|
| REQ-RSK-01 | 事象 | 子チケットを閉じようとしたとき、ccnavi は、その子のワークツリーの基準点から HEAD までの差分を数えて等級の付いた点を出し、エージェントが書き換えられない場所に残すこと |
| REQ-RSK-02 | 常時 | ccnavi は、点の配点を保護された設定ファイルから読み、無ければ組み込みの配点で、壊れていれば組み込みに落ちてそのことを示すこと。等級の名前は固定で、閾値だけを設定で変えられること |
...
| REQ-RSK-05 | 状態 | フェーズの点（子の最大値）が高い等級以上である間、ccnavi は、種類や子の宣言がレビュー不要でも、そのフェーズを人間レビューが要る扱いにすること。実績が小さいことを理由に宣言のレビュー要を下げないこと。レビューをどこで受けるかは指さず、宣言が「見ない」だったフェーズだけをこのセッションで見る扱いに上げ、マージリクエストで見ることを勧める文を添えること |
| REQ-RSK-06 | 事象 | 子を閉じたとき・フェーズが終わったとき・診断を求められたとき・レビューを依頼するとき、ccnavi は、点と等級と加点した理由を示すこと |
...
（`vscode-extension/ccnavi-board/`）も読む。判定の答え（レビューで止まっているか、承認待ち、ルールの当たり方）を
...
親のものと同じ場合があるため。作業ディレクトリで決めると、子のワークツリーへ絶対パスで
書いた呼び出しが親のチケットで判定される。行き先で決めれば、誰が書いても
その場所のチケットで判定され、サブエージェントの起動の仕方が判定に影響しない。

REQ-TKT-09 が承認済みチケットの置き場へ動かす者を人に限るのは、そこに在ることが承認の証拠だから
（ADR-0055）。エージェントが動かす向きは提案の置き場（レビュー待ち）へだけで、範囲が消える向きなので
承認は要らない。REQ-TKT-18 の続きの子を人が起こすのは、人がその範囲で作業してよいと決めたことが
承認そのものだから。エージェントに起こさせると、もう一度承認が要る。
REQ-TKT-09 が状態を置き場で表すのは、`ls` で見え、コミットに残り、閉じたつもりの食い違いが
...
REQ-TKT-15 の除外は、止めている間でも親が子の成果をまとめ、片付け、レビューを頼み確かめる
必要があるため。git のラッパースクリプトは push を通さないので、止めている間に通しても外へ出るものは無い。

REQ-TKT-16 が合流を前提に入れるのは、合流していない成果はマージリクエストに現れず、
人が見ていないものを「見た」ことになるため。判定ではなく依頼の前提なので、
「ブランチの親子を見ない」（4 章）と両立する。

REQ-TKT-45 が厳しい側を採るのは、緩い側を採ると、延期を 1 つ挟むだけで宣言した
マージリクエストのレビューがこのセッションのレビューに落ちるため。子の宣言と実績のリスクを
要否だけとして扱うのは、どちらも「人が見る」としか言っておらず、どこで見るかを言う材料を
持たないため（REQ-RSK-05）。場所を言えるのは、人が持つ設定（種類）と人が承認した計画だけ。

REQ-TKT-46 が見る場所を確かめるのは、確かめないとこの経路がマージリクエストのレビューの
抜け道になるため。ホストへ出ない経路は写しを突き合わせられないので、宣言が
「このセッションで見る」であることが、通してよい根拠のすべてになる。止めるのは緩める側だけで、
逆向き（このセッションで見る宣言のフェーズを、依頼してマージリクエストで見る）は妨げない。
実績のリスクが高いフェーズで勧めるのがこの向きなので、塞ぐと勧めた先が無くなる。

REQ-TKT-18 が人の直接起動に限るのは、チャットでの「進めてよい」を hook が見られないため。
エージェントが「人が許した」と主張して進める形は、止められた側が自分で先へ進む形になる。
...
| REQ-MLT-21 | 異常時 | もし同じ識別子で中身の違うフェーズの種類が複数の層にあるか、フェーズの種類の表示名が層をまたいで重なるか、合成後の配点の閾値が `medium <= high <= critical` を守らないならば、ccnavi は、その層のフェーズの種類か配点を空として扱い、そのことを診断で error として示し、配点については子の記録と閉じるときの出力でも示すこと |
...
| REQ-MLT-34 | 事象 | プロジェクト向けの親チケットに着手するとき、ccnavi は、共通層にある設定ファイルごとに親のワークツリーのプロジェクトの層の同じ名前のファイルと中身を比べ、違えば共通層の中身で上書きし、上書きで消える識別子と写したファイルを着手の出力と親の印に示すこと。共通層に無いファイルは消さないこと |
| REQ-MLT-35 | 事象 | 親の印が上書きを記録していてまだ知らせていないとき、ccnavi は、その親の最初のレビュー（マージリクエストへの依頼か、このセッションで受けるレビュー）でプロジェクトの設定が変わったことを示し、知らせたことを印に残すこと |
| REQ-MLT-36 | 常時 | ccnavi は、実行後の監視と控えと復元で、プロジェクトの層の変更のうち、中身が共通層の同じ種類のファイルと同じで、かつ親の印がその相対パスと中身の指紋を名指ししているものだけを、着手が写したものとして戻さず報告しないこと |

REQ-MLT-05 がパスを持たないツールを和にするのは、その呼び出しがどのプロジェクトのものかを決められないため。Bash で当てる仕掛け
（`cwd`、`cd` の追跡、引数の語の走査）は「どのルールファイルを引くか」にしか効かず、副作用は結局実行後の監視が拾う。WebFetch・Skill・
Agent は当てる材料を持たない。当てずに和を取れば、`deny` と `ask` は増える側に倒れ、緩むのは `allow` の共有だけになる。
...
REQ-MLT-21 がその層を空にするのは、衝突した片方を黙って採ると、どちらの `review:` が効いているかを人が読めないため。
空にすれば、その層の種類を指す計画は承認で止まり、止まる側に倒れる。配点の項目の同じ識別子は REQ-MLT-20 で両方を数える。
空にすると点が下がり、緩む側に倒れるため（ADR-0084）。
...
REQ-MLT-14 が提案と承認済みチケットをそのツリーの git に置くのは、承認を機械のあいだで運ぶため。
A が承認して B の機械で作業し A がレビューする流れでは、承認の記録が承認した人の機械にしか無いと、
B の側では「承認されなかったこと」になる。プロジェクトが public のリポジトリならワークスペースの運用の
痕跡が残るが、その痕跡こそが共有したいもの。痕跡は feature ブランチに乗り、マージリクエストの squash merge で
1 コミットに畳まれる。git のラッパースクリプトの記録は判断ではなくその機械で何を打ったかでしかないので、
ワークスペース側に置く。

提案は ccnavi ディレクトリ（`.ccnavi/`）の外に置く。ccnavi ディレクトリの下は組み込みが丸ごと止めていて、提案は親が書くものだから。
承認済みチケットとフェーズのマーカーは ccnavi ディレクトリの下で、その 1 本がそのまま守りになる。

REQ-MLT-30 が「親のツリーに無ければ読まない」のは、子のワークツリーには切った時点の承認済みチケットが
checkout されているから。親のツリーで閉じたあとも子の側には開いた版が残るので、そちらに落とすと
閉じたチケットの範囲が復活する。落ち先を元ツリーに置くのは、ワークツリーは畳めば消えるが元ツリーは
消えず、そこが合流先でもあるため。元ツリーより先の置き場に在る版があるときに採らないのは、そのとき
「他の版は合流の結果」という根拠が成り立たず、閉じた子をもう一度閉じることになるから（ADR-0073）。

REQ-MLT-34 が共通層で上書きするのは、共通層は共通の設定を各プロジェクトへ配るための定義で、正本は各プロジェクトの層だから（ADR-0084）。
共通層はワークスペースの git にあり、プロジェクトだけを clone した人からは見えない。共通層に無いファイルを消さないのは、フェーズの種類のように
ツリーごとに持つものを消すと、計画が承認で止まるため。REQ-MLT-35 が最初のレビューで知らせるのは、上書きを人の目に通すため。
REQ-MLT-36 が内容と印の両方を見るのは、中身だけならシェルで写した分も外れて知らせが残らず、印だけなら印を置いてから中身を書き換える形が外れるため。

REQ-MLT-33 が既定でない位置を書き換えないのは、人が決めた綴りの先で何が使われているかを導入スクリプトが
決められないため。
...
- **敵対的な回避への防御**。意図的な文字列の分割、変数展開、alias 経由の実行は対象にしない。中で実行されるコマンドを取り出す（REQ-PRE-13）のは ccnavi が組み込みで持つ実行役のコマンドだけで、一覧に無いもの（`ssh host <cmd>`・`watch`・`python -c` など）の中も対象にしない。完全なシェルパーサを書いても守れる範囲は広がらない。破られてはいけない線は OS の権限分離と sandbox が引く
...
| 8 | ガード自身の設定ファイルと実行ファイルを壊す入力を通す | ルールの `deny` を空にする書き換え、hook の登録を消す書き換え、設定ファイルの削除、置いていない設定ファイル、シェルからの書き込み、実行ファイルの差し替え、セッション開始を通る場合と通らない場合、この面の有効と無効 | REQ-SLF-01〜09 |
| 9 | 親 1 本と子 2 本をフェーズ 2 つで通す | 子ごとのワークツリー、親の作業ディレクトリから子のツリーへの絶対パスの書き込み、親を超える範囲の子、孫を持つ子、ワークツリーの無い子、識別子の形が違う子、作業中・完了の置き場への直接の作成と移動、サブエージェントからの状態の移動（実行役のコマンドを通した形を含む）、基準点より後にコミットされた範囲外の変更、人間レビュー要のフェーズと不要のフェーズ、全部閉じた後の起動とシェル実行、合流していない子ブランチを持つ依頼、依頼の前と後のスレッド、変更要求のあるレビュー、未解決スレッドのあるマージリクエストと無いもの、終わったフェーズへの子の追加、承認済みチケットの置き場の差し替え、ルールが許可または確認にした場所への範囲外の書き込み、他のチケットの提案の書き込み、このセッションで見るフェーズの通しと、その経路でマージリクエストのフェーズを通そうとする形 | REQ-TKT-01〜25, REQ-TKT-43〜47, REQ-APV-01, REQ-APV-02, REQ-APV-05, REQ-PRE-05, REQ-PRE-13 |
| 10 | ワークスペース 1 つとプロジェクト 2 つで、プロジェクトをまたぐ入力を通す | プロジェクト A と B への書き込み、プロジェクトの外と 2 段深い場所への書き込み、プロジェクトの中に `cwd` を置いた Bash と親に置いた Bash と `cd` を挟んだ Bash、片方のプロジェクトだけが持つ `allow` `ask` `deny`、共通層と自身の層とプロジェクトの層に置いたルール、同じ `id` を持つ 2 つの層（全欄一致と中身違い）、読めない層、層の設定ファイルと ccnavi ディレクトリの下のスクリプトへの Write とシェル書き込み（ccnavi ディレクトリごと消す形と綴り違いを含む）、プロジェクトから切ったワークツリー側の設定の書き換え、層をまたいで衝突するフェーズの種類と配点、閾値を書いた層と書かない層、層の外を指す `script:`、コロンを含む `id`、予約名のプロジェクト、一部が落ちる承認、`project:` の違うワークツリーへの着手、置き場に無い `project:` の承認、ワークスペースルートとワークツリーで起きた実行後の変更、プロジェクト置き場が無い入力、実行ファイルの位置の設定が無い・新しい置き場・既定でない位置の配布先 | REQ-MLT-01〜33, REQ-HKS-08 |
| 11 | フェーズの種類と計画を持つ親を、計画の承認から締めまで通す | 種類の定義（作業と対応、レビューの要否、範囲の上限、成果物、並行してよい組、一緒に要る組）、識別子か表示名が重なる定義、全体計画に対応の種類を置いた提案、順序を飛ばした子の承認、`overlap` の組、レビューの延期と最後の項での延期、成果物の無いまま閉じる、改版で計画を変える、空のフィードバック計画、フィードバック対応で指摘が残った場合、下書きを外す条件、人が端末から締める入力、親と種類の上限を超える子の承認とそのワークツリーでの書き込み（有効と予行）、壊れたフェーズの種類の定義、ボードで見せたあとに書き換えた提案と照合の値が無い承認、ボードの承認のあとの端末でのコミットと push、エージェントが打つ承認の経路の sh（綴りの大文字小文字違いを含む）、承認を頼む前の確認（通る提案・落ちる提案・承認待ちが無い・承認待ちに無い識別子・範囲の超過だけの子・読めない提案）と、提案を書いた回の案内（同じ文脈で 2 度目、提案の置き場の外） | REQ-TKT-26〜39, REQ-TKT-41, REQ-TKT-42, REQ-APV-02, REQ-APV-09〜14 |
| 12 | 子を閉じてリスクを数える入力を通す | 差分の量が閾値の前後にある子、保護された場所のスクリプトと読めない出力を返すスクリプト、定性の問いが揃っていない子、点が高い等級に達したフェーズ、壊れた配点、配点のファイルが無い場合 | REQ-RSK-01〜06, REQ-TKT-40, REQ-DIA-06〜09, REQ-PRE-11, REQ-PRE-12 |
...
| リスクスコアの配点表と閾値 | 「等級の付いた数値が出ること」と「実績で測ること」までを要求とする。配点は設定で動く |
