...
| REQ-MLT-33 | 事象 | 前の既定の置き場を指す実行ファイルの位置の設定を見つけたとき、導入スクリプトは、新しい置き場で起動できる場合に限りそれを書き換え、既定でない位置は書き換えずに名指しすること。前の既定の置き場に残った振り分けのスクリプトは、その回に位置の設定を前の既定から書き換えたか設定が既に新しい置き場を指しており、かつ新しい置き場で起動でき、導入スクリプトを打ったセッションの位置の設定がそこを指さない場合に限り消すこと |
...
REQ-MLT-33 が書き換えを前の既定の綴りに限るのは、人が決めた綴りの先で何が使われているかを導入スクリプトが
決められないため。新しい置き場で起動できないうちに書き換えると、hook は何も起動しなくなる。前のスクリプトを消す条件が
多いのは、消すと戻せず、消した瞬間からそれを起動しているセッションの hook が動かなくなり、守りが黙って消えるため。
打ったセッション自身の設定はセッションを開き直すまで変わらないので、そこを指していれば消さずに開き直しを促す。
「既に新しい置き場を指している」回も消すのは、そうして消さなかった回の次に、開き直して打ち直した回で消すため。
位置の設定が無かった回と既定でない位置の回に消さないのは、そのスクリプトを何が起動しているかを設定から読めないため。
