...
| REQ-PRE-14 | 常時 | ccnavi は、中で実行されるコマンドに、許可のルールと足止めの中で通す形を当てないこと |
...
足止めの中で通すスクリプトを `env sh …` で実行する形は足止めを抜ける。
...
| REQ-TKT-39 | 常時 | ccnavi は、チケット制御（提案の承認・承認済みチケットの範囲・フェーズの足止め・サブエージェントの制限・状態とレビューの操作）を使うかを、ルールによる判定とは別の 1 つの設定（`enable` / `disable`、既定 `enable`）で切り替えられること。それ以外の値は `enable` として動き、設定の検証が error として示すこと。承認済みチケットの置き場を空にすることでは切り替わらないこと |
...
REQ-TKT-15 の除外は、足止めの中でも親が子の成果をまとめ、片付け、レビューを頼み確かめる
必要があるため。git のラッパースクリプトは push を通さないので、足止めの中で通しても外へ出るものは無い。
...
エージェントが「人が許した」と主張して進める形は、止められた側が足止めを解く形になる。
