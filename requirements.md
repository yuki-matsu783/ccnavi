...
| REQ-APV-13 | 事象 | 承認できる状態かを確かめるよう求められたとき、ccnavi は、承認済みチケットを置かずに、承認の対象に入るものと入らないもの、入らない理由を示し、指定されたもの（指定が無ければ承認待ち全部）がそのまま承認の対象に入るかどうかを終了コードで返すこと。判定は承認が使うのと同じ関数で求めること |
...
REQ-APV-14 が止めないのは、承認待ちの置き場への作成と編集がエージェントの仕事だから
（REQ-TKT-09）。1 度に限るのは、提案を書くたびに同じ文を積むとコンテキストを食うため。
