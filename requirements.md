...
素通りは強制の仕組み自体を無意味にし、しかも気づけない。判断がつかないときは拒否側へ倒す。
...
REQ-CMN-08 は、遅延が許可に化けることを防ぐ。期限に達すると呼び出し側が応答ごと捨てて素通りになるので、
その手前で自分から拒否側に着地する。外側で打ち切られた場合は 4 章。

REQ-CMN-01 が実行前に限られるのは、同期の経路にいて遅れが素通りに化けるのが実行前だけだから。
...
REQ-PRE-06 は REQ-PRE-04 の例外にあたる。拒否側へ倒すと、壊れた設定を直す操作まで止まって回復できなくなる。

REQ-PRE-10 は、ccnavi が判定を持たない呼び出しに確認を上乗せしても、同じ呼び出しで 2 度聞かれるだけだから。
除外の 2 つはどちらも許可へ倒さないためのもので、REQ-PRE-04 は確定できなかった事実が委ねた先に伝わらず、
...
行き先が読めないときも REQ-PRE-04 へ倒さない。倒すと生の文字列に当てることになり、
...
この面は実行後に走り、操作そのものを取り消せないので、REQ-PST-02 は差し戻しの文を返す。
...
「エージェントが書けない場所」は、実行前の判定が第 1 線、この面が引数に現れない経路に対する第 2 線になる。
...
REQ-PST-01 の保護領域はプロジェクト設定の宣言から導くので、宣言そのものが書かれたファイルはこの面では守れない。
そこは REQ-SLF が受け持つ。
...
この面は hook から呼ばれず、人が端末から叩く。REQ-DIA-06、REQ-DIA-02 の JSON 版
...
`deny` と `ask` は増える側に倒れる。

REQ-MLT-06 は、共通層が読めているのに組み込みへ落とすと共通層の `deny` が消えるため。
共通層のルール自身が読めないときは REQ-PRE-06 のまま組み込みの既定に落ち、層は足さない。

REQ-MLT-18 は、プロジェクトの層を書けたエージェントが共通層の `deny` を消せないようにする。

REQ-MLT-21 は、衝突した片方を黙って採らず、止まる側に倒す。配点の項目の同じ識別子は、空にすると点が下がるので
...
| 6 | ルールを差し替えて同じ入力を通す | 既定の場所と引数で指定した場所、やさしい記法と正規表現の両方、文面を欠いたルール、両方を書いたルール、解釈できないルール、版番号の異なる書式 | REQ-RUL-01〜10 |
