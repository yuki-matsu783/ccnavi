...
| REQ-MLT-05 | ユビキタス | ccnavi は、Bash の判定に、ワークスペースルートのルールと全プロジェクトのルールを区画ごとに合わせたものを用い、呼び出し元の作業ディレクトリによらず同じ判定を返すこと |
...
| REQ-MLT-17 | ユビキタス | `--explain` は、プロジェクトの一覧と、それぞれのルールファイルが読めるか、区画ごとの件数を示すこと |
