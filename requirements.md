...
| REQ-TKT-15 | 状態 | 人間レビュー要のフェーズが終わり、そのフェーズにレビュー済みの印が無い間、ccnavi は、サブエージェントの起動と、チケットの状態を動かす操作・レビューの依頼と確認・git のラッパ以外のシェル実行を拒否し、理由に利用者への問い合わせを入れること |
...
必要があるため。git のラッパは push を通さないので、ゲートの中で通しても外へ出るものは無い。
