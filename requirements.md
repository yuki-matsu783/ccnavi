...
| REQ-TKT-07 | イベント駆動 | チケットの承認を求められたとき、ccnavi は、主となる作業ツリーとすべての作業ツリーの提案の置き場を走査し、未承認のチケットを一度に示すこと |
| REQ-TKT-08 | イベント駆動 | 承認済みのチケットが閉じられたとき、ccnavi は、人の承認を経ずにそのチケットの許可を効かせなくすること。再び効かせるのは人の操作によること |
| REQ-TKT-09 | ユビキタス | ccnavi は、チケットを閉じる操作をサブエージェントに許さないこと |
| REQ-TKT-10 | イベント駆動 | 同じ親の同じフェーズに属する子チケットがすべて閉じられたとき、ccnavi は、エージェントに、ターンを終えて利用者にレビューを求めるよう伝えること |
| REQ-TKT-11 | 状態駆動 | フェーズが終わり、そのフェーズにレビュー済みの印が無い間、ccnavi は、サブエージェントの起動と、チケットを閉じる操作・レビュー確認・git の包み以外のシェル実行を拒否し、理由に利用者への問い合わせを入れること |
| REQ-TKT-12 | イベント駆動 | レビュー確認を求められたとき、ccnavi は、親チケットのブランチに対応するマージリクエストを引き、未解決のスレッドが無ければレビュー済みの印を置き、あれば印を置かずに未解決の一覧を示すこと |
| REQ-TKT-13 | イベント駆動 | 未解決のスレッドを残したまま進める判断を求められたとき、ccnavi は、人が直接起動したときに限りそれを受け付け、未解決の一覧を示したうえで印を置くこと |
| REQ-TKT-14 | イベント駆動 | 終わったフェーズに子チケットが新たに承認されたとき、ccnavi は、そのフェーズのレビュー済みの印を消すこと |
| REQ-TKT-15 | イベント駆動 | サブエージェントが開始されたとき、ccnavi は、承認済みで閉じていない子チケットの識別子・作業ツリー・範囲をサブエージェントに渡すこと |
| REQ-TKT-16 | イベント駆動 | サブエージェントが終了しようとしたとき、その作業ツリーに範囲外の変更が残っていれば、ccnavi は、1 回に限りそれを示して終了を差し戻すこと |
| REQ-TKT-17 | ユビキタス | ccnavi は、提案の置き場と承認済みの写しの置き場を設定で差し替えられ、写しの置き場が保護されていないときはそれを診断で示すこと |
| REQ-TKT-18 | イベント駆動 | 設定の検証を求められたとき、ccnavi は、チケットの無い作業ツリー・識別子と名前の一致しない作業ツリー・読まれない場所に置かれた写し・無い認証情報を示すこと |
...
REQ-TKT-08 が閉じる操作に承認を求めないのは、閉じるのが範囲の消える向きで、
エージェントが書いても危険が増えないため。逆に再開は範囲が戻る向きなので人の手に置く。

REQ-TKT-11 の除外は、門の中でも親が子の成果をまとめ、片付け、レビューを確かめる
必要があるため。git の包みは push を通さないので、門の中で通しても外へ出るものは無い。

REQ-TKT-13 が人の直接起動に限るのは、チャットでの「進めてよい」を hook が見られないため。
...
- **秘密情報の保管・暗号化・受け渡し**
- **ブランチの親子とマージ先の検証**。子の作業ツリーがどのブランチから切られ、どこへ戻るかは見ない。ccnavi が確かめるのは作業ツリーの名前とチケット識別子の一致だけで、切り方と戻し方はメインエージェントの手順が持つ
...
| 9 | 親 1 本と子 2 本をフェーズ 1 つで通す | 子ごとの作業ツリー、親の作業ディレクトリから子のツリーへの絶対パスの書き込み、親を超える範囲の子、孫を持つ子、作業ツリーの無い子、サブエージェントからの閉じる操作、全部閉じた後の起動とシェル実行、未解決スレッドのあるマージリクエストと無いもの、終わったフェーズへの子の追加、写しの置き場の差し替え | REQ-TKT-01〜18, REQ-APV-01, REQ-APV-02, REQ-APV-05, REQ-PRE-05 |
