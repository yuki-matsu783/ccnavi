...
| REQ-TKT-16 | 事象 | レビューの依頼を求められたとき、ccnavi は、そのフェーズにレビューが要ること・フェーズが終わっていること・そのフェーズの子ブランチがすべて親ブランチに取り込まれていること・未コミットの変更が無いこと・push 済みであること・そのフェーズのレビューが延期されていないこと・そのフェーズがレビュー済みでないこと・まだ依頼していないこと（依頼の後に親ブランチの HEAD が動いていれば依頼し直せる）を確かめ、1 つでも欠ければ全件を示して依頼せず、満たせば対応するマージリクエストを（無ければ下書きで作って）依頼を投稿し、依頼の時点を記録すること |
...
| REQ-TKT-44 | 常時 | ccnavi は、提案の置き場と承認済みチケットの置き場の下への書き込みを、実行前の判定・実行後の監視・サブエージェント終了時の検査のどれでも、チケットの範囲の外として扱わないこと |
