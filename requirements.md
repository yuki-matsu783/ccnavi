...
| ID | 型 | 要求 |
|---|---|---|
| REQ-MLT-01 | ユビキタス | ccnavi は、プロジェクト根の直下の対象置き場（既定 `projects/`）にある git リポジトリだけを対象として数え、それより深い場所と置き場の外のリポジトリを対象としないこと |
| REQ-MLT-02 | ユビキタス | ccnavi は、書き込みの対象が行き着く先の属するツリー（プロジェクト根、対象、作業ツリー）を最長一致で決め、行き先がプロジェクト根の外にあるとき判定を持たないこと |
| REQ-MLT-03 | ユビキタス | ccnavi は、書き込み系ツールの判定に、行き先のツリーが属する対象のルールファイル（対象の根からの相対、既定 `config/rules.yml`）を用い、対象に属さないツリーにはプロジェクト根のルールファイルを用いること |
| REQ-MLT-04 | ユビキタス | ccnavi は、対象のルールファイルとして対象の main にあるものだけを読み、作業ツリーの中にあるものを読まないこと |
| REQ-MLT-05 | ユビキタス | ccnavi は、Bash の判定に、プロジェクト根のルールと全対象のルールを区画ごとに合わせたものを用い、呼び出し元の作業ディレクトリによらず同じ判定を返すこと |
...
| REQ-MLT-10 | ユビキタス | ccnavi は、実行後の監視を、プロジェクト根と、そのターンで書き込みのあった対象と作業ツリーのそれぞれの git で行い、差し戻しもそのツリーの git で行い、報告にツリーの名前を添えること |
...
| REQ-MLT-14 | ユビキタス | ccnavi は、対象ごとの提案の置き場と git ラッパの記録をプロジェクト根の下に置き、対象のリポジトリの中に書かないこと |
