...
道具（hook の登録、実行ファイル、保護済みスクリプト、スキル）を持つ private のリポジトリを資産と呼び、
その下の置き場に clone した別々のリポジトリを対象と呼ぶ。Claude Code は資産を開く。ルールは対象ごとにあり、
対象の git で育てる。設計は §25。

| ID | 型 | 要求 |
|---|---|---|
| REQ-MLT-01 | ユビキタス | ccnavi は、プロジェクトルートの直下の対象置き場（既定 `projects/`）にある git リポジトリだけを対象として数え、それより深い場所と置き場の外のリポジトリを対象としないこと |
| REQ-MLT-02 | ユビキタス | ccnavi は、書き込みの対象が行き着く先の属するツリー（プロジェクトルート、対象、作業ツリー）を最長一致で決め、行き先がプロジェクトルートの外にあるとき判定を持たないこと |
| REQ-MLT-03 | ユビキタス | ccnavi は、書き込み系ツールの判定に、行き先のツリーが属する対象のルールファイル（対象の根からの相対、既定 `config/rules.yml`）を用い、対象に属さないツリーにはプロジェクトルートのルールファイルを用いること |
| REQ-MLT-04 | ユビキタス | ccnavi は、対象のルールファイルとして対象の main にあるものだけを読み、作業ツリーの中にあるものを読まないこと |
| REQ-MLT-05 | ユビキタス | ccnavi は、Bash の判定に、プロジェクトルートのルールと全対象のルールを区画ごとに合わせたものを用い、呼び出し元の作業ディレクトリによらず同じ判定を返すこと |
| REQ-MLT-06 | 状態駆動 | 対象のルールファイルが読めない間、ccnavi は、その対象への書き込みを組み込みの既定で判定し、Bash の合成からその対象を外し、外したことを記録と診断で示すこと |
| REQ-MLT-07 | ユビキタス | ccnavi は、記録と文面で、当たったルールの識別子に対象の名前を添えること |
| REQ-MLT-08 | ユビキタス | ccnavi は、対象のルールファイルを設定ファイルの自己防衛の対象（書き込みの拒否、シェル書き込みの拒否、控えと復元）に含めること |
| REQ-MLT-09 | ユビキタス | ccnavi は、対象の列挙とルールの読み込みのために、実行前の判定の間に外部プロセスを起こさないこと |
| REQ-MLT-10 | ユビキタス | ccnavi は、実行後の監視を、プロジェクトルートと、そのターンで書き込みのあった対象と作業ツリーのそれぞれの git で行い、差し戻しもそのツリーの git で行い、報告にツリーの名前を添えること |
| REQ-MLT-11 | ユビキタス | ccnavi は、チケットの `repo:` を人の承認で確定し、置き場に無い名前を承認しないこと |
| REQ-MLT-12 | ユビキタス | ccnavi は、書き込みの行き先のツリーが属する対象と、そのツリーに結び付く写しの `repo:` が違うとき止めること。エージェントが申告した対象を判定に使わないこと |
| REQ-MLT-13 | ユビキタス | ccnavi は、作業ツリーの切り元の対象を相互参照で確かめ、写しの `repo:` と違う作業ツリーを着手させないこと |
| REQ-MLT-14 | ユビキタス | ccnavi は、対象ごとの提案の置き場と git ラッパの記録をプロジェクトルートの下に置き、対象のリポジトリの中に書かないこと |
| REQ-MLT-15 | ユビキタス | ccnavi は、対象置き場が無いか空のとき、この節の前と同じ判定・置き場・記録を保つこと |
| REQ-MLT-16 | ユビキタス | `--lint` は、対象置き場が無視されているか、各対象のルールが読めるか、対象が `.claude/` を持たないか、作業ツリーの切り元と `repo:` が合うかを言うこと |
| REQ-MLT-17 | ユビキタス | `--explain` は、行き先のツリーとその対象、読んだルールファイル、Bash なら合成に入った対象の一覧を示すこと |

REQ-MLT-05 が Bash を和にするのは、Bash の呼び出しがどの対象のものかを当てる仕掛け（`cwd`、`cd` の追跡、引数の語の走査）が
「どのルールファイルを引くか」にしか効かず、副作用は結局実行後の監視が拾うため。当てずに和を取れば、
`deny` と `ask` は増える側に倒れ、緩むのは `allow` の共有だけになる。

REQ-MLT-12 が申告を見ないのは、REQ-TKT-01 と同じ理由。エージェントが「対象は B」と言った値でルールを選ぶと、
A に書きながら B を名乗る形が通る。行き先で決めれば、思い違いはその場で止まる。

REQ-MLT-14 が対象の中に書かないのは、対象が public のリポジトリであり、提案と記録は資産の運用の痕跡だから。
対象に入るのはルールファイルだけにする。
...
| 10 | 資産 1 つと対象 2 つで、対象をまたぐ入力を通す | 対象 A と B への書き込み、対象の外と 2 段深い場所への書き込み、対象の中に `cwd` を置いた Bash と親に置いた Bash と `cd` を挟んだ Bash、片方の対象だけが持つ `allow` `ask` `deny`、同じ `id` を持つ 2 つの対象、読めない対象のルール、対象のルールファイルへの Write とシェル書き込み、`repo:` の違う作業ツリーへの着手、置き場に無い `repo:` の承認、対象の main と作業ツリーで起きた実行後の変更、対象置き場が無い入力 | REQ-MLT-01〜17 |
