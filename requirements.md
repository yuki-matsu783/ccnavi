...
| REQ-MLT-14 | ユビキタス | ccnavi は、提案と承認済みの写しとフェーズの印を、そのツリーの `.ccnavi/` に置き、そのツリーの git が追跡できる場所に書くこと。git ラッパの記録はワークスペースルートの下に置き、プロジェクトのリポジトリの中に書かないこと |
| REQ-MLT-18 | ユビキタス | ccnavi は、承認のとき写しを親チケットのツリーに置き、同じ識別子の写しが複数のツリーにあるときは親のツリーの版だけを読み、そこに無ければ読まないこと |
...
REQ-MLT-14 が提案と写しをプロジェクトの中に置くのは、承認を機械のあいだで運ぶため。A が承認して B の機械で
作業し A がレビューする流れでは、承認の記録が承認した人の機械にしか無いと、B の側では「承認されなかったこと」に
なる。以前はプロジェクトが public のリポジトリであることを理由にワークスペース側へ寄せていたが、
その痕跡こそが共有したいものなので判断を裏返した。痕跡は feature ブランチに乗り、マージリクエストの
squash merge で 1 コミットに畳まれる。git ラッパの記録は判断ではなくその機械で何を打ったかでしかないので、
今までどおりワークスペース側に残す。

REQ-MLT-18 が「親のツリーに無ければ読まない」のは、子の作業ツリーには切った時点の写しが checkout されて
いるから。親のツリーで閉じたあとも子の側には開いた写しが残るので、そちらに落とすと閉じたチケットの範囲が
復活する。
