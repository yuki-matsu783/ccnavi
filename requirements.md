...
人だけではなく `ticket start` / `done` / `cancel` と `review request` / `check` / `ready` でもある。
...
| REQ-TKT-18 | 事象 | 未解決のスレッドの扱いを求められたとき、ccnavi は、人が直接起動したときに限りそれを受け付け、未解決の一覧を示したうえで、受け入れて進む（マーカーを置き、レビュー待ちのチケットを閉じる）か、続きの子チケットを起こす（レビュー待ちのチケットを閉じ、同じフェーズの番号で、見た子の範囲の和と指摘の一覧を持つ子を承認済みチケットの置き場に直に置き、そのフェーズのマーカーを消す）かを選ばせること。続きの子は人が選んだことで承認されたものとし、別の承認を求めないこと |
...
| REQ-TKT-28 | 常時 | 計画の項でレビューを延期できること。延期したフェーズの終わりでは進行を止めず、延期したフェーズを待つ後ろのフェーズのうち、延期していない最初のものの依頼に延期した分を含めること。計画の最後の項とレビュー不要の種類は延期できないこと |
...
| REQ-TKT-34 | 事象 | フィードバック対応フェーズの最後のレビューで指摘が残ったとき、ccnavi は、同じフェーズに子を足してやり直す道と、残りを切り出す道の 2 つを示すこと。切り出す先は、マージリクエストで見た親なら別の課題、どのフェーズもマージリクエストに出していない親なら新しい親チケットの提案とすること。新しいフィードバック対応フェーズは足せないこと |
...
| REQ-TKT-49 | 事象 | 全体計画を承認するとき（改版を含む）、ccnavi は、各番号が待つ番号と延期の引き受け手を計算して親の承認済みチケットに写し、以後の判定はその写しで行うこと。承認のあとで種類の定義が変わっても、改版を承認するまで進行中の親の待ち方を変えないこと |
| REQ-TKT-50 | 事象 | DAG で待たせる全体計画を承認するとき、ccnavi は、依存が循環する定義、後ろの項が前の項の祖先になる並び、他のすべての項を祖先か同じ種類に持たない最後の項を拒み、承認の画面に各番号の待ちを示すこと。DAG は、合成に入る層で定義を持つもの全部が DAG を指定したときに限ること |
