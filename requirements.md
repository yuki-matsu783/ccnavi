...
| REQ-PRE-16 | 常時 | シェルのコマンドが `cd` で作業ディレクトリを移したうえで相対パスを扱う場合、ccnavi は、移った先から見た綴りにも拒否と確認のルールとサブエージェントの禁止を当て、許可のルールと止めている間でも通す形には当てないこと。行き先が綴りから決まらない `cd` のあとでコマンドが実行される場合は、その呼び出しを読み切れなかったものとして扱うこと |
...
外れるため。足すだけなら、`cd` を読み違えても当たるはずのものが当たらなくなることはない。
