...
| REQ-DIA-08 | イベント駆動 | 設定の検証を機械可読な形で求められたとき（`--lint --json`）、ccnavi は、文面の版と同じ苦情を同じ深刻度で JSON にして返し、終了コードも文面の版と同じにすること |
| REQ-DIA-09 | イベント駆動 | 診断（試験・見本・検証・実効権限の表示）で 1 つのプロジェクトのルールファイルの差し替え（`--project-rules-file <名前>=<パス>`）を与えられたとき、ccnavi は、その名前のプロジェクトのルールとして差し替え先を読むこと。hook からの判定では差し替えを捨て、守る対象（REQ-MLT-08）は本来の場所のままとすること |
