...
**REQ-PRE-08 が届かない場所が 1 つある。** チケット制御が効いている間、承認待ちの置き場への
書き込みは組み込みが言及する（REQ-APV-14）ので、「設定が言及していない呼び出し」ではなくなり、
プロジェクトの設定にかかわらずこの状態で通る。提案を書けなければチケット制御が始まらないため
（ADR-0058）。緩むのはその置き場の中だけで、同じツリーの他の場所は変わらない。
...
| REQ-APV-13 | 事象 | 承認できる状態かを確かめるよう求められたとき、ccnavi は、承認済みチケットを置かずに、承認の対象に入るものと入らないもの、入らない理由（絞りそのものが通らないときはその理由）を示し、指定されたもの（指定が無ければ承認待ち全部）がそのまま承認の対象に入るかどうかを終了コードで返すこと。判定は承認が使うのと同じ関数で求め、承認が承認の対象から落とさないものでは落とさないこと |
| REQ-APV-14 | 事象 | エージェントが承認待ちの置き場に提案を書こうとしたとき、ccnavi は、それを止めずに、承認を依頼する前に確かめる手立て（REQ-APV-13）を 1 つの文脈につき 1 度伝えること |
...
REQ-APV-14 が止めないのは、承認待ちの置き場がエージェントの書く場所だから。REQ-TKT-09 が
拒否するのは承認済みチケットの置き場とレビュー待ちの置き場への直接の作成と移動で、承認待ちの
置き場はそこに入らない。1 度に限るのは、提案を書くたびに同じ文を積むとコンテキストを食うため。
この案内を組み込みで持つ代償は REQ-PRE-08 の解説と ADR-0058 にある。
