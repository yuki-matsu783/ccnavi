...
| `.ccnavi/common/risks.yml` | 子を閉じるときに差分を数える配点。HIGH 以上でゲートが閉じる | 組み込みの配点（定量 4 項目） |
...
- **3 本のファイルを書き換えない。** 下書きは作業ツリーの `tmp/` に置き（作業ツリーが
...
`raw-git` に当たる。単発で試したいものも見本ファイルを `tmp/` に書いて
