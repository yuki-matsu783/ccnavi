...
| `(risk)` | 読めない、版が違う、levels の順、id の重複・形、points が負、当て方が 0 か 2 つ、script の置き場が外、glob が組めない | 知らない等級名 |
...
統合すればそのまま統合先の設定になる道を持つ。このリポジトリでは `guard-ccnavi-config` と
...
配点を差分に当てる道具は `ticket finish` しか無い。ここで見るのは、当てたときの数字が
