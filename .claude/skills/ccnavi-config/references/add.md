...
4. 下書きをワークツリーの `scratchpad/`（無ければセッションのスクラッチパッド。docs/claude/scratchpad.md）に書く。本物の全文を写して足す（断片だと lint に掛けられない）
...
素朴に `deny: ^{!root}` と置かない。セッションのスクラッチパッド（docs/claude/scratchpad.md）、
