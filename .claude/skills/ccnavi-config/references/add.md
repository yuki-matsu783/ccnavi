...
**`Grep` と `Glob` を、場所を守るルールに足さない。** 当たるのは探し始める場所のパスだけで、
そこから降りて読まれたファイルは判定に届かない（ADR-0050）。`.env` を守るルールに `|Grep` を
足すと、名指しの検索は止まるがルートからの検索は素通りしたままで、守ったつもりの穴になる。
`--lint` がこの形を warn で言う。ファイルの中身を守るのは `.gitignore` と
`.claude/settings.json` の `permissions.deny` の `Read(...)` の仕事。

「どこを起点に探すか」を縛るなら `Grep|Glob` だけで書く。これは正しく効く。ただし
`*/logs/*` は末尾に続きを要求するので、**ディレクトリそのものを起点にした呼び出しに当たらない**。
...
`glob` で書くなら `"*/logs"`（ディレクトリ自身）と `"*/logs/*"`（中）の 2 本が要る。
