...
- `{root}` はワークスペースルートの実パスに置き換わる。glob なら `{root}/wip/*`、regex なら
  `^{root}[\\/]`。絶対パスを直書きしない
- `{!root}` は「ワークスペースルートの**外**」（ADR-0058）。先読みが使えないので、実行ファイルが
  ルートを 1 文字ずつ分解した入れ子の式に展開する。場所を列挙して漏らしたくないときに使う。
  **`{root}` と性質が違うので、下の節を読んでから書く**
...
| 直後の量化子 | **error**。`^{!root}?` は展開結果ごと省略でき、**どの対象にも当たる式**になる |
...
**素朴に `deny: ^{!root}` と置いてはいけない。** 外は広い。一度に全部塞がる。

| 塞がるもの | なぜ困るか |
|---|---|
| セッションのスクラッチパッド | docs/claude/scratchpad.md が「ワークツリーがまだ無いときは」の逃げ道として指している |
| 他リポジトリの参照・clone | `projects/` の外に置いた参照用のツリー |
| 他ツールの出力先 | 一時ディレクトリに吐くものを指定する場面 |

先に考えること。

- **`match` を絞る。** 「外への書き込みを止める」なら `Write|Edit|NotebookEdit` にして
  `Read` は残す。読む手を取り上げると、外に吐かれたものを確かめられなくなる
- **例外が要るなら `{!root}` は向いていない。** 1 つの否定しか持てず、そこに例外を彫る手が無い。
  場所を名指しするルールのほうが適している

**ルートが 256 字以上の機械では組み立てられず、そのルールが見るツールが全部止まる**
（守りが消えるより止まるほうを選ぶため）。回復は利用者が `rules.yml` から外すことだけで、
エージェントには外せない。`--lint` が error で名指しする。

展開の中身と、なぜ先読みを使わないかは ADR-0058 と `ccnavi.md` §5.3 にある。
...
項目は 1 件につき加点条件を **1 つ**だけ書く。

| 加点条件 | 何を数えるか | 決めるときに考えること |
|---|---|---|
| `lines_over` / `files_over` / `deleted_over` | 差分の行数・ファイル数・消したファイル数が基準を**超えた**ら加点 | 基準はこのプロジェクトの普通の子の大きさで決める。`ccnavi-git.sh log --shortstat` で最近の差分を見る |
...
**基準は宣言を裏切るために在る。** 「軽い」と宣言した作業が大きくなったときにレビューへ
