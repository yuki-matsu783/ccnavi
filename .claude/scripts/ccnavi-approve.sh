#!/bin/sh
# ccnavi-approve — 提案を承認し、写しを親のブランチに乗せて push する。人が端末で打つ。
#
#   sh .claude/scripts/ccnavi-approve.sh
#
# 承認そのものは ccnavi の `--approve`。写しは親チケットの作業ツリーの
# $CCNAVI_APPROVED（既定 .ccnavi/tickets）に置かれる。そこはプロジェクトの git が
# 追跡していて、コミットして push するまで他の機械には届かない（設計 §24.5）。
# A が承認して B の機械で作業する流れは、この push で成り立つ。
#
# 実行ファイルはネットワークに出さない（設計 P11）ので、コミットと push はここでやる。
# 実行ファイルが持つのは、どこに写しを置くかと、何を承認してよいかの判定だけ。
#
...
sh .claude/scripts/ccnavi-approve.sh
  ...
  承認待ちのチケットを束で見せ、承認したら写しをコミットして push する。
...
[ -z "$common" ] && {
	printf 'ccnavi-approve: git リポジトリの中で実行してください。\n' >&2
	exit 2
}
case "$common" in
*/.git) root="${common%/.git}" ;;
...
# 承認。落ちたらそこで終わり。写しが 1 つも書かれていないので、運ぶものも無い。
"$@" || exit 1
...
printf '%s\n' "$trees" | while IFS= read -r tree; do
	...
	if [ -z "$branch" ] || [ "$branch" = "HEAD" ]; then
		printf 'ccnavi-approve: %s はブランチの上に居ない。写しは手でコミットしてください。\n' \
			"$name" >&2
		continue
	fi

	git -C "$tree" add -- "$approved"
	git -C "$tree" commit --quiet -m "ccnavi: 承認済みの写しを更新" || {
		printf 'ccnavi-approve: %s で写しをコミットできない。\n' "$name" >&2
		continue
	}
	case "$branch" in
