...
class SelfGuardTest(unittest.TestCase):
    ...
    def test_プロジェクトの層は自分のgitから戻し写しの向きも切り元で決まる(self):
        # プロジェクトは自分の git を持つ。戻す先を聞く相手はワークスペースの git では
        # なくそのプロジェクトで、写しが入るのもそのプロジェクトから切った作業ツリーの
        # ほう。ワークスペースから切った w1 の中に `projects/lib/...` の綴りは無い。
        # 切り元から切った作業ツリーの写しは test_config_union_guard.py が黒箱で見る。
        self.worktree()
    ...
    def own_copies(self, *rels):
        """作業ツリー w1 の中の写しの綴り。root からの相対で、並ぶ順のまま。

        ワークスペースから切ったツリーには、ワークスペースが追跡しているものの
        写しだけが入る。設定ファイル 2 つは必ず入り、残りは渡した層のうち root の
