...
**この境目はバージョンで動く。実際に一度塞がって、また開いた。** 2.1.246 で `< file` の
リダイレクトと `tac`・`egrep` に、2.1.257 で Bash の読み取りコマンドに適用が広がり、2.1.259 で
**deny 対象を含むディレクトリへの `grep -r` も確認を求めるように**なった。その版では上の表の
最後の行が塞がっていた。2.1.260 で一部が戻され、2.1.273 では再び通る（`deny` に
`Read(./private/**)` だけを置いて測った。名指しの `cat` と `grep` は拒否されるのに、
`grep -r <語> .` は `private/notes.txt` の中身を返す）。`cd` を挟んだ形だけは止まったままで、
`--exclude-dir` は止まる版では回避にならない。

ccnavi の側で決められることではない。だからここに書いたのは「2.1.273 でそうだった」という記録で、
**守りの前提として数えるなら自分の版で測り直す**。この 1 行が ADR-0050 の分担でいちばん
脆いところで、3 層のうち 1 層が外の都合で厚くも薄くもなることを意味する。

`deny` はどのモードでも効き（`bypassPermissions` でも外れない）、PreToolUse の hook が `allow` を
返しても覆せない。ccnavi から緩める道は無く、これは ccnavi の `allow` が Claude Code へ許可を
返さないこと（ADR-0008）と同じ向き。ccnavi は厳しくする側にしか関われない。
