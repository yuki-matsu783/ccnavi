...
動かした 4 枚。数は**それぞれを動かした時点**のもので、後から増えても直さない — 判断の根拠として残してある。
上 3 行は issue #98（main の `e927ede`）、4 行目は issue #87 の作業（main の `bdaa289`）。
...
| 0068 `cd` で移った先から見た綴りを、当てる先として足す | 0066 | 8 / 14 | #101（画面の CSS の置き場）と #104（`cd` の読み）が別々に伸びて、同じ 0066 で合流した。置き場はどちらも保護されていないので数で決めた |
...
| [0068](0068-cd-moves-the-reading.md) | `cd` で移った先から見た綴りを、当てる先として足す |
