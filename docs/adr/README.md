...
動かした 4 枚。数は**動かした時点**のもので、後から増えても直さない — 判断の根拠として残してある。最初の 3 枚は issue #98（main の `e927ede`）、4 枚目は #101 と #104 が合流して重なった 0066（main の `a5a0a61`）。
...
| 0067 `cd` で移った先から見た綴りを、当てる先として足す | 0066 | 8 / 14 | 残した 0066（画面の CSS を部品と同じ置き場に）は拡張の `src/` と `test/` から 14 か所。どちらも保護された場所からは引かれていないので、数で決めた |
