...
動かした 6 枚。数は**動かした時点**のもので、後から増えても直さない — 判断の根拠として残してある。最初の 3 枚は issue #98（main の `e927ede`）。4 枚目は #101 と #104 が別々に 0066 を採ったまま合流したもので（main の `a5a0a61`）、5 枚目はその直し（#106）と #105 が同じ 0067 を採ったもの（main の `bdaa289`）。6 枚目は 5 枚目で残した 0067 と #102 の 0067 が合流したもの（main の `5548a45`）。
