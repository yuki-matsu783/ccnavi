...
番号は 1 枚につき 1 つで、**重複させない**。足すときは最後尾の次を採る。重複させてしまったら、
**引かれている数が少ないほう**を空き番号へ動かし、下の表に旧番号を残す（古い綴りで引いた人が
辿れるように）。「後から置いたほう」ではなく数で決めるのは、後から置いた側が本文と実装から
何十か所も引かれていることがあるため。数え方は「追跡されているファイルのうち、その枚自身を
除いて `ADR-00NN` と書かれている行」。`tests/core/test_adr_numbers.py` が、番号の重複・題が
`# ADR-<番号>:` で始まること・下の一覧と実体の食い違いを見る。

動かした枚（issue #98、2026-09-20 に数えた値）。
...
| 0064 ボードの画面を React にし、拡張ホストは中身だけを渡す | 0060 | 6 / 7 | 数はほぼ同じ。残した 0060（起点のデフォルトブランチ）は保護された `.ccnavi/scripts/ccnavi-fetch.sh` からも引かれていて、動かすと人の手が要る。こちらは題に番号を書いていなかった |
| 0065 レビューはマージリクエストのほかに、このセッションでも受ける | 0051 | 6 / 14 | 残した 0051（`regex` も大文字小文字を区別しない）は `.ccnavi/common/rules.yml` と `rule-samples.yml` からも引かれている |
