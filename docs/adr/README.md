...
何十か所も引かれていることがあるため。`tests/core/test_adr_numbers.py` が、番号の重複・題が
...
| いま | 旧 | 動かした理由 |
|---|---|---|
| 0063 ワークスペースの外は、先読みではなく展開で書く | 0058 | 承認の権威（0058）は本文と実装から 30 か所引かれていて、こちらは 1 か所だった |
| 0064 ボードの画面を React にし、拡張ホストは中身だけを渡す | 0060 | 起点のデフォルトブランチ（0060）は保護されたスクリプト（`.ccnavi/scripts/ccnavi-fetch.sh`）からも引かれていて、動かすと人の手が要る。こちらは題に番号を書いていなかった |
| 0065 レビューはマージリクエストのほかに、このセッションでも受ける | 0051 | 大文字小文字の側（0051）は `.ccnavi/common/rules.yml` と `rule-samples.yml` からも引かれている |
