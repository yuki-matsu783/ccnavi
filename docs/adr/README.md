...
動かした 7 枚。数は**動かした時点**のもので、後から増えても直さない — 判断の根拠として残してある。最初の 3 枚は issue #98（main の `e927ede`）。4 枚目は #101 と #104 が別々に 0066 を採ったまま合流したもので（main の `a5a0a61`）、5 枚目はその直し（#106）と #105 が同じ 0067 を採ったもの（main の `bdaa289`）。6 枚目は 5 枚目で残した 0067 と #102 の 0067 が合流したもの（main の `5548a45`）。7 枚目は #115 と #116 が別々に 0073 を採ったまま合流したもの（main の `caa93b1`）。
...
| 0075 ボードは「動いた」を印で見せ、その印を時間では消さない | 0073 | 2 / 6 | 数で決めた。残した 0073（権威のツリーが無ければ元ツリーを採る）は `requirements.md`・`ccnavi.md`・`tests/ticket/` と ADR-0024 から引かれている。どちらも保護された場所からは引かれていないので、置き場では決まらない |
...
| [0075](0075-board-shows-what-moved.md) | ボードは「動いた」を印で見せ、その印を時間では消さない |
