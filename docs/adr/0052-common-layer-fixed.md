...
**その設計は ADR-0067 で入った（issue #65）。** 3 本は同じ門に載り、診断の外では落ちる。
...
設定の見た目がそう言わない。これは設定として嘘をついていることになる。
...
ADR-0067 のあとは、その hook の command の道も閉じた。それでも `common_shell_clause` は
...
- **共通層を既定の外へ置く手段が env から無くなる。** 複数のワークスペースで 1 本の
  ...
  フラグを書き足すか、シンボリックリンクに頼ることになる（ADR-0067 でフラグの道も
...
- ADR-0051（`regex` も大文字小文字を区別しない）。同じ場所への書き込みが経路で割れる
  形を作らない、という判断を引いている
- ADR-0067（共通層の 3 本のフラグを診断の経路に限る）。ここで残した穴を塞いだ回
