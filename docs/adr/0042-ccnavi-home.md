...
設計 §11.2 は「共通層は今の `.claude/ccnavi/` のまま」としていた。層の傘を作ったとき、既存の置き場を
...
共通層の置き場は `CCNAVI_PROJECT_HOME`（傘の名前）に付いて動かない。傘の名前は各層の綴りで、
...
ccnavi のものが 1 つの傘の下にまとまり、`.claude/` には Claude Code 自身のものだけが残る。共通層も
層の傘の組み込みルール（`*/.ccnavi/*`）で名指しのツールから守られる。前は `rules.yml` の 1 行に
...
- **エージェントが共通層の yml を直接書けなくなる。** 組み込みの傘のルールはルールファイルから外せない。
