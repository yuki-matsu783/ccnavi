...
| 子のフロー（担当のサブエージェントが読む手順書） | チケットと同じツリーの `references/<子>/flow.json`（`flow:` で指せる）。**フローはエージェントが書かない。`references/` を範囲に入れない。** 書くのは人（ボードのフロー編集画面）で、範囲に入れなければエージェントの書き込みは範囲の外で止まる（ADR-0085） |
