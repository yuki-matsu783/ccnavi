...
`decide` は今までどおり統合ターミナルへコマンドを送り、y/N は人がターミナルで押す。
...
| フェーズ行の「受け入れ」 | 依頼済みで止まったまま（レビュー待ち）のフェーズに出る。親のワークツリーに `cd` して `sh '<ワークスペースルート>/.ccnavi/scripts/ccnavi-review.sh' decide <N>` を送る（プロジェクトのワークツリーには `.ccnavi/scripts/` が無いので、ルートから綴る） |
...
| bash | decide と clone / fetch / pull を送るターミナル。承認は通らない（子プロセスで打つ）。Windows は Git Bash（`C:\Program Files\Git\bin\bash.exe`、無ければ PATH の `bash`） |
...
| 5 | 受け入れ | レビュー待ち（依頼済みで止まったまま）のフェーズの「受け入れ」を押す | ターミナルで親のワークツリーに `cd` してから `decide <N>` が走る |
...
src/
  ...
  core/
    ...
    commands.ts       ターミナルに送るコマンド行（decide / close-early）と、承認を子プロセスで打つ引数の並び
