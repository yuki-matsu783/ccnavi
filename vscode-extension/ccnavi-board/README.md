...
| 5b | レビュー済み連絡 | 同じフェーズの「レビュー済み連絡」を押す。「コピー」で貼る。もう一度押して「新しいセッションで開く」 | オーバーレイに「フェーズ N のレビュー済みを連絡」と、親のワークツリーで `check --phase N` を打つ文（MR の URL 付き）。マーカーもターミナルも動かない。貼った文でエージェントが `check` を打ち、足止めが解ける。「新しいセッションで開く」で Claude Code の新しいタブに同じ文が埋まる |
