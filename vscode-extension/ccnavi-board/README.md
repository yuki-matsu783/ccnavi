...
| 5 | 残った指摘を決める | レビュー待ち（依頼済みで止まったまま）で未解決の指摘が付いたフェーズの「決める」を押し、指摘ごとに対応方針を選んで「この方針で決める」 | 全部に選ぶまで押せない。押すと「反映している…」で閉じられず、反映できたら決めた文とコピー・新しいセッションで開くが出る。「このフェーズで直す」を選んだ分は続きの子チケットがボードに現れ、issue に回した分はマージリクエストのコメントに issue の綴りが残る |
