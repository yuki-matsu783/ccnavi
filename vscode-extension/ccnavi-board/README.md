...
| 3c | 更新できない | 返らない実行ファイル（中身が `sleep 999` の sh スクリプトなど）を設定 `ccnaviBoard.binPath` に指して「更新」を押す | 60 秒はボタンが「更新中」のまま。そのあと「ccnavi --explain --json が失敗した: 60 秒で返らないので打ち切った」のエラー画面に替わる。更新ボタンごと消えるので、やり直しはコマンドパレットの「ボードを更新」 |
...
| 4e | 承認できない | 返らない実行ファイル（3c と同じもの）を設定 `ccnaviBoard.binPath` に指して「承認待ち N 件を承認」を押す | オーバーレイが 60 秒「承認待ちの一覧を読み込んでいる…」のまま。そのあと「ccnavi --approve --preview --json を 60 秒で打ち切った。承認済みチケットは置かれていない」と出て、「閉じる」で閉じられる |
