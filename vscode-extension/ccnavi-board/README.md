...
| 読めないファイル | YAML として読めない・別名（`*名前`）がある・`nodes` が無い・ノードに `id` が無い・`id` が重なるときは、画面を開かず理由を出す。エディタで直してから「再読込」 |
...
src/
  ...
  core/
    ...
    flow-doc.ts       子のフロー（YAML）の読み書きと編集（知らない欄・種類を落とさない）、雛形、入れ子の段の数え方と注意
