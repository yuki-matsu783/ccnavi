...
| 何 | どう出るか、何が起きるか |
|---|---|
| 種類 | `phases` を 1 件 1 行（id・title・kind のラベル・review・scope）で並べる。既定は全部畳んであり、行を押すとその下に欄が開く。開いた行は id で控え、再読込のあとも開いたまま。欄は id（対応表のキー）・title・kind（work / feedback）・review（none / mr）・scope（inherit か glob の並び）・deliverables と、「ほかの種類との関係・補足」の 1 行に畳んだ overlap・requires・after・agent・when（値がある種類だけ最初から開く）。overlap・requires・after は、このファイルのほかの種類の id をチェックで選ぶ（自分の id は候補に出ない。ほかの層の種類は id を打って Enter で足す）。一覧の上にファイルの頭の `order`（全体計画の待ち方。sequential / dag）の選択がある。sequential は既定で、元から欄が無ければ書かない。scope と deliverables は `,` 区切りの 1 欄。足す（足した行は開いて出る。図を見ていたら一覧へ移る）・消す・上下に動かす・改名する。足した種類の既定は `scope: inherit`（glob を埋め忘れて「何も書けない」種類にならないように） |
