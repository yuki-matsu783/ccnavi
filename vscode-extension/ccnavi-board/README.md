...
今の束が同じ」ことを確かめてから承認済みチケットを置く。違えば何も置かず、束を読み直して出し直す。端末の壁
...
| 上部の「承認待ち N 件を承認」 | `--approve --preview --json` を打ち、承認のオーバーレイを出す。束の識別子の表、承認画面の本文、承認の対象にしない提案とその理由、読めない提案が並ぶ。絞り込み中は、見えている承認待ちの識別子を並べて渡し（`--approve --preview --json i0002 i0002-01`）、その分だけの束になる。絞り込みが無ければ承認待ち全部。親や親の改版を束から外した子は実行ファイルが落とし、何も承認しない |
| 未承認カードの「この 1 件を承認」 | そのカードの識別子だけを渡し（`--approve --preview --json i0002-03`）、1 件だけの束でオーバーレイを出す。同じ親の承認待ちが他にあっても巻き込まない。親や親の改版が承認待ちのまま子だけを押したときは、実行ファイルが断った理由をオーバーレイに出し、何も承認しない |
| オーバーレイの「この N 件を承認する」 | 見せた識別子をそのまま `--approve --yes <識別子,…> --json` に渡す。束が変わっていれば何も置かれず、読み直して「見せた束と今の束が違った」と出る。承認できたらオーバーレイが「N 件を承認した」に替わり、Claude Code に渡す文と「コピー」「新しいセッションで開く」「閉じる」が出る |
...
| 4c | 1 件だけ承認 | 同じ親の子を 2 枚承認待ちにして、片方のカードの「この 1 件を承認」を押す | オーバーレイの束はその 1 件だけ。承認すると、もう片方は承認待ちのまま残る |
| 4b | 束が変わった | オーバーレイを出したまま、別のターミナルで `wip/tickets/todo/` に提案をもう 1 枚置き、「この 1 件を承認する」 | 承認済みチケットは置かれず、オーバーレイが「見せた束と今の束が違った」の注意付きで 2 件に読み直される |
