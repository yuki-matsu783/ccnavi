...
列は提案の置き場（未着手 / 作業中 / 完了 / 取り消し）。カードの枠付きの札は人が動く必要がある状態だけ
...
- **作業中のチケットがある間は保存できない。** 提案が `doing` のチケットが 1 件でもあれば（どのワークツリーでも）、
...
| 「要対応だけ」 | 人が動く必要があるカードだけを出す。条件は、承認待ち（`pending_approval`。新規の未承認と親の改版。札の「未承認」とは違い、改版を含み、承認されずに取り消した提案は含まない）、レビュー準備中／レビュー待ち、未着手・作業中なのにワークツリーが無い、HIGH 以上のリスク、本物が決まらない写り、不備、親ならフェーズ行の要約（レビュー準備中／レビュー待ち・HIGH 以上）に出るもの。組み立て（`board.ts`）が付けた印を画面が読むだけ。プロジェクト・親の絞り込みと重ねて効き、列の件数と「承認待ち N 件を承認」もその分になる。選択は Webview の状態として覚える |
...
- 提案（`wip/**/tickets/`、全ワークツリーの同じ場所）・承認済みチケットとマーカー（`.ccnavi/tickets/`）・
...
| 3 | 自動更新 | ボードを開いたまま、別のターミナルで `sh .ccnavi/scripts/ccnavi-ticket.sh start <子>` | コマンドを打たなくてもカードが作業中へ動く |
...
| 4b | 一覧が変わった | オーバーレイを出したまま、別のターミナルで `wip/tickets/todo/` に提案をもう 1 枚置き、「この 1 件を承認する」 | 承認済みチケットは置かれず、オーバーレイが「見せた一覧と今の一覧が違った」の注意付きで 2 件に読み直される |
| 4c | やめる | オーバーレイで「やめる」か Esc | 何も置かれず閉じる |
| 4d | 1 件だけ承認 | 同じ親の子を 2 枚承認待ちにして、片方のカードの「この 1 件を承認」を押す。もう一度同じ手順でオーバーレイを出し、別のターミナルでもう片方の提案に書き足してから「この 1 件を承認する」 | オーバーレイに出るのはその 1 件だけ。承認すると、もう片方は承認待ちのまま残る。書き足しても 1 件のまま承認される（絞った 1 件の中身は変わっていないため）。押した 1 件の提案を `wip/tickets/todo/` から消してから押すと、「違った」の注意付きで承認待ち全部に戻る |
...
| 4g | 承認が途中で書けない | 子の承認済みチケットの置き場（`.ccnavi/tickets/`）に、その子と同じ名前のディレクトリ（例 `i0001-01.md`）を作ってから、親と子をまとめて承認する | 「ccnavi --approve --yes が i0001-01 で止まった: 書けない（…）。i0001 の 1 件は承認済みチケットに入っている。コミットと push は送っていない…」と出る。ボードを更新すると、親だけが承認済で出る。止まるまでに出た行（マーカーを消した、など）があれば、文面の末尾に改行区切りで続く。書けたあとの後始末（マーカーを置く）で落ちたときは「…の後始末で止まった」に変わる |
