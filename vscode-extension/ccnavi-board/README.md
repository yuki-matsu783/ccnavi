...
暖色の濃い地 `#262624`）に置き換える。意味の色（error の赤、warning の黄、成功の緑）はテーマのままで、青系の強調
（承認待ち・進行中・焦点）だけ橙に寄せる。実装は body のクラスの下でテーマ変数を上書きするだけなので、部品の CSS は
テーマ変数を読むまま変わらない（`src/core/appearance.ts`）。
