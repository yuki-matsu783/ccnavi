...
| 上部の「承認待ち N 件を承認」、未承認カードの「承認」 | `--approve --preview --json` を打ち、承認のオーバーレイを出す。一覧の識別子の表、承認画面の本文、承認の対象にしない提案とその理由、読めない提案が並ぶ。絞り込み中は、見えている承認待ちの識別子を並べて渡し（`--approve --preview --json i0002 i0002-01`）、その分だけが承認の対象になる。絞り込みが無ければ承認待ち全部。親や親の改版を対象から外した子は実行ファイルが落とし、何も承認しない |
| オーバーレイの「この N 件を承認する」 | 見せた識別子をそのまま `--approve --yes <識別子,…> --json` に渡す。一覧が変わっていれば何も置かれず、読み直して「見せた一覧と今の一覧が違った」と出る。承認できたら通知に「コピー」「新しいセッションで開く」 |
