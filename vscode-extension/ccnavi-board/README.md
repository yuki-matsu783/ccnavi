...
（終了は緑の塗り、進行中は青の輪）、右が状態。カードの内寸が 480px 未満（1920px の画面で 4 列のときも含む）では、状態を札と同じ項目の要約に絞る（レビュー準備中／レビュー待ち、HIGH 以上のリスク。順調な段階は空）。他の列を畳むかドラッグで広げて 480px 以上になると、段階の状態、マーカー、レビューの要否、リスクの点と理由まで出る全文に替わる。狭いときも全文は読み上げに渡り、行にマウスを載せるとツールチップに出る。止めている段階は名前も状態も赤になる。
...
| 自身の層 | 「ワークスペース自身」の枠に、自身の層のルール（既定 `.ccnavi/config/rules.yml`）の有無と「ルール管理」「共通層からコピー」が出る。無いのは正常なので warn の色にしない。「自身の層のフェーズの種類」の行はチケット制御が disable なら出ない |
...
| 14b | チケット制御を切る | `.claude/settings.json` の env に `"CCNAVI_TICKET_CONTROL": "disable"` を書く | サイドパネルが「プロジェクト管理」「ルール管理」「見た目」の 3 つになり、コマンドパレットから「ボードを開く」「ボードを更新」「リスク管理画面を開く」「フェーズ管理画面を開く」が消える。プロジェクト管理の各行から「チケット管理」と「フェーズ管理」が、「ワークスペース自身」の枠から「自身の層のフェーズの種類」が消える。行を消すと戻る |
...
| 29c | 自身の層のルール設定 | 「ワークスペース自身」の枠で「共通層からコピー」→「ルール管理」。glob を変えて保存せずに、`Write` とワークスペースルートの下のパスで「判定」 | `.ccnavi/config/rules.yml` が出来る。タブの題が「ccnavi ルール設定: 自身の層」。当たったルールの id が `self:...`。どのツリーの子チケットを `start` しても保存が止まる |
