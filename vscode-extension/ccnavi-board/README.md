...
（終了は緑の塗り、進行中は青の輪）、右が状態。既定の 4 列の幅では状態を人が動くべきことだけに絞る（ゲート閉、まだ済んでいないレビュー要、HIGH 以上のリスク。順調な段階は空）。他の列を畳むかドラッグで広げてカードの内寸が 480px 以上になると、段階の状態、マーカー、リスクの点と理由まで出る全文に替わる。全文は行のツールチップでも読める。ゲートが閉じた段階は名前も状態も赤になる。
