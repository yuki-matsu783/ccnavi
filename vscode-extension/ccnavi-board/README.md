...
実行ファイルの `--lint --json --flow` に一時ファイルで聞き、画面は自分で判定しない（ADR-0035）。置き場は承認済みの領域
...
| 読めないファイル | 開くときに読んだ本文を一時ファイル（OS の一時ディレクトリの `ccnavi-flow-*/flow.yml`。画面を閉じると消す）に書き、`--lint --json --flow <一時ファイル>` に掛ける。実行ファイルが `(flow)` の error を言えば（YAML として読めない・別名（`*名前`）がある・`nodes` が無い・ノードに `id` が無い・`id` が重なる など。読み手と検査は `SubagentStart` と同じ）、画面を開かずその理由を出す。エディタで直してから「再読込」。実行ファイルが通しても画面の YAML の読み手（`yaml`、YAML 1.2）で読めない・ノードの並びが取れないときは、描けないとして開かない。`--flow` を知らない古い実行ファイルでは確かめられないので開かない |
...
| 保存 | 書き出す本文を一時ファイルに書いて `--lint --json --flow` に掛け、`(flow)` の error があれば書かずに理由を出す（ほかの設定の苦情では止めない）。次に押した時点で `--explain --json` を打ち直し、`locked` が偽で、置き場が読んだときと同じで、読み込んでから外で変わっていない（無かったファイルはまだ無い）、ツリーのルートからファイルまでの途中にリンクが無いときだけ書く。どれかが崩れれば書かずに理由を出す。書き込みは同じディレクトリの一時ファイルを `rename` で入れ替える（リンクを辿らず、途中で落ちても半端なファイルを残さない。一時ファイルは落ちても消し、消せずに残った `.*.tmp` は `ccnavi-push-approved.sh` が運ばない）。256KB を超えるフローは書かない。書く形は 2 字下げのブロック形式の YAML（複数行の文は `|`、別名は使わず、実行ファイルの YAML 1.1 が文字以外に読む綴り `yes` `0755` などは引用符で囲む）。読みは YAML 1.1 を真似しないので、手で裸の `yes` と書いた値は文字として読まれ、画面で保存すると `"yes"`（文字）として書かれる。聞き直してから書くまでの短い間に子が着手されると、着手の直後に書き込みが入りうる（残る隙間） |
...
src/
  ...
  core/
    ...
    flow-lint.ts      フローの本文を一時ファイルに書いて実行ファイル（--lint --json --flow）に確かめさせ、(flow) の error を理由にする
