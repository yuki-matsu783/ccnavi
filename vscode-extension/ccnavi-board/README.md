...
上部の「要対応だけ」を入れると、人が動く必要があるカード（札が付くもの、不備、レビュー待ちやゲート閉のフェーズを持つ親）だけが残る。
...
「利用者がフェーズ N のレビューを終えた。親の作業ツリーで `check --phase N` を打て」の文を出し、「コピー」「新しいセッションで開く」で
Claude Code に渡す。拡張はマーカーを置かず、実行ファイルも sh も起こさない。`check` を打ってマーカーを置くのは文を受けたエージェントで、
未解決の指摘が残っていれば `check` がその一覧と次の道を返す。
...
| フェーズ行の「レビュー済み連絡」 | 「受け入れ」と同じ条件で出る。何も起こさず、Claude Code に渡す文（親の識別子・フェーズ・依頼のマーカーの MR の URL・親の作業ツリーで `ccnavi-review.sh check --phase <N>` を打つこと）を組んでオーバーレイに出す。「コピー」「新しいセッションで開く」「閉じる」は承認の後と同じ。親の作業ツリーが無ければ通知で断る。承認の読み込み中・承認中は被せない |
...
| 「要対応だけ」 | 人が動く必要があるカードだけを出す。条件は札（未承認・ゲート閉・作業ツリーなし・人のレビュー待ち・HIGH 以上のリスク・本物が決まらない写り）と不備、親ならフェーズ行の要約（ゲート閉・レビュー待ち・HIGH 以上）に出るものと同じで、組み立て（`board.ts`）が付けた印を画面が読むだけ。プロジェクト・親の絞り込みと重ねて効き、列の件数と「承認待ち N 件を承認」もその分になる。選択は Webview の状態として覚える |
...
| 5d | 要対応だけ | 「要対応だけ」を入れる。「更新」する。外す | 順調な子と閉じた子が消え、未承認・ゲート閉・レビュー待ちの親などだけが残る。列の件数がその数になる。更新しても入ったまま。外すと全部戻る |
