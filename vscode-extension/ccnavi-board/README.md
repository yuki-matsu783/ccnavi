...
（未承認・ゲート閉・ワークツリーなし・レビュー依頼済・実績のリスクが HIGH 以上・本物が決まらない写り）。
「レビュー依頼済」は人のレビュー待ち（依頼を出したのにゲートが閉じたまま。JSON の `review_waiting`）の間だけで、
レビューが済んでゲートが開いたカードには出ない。親カードのフェーズ行の要約の「レビュー依頼済」も同じ条件で出る。
...
（終了は緑の塗り、進行中は青の輪）、右が状態。カードの内寸が 480px 未満（1920px の画面で 4 列のときも含む）では、状態を札と同じ項目の要約に絞る（ゲート閉、ゲートが閉じたままのレビュー依頼済、HIGH 以上のリスク。順調な段階は空）。他の列を畳むかドラッグで広げて 480px 以上になると、段階の状態、マーカー、レビューの要否、リスクの点と理由まで出る全文に替わる。狭いときも全文は読み上げに渡り、行にマウスを載せるとツールチップに出る。ゲートが閉じた段階は名前も状態も赤になる。
上部の見出しは承認待ちと不備の件数を 1 件以上のときだけ出し、残りと全体は薄く添える。
上部の「要対応だけ」を入れると、人が動く必要があるカード（承認待ち、札が付くもの、不備、レビュー待ちやゲート閉のフェーズを持つ親）だけが残る。
...
未解決の指摘が残っていれば `check` がその一覧と次の道を返す。文が「連結せず単体で」と言うのは、ゲートの閉じた親のワークツリーで通るのが
...
| フェーズ行の「受け入れ」 | 依頼済みでゲートが閉じたままのフェーズに出る。親のワークツリーに `cd` して `sh '<ワークスペースルート>/.ccnavi/scripts/ccnavi-review.sh' accept <N>` を送る（プロジェクトのワークツリーには `.ccnavi/scripts/` が無いので、ルートから綴る） |
...
| 「要対応だけ」 | 人が動く必要があるカードだけを出す。条件は、承認待ち（`pending_approval`。新規の未承認と親の改版。札の「未承認」とは違い、改版を含み、承認されずに取り消した提案は含まない）、ゲート閉、未着手・作業中なのにワークツリーが無い、人のレビュー待ち、HIGH 以上のリスク、本物が決まらない写り、不備、親ならフェーズ行の要約（ゲート閉・レビュー待ち・HIGH 以上）に出るもの。組み立て（`board.ts`）が付けた印を画面が読むだけ。プロジェクト・親の絞り込みと重ねて効き、列の件数と「承認待ち N 件を承認」もその分になる。選択は Webview の状態として覚える |
...
| 3c | 更新できない | 返らない実行ファイルを設定 `ccnaviBoard.binPath` に指して「更新」を押す。用意するのは**その OS が直接起動できる形**（Linux・macOS・WSL は `#!/bin/sh` と `sleep 999` を書いて実行ビットを立てた sh、Windows は同じことをする `.cmd`）。シェルを介さず起動するので、実行ビットとシェバンが無いと 60 秒待たずに即エラーになる。**名前を `ccnavi-launcher.sh` にしない**（振り分けの sh として読み飛ばされ、本物の実行ファイルで成功してしまう） | 60 秒はボタンが「更新中」のまま（待っている間に別のタブへ移って戻ると webview が作り直され、活性の「更新」に戻る）。そのあと「ccnavi --explain --json が失敗した: 60 秒で返らないので打ち切った」のエラー画面に替わる。更新ボタンごと消えるので、やり直しはコマンドパレットの「ボードを更新」 |
...
| 4f | 承認が途中で切れる | `--yes` が引数にあるときだけ返らず、それ以外は本物の ccnavi に渡す実行ファイルを指して、オーバーレイの「この N 件を承認する」を押す | オーバーレイが 60 秒「承認している…」のまま。そのあと「ccnavi --approve --yes を 60 秒で打ち切った。…」と出る。binPath を戻して「ボードを更新」すると、打ち切られるまでに置かれたぶんが承認済で出る（コミットと push は送られていない） |
| 5 | 受け入れ | レビュー依頼済みでゲートが閉じたフェーズの「受け入れ」を押す | ターミナルで親のワークツリーに `cd` してから `accept <N>` が走る |
| 5b | レビュー済み連絡 | 同じフェーズの「レビュー済み連絡」を押す。「コピー」で貼る。もう一度押して「新しいセッションで開く」 | オーバーレイに「フェーズ N のレビュー済みを連絡」と、親のワークツリーで `check --phase N` を打つ文（MR の URL 付き）。マーカーもターミナルも動かない。貼った文でエージェントが `check` を打ち、ゲートが開く。「新しいセッションで開く」で Claude Code の新しいタブに同じ文が埋まる |
| 5c | MR のリンク | 依頼済みのフェーズ行と親カードの「MR #N」を押す | 既定のブラウザで、フェーズ行は依頼のコメント、親カードはマージリクエストが開く。提案のプレビューは開かない |
| 5d | 要対応だけ | 「要対応だけ」を入れる。親の計画を書き換えて改版を承認待ちにする。「更新」する。外す | 順調な子（承認済みで作業中、閉じてレビューも済んだもの）が消え、承認待ち・ゲート閉の子・レビュー待ちの親などだけが残る。改版の親は承認済のまま要対応に残り、「承認待ち N 件を承認」の N に入る。列の件数がその数になる。更新しても入ったまま。外すと全部戻る |
...
| 6d | フェーズ行の要約と全文 | 4 列のまま親カードのフェーズ行を見る。行にマウスを載せる。他の 3 列を畳む | 順調な段階は段階名だけで、ゲート閉・レビュー依頼済・HIGH 以上のリスクだけが右に出る。ツールチップに「終了 · レビュー依頼済 · … · リスク: …」の全文。畳んで広がると右が全文に替わる |
