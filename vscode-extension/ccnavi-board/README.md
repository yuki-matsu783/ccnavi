...
列は 未着手 / 作業中（承認済み・レビュー待ちを含む）/ 完了 / 取り消し。置き場（ADR-0055）との対応は、未着手 = `wip/proposals/todo/`、作業中 = `.ccnavi/approved/doing/` と `wip/proposals/review/`（レビュー待ちはカードの属性「レビュー待ち」で分かる）、完了 = `.ccnavi/approved/done/`、取り消し = 同じ `done/` で `cancelled_at` を持つもの。カードに枠付きの札が付くのは、人が動く必要がある状態だけ
（未承認・レビュー準備中／レビュー待ち・書き込み停止中・ワークツリーなし・実績のリスクが HIGH 以上・本物が決まらない写り）。
レビューが済むまで止めている間は、その段の名前の札を 1 枚だけ出す。**依頼を出すまでが「レビュー準備中」**
...
（終了は緑の塗り、進行中は青の輪）、右が状態。カードの内寸が 480px 未満（1920px の画面で 4 列のときも含む）では、状態を札と同じ項目の要約に絞る（レビュー準備中／レビュー待ち、HIGH 以上のリスク。順調なフェーズは空）。他の列を畳むかドラッグで広げて 480px 以上になると、フェーズの状態、マーカー、レビューの要否、リスクの点と理由まで出る全文に替わる。狭いときも全文は読み上げに渡り、行にマウスを載せるとツールチップに出る。止めている段階は名前も状態も赤になる。
上部の見出しは承認待ちと不備の件数を 1 件以上のときだけ出し、残りと全体は薄く添える。
上部の「要対応だけ」を入れると、人が動く必要があるカード（承認待ち、札が付くもの、不備、レビュー準備中やレビュー待ちのフェーズを持つ親）だけが残る。
...
| 「要対応だけ」 | 人が動く必要があるカードだけを出す。条件は、承認待ち（`pending_approval`。新規の未承認と親の改版。札の「未承認」とは違い、改版を含む）、レビュー準備中／レビュー待ち、書き込み停止中（`blocked`。不備としても出る）、未着手・作業中なのにワークツリーが無い、HIGH 以上のリスク、本物が決まらない写り、不備、親ならフェーズ行の要約（レビュー準備中／レビュー待ち・HIGH 以上）に出るもの。組み立て（`board.ts`）が付けた印を画面が読むだけ。プロジェクト・親の絞り込みと重ねて効き、列の件数と「承認待ち N 件を承認」もその分になる。選択は Webview の状態として覚える |
...
| 42e | 図を触っても設定は動かない（**掴んで離す仕草は自動では見られない**。d3-drag が happy-dom の下で終わらないため、ここだけが見る） | 図で点を摘まんで動かし、別のタブに移って戻る。そのあと `git diff` を見る | 動かした位置は残っている。`phases.yml` に差分は出ない（座標は設定に書かない）。点を押すと一覧に戻り、その種類の行が開く |
...
src/
  ...
  webview/            画面（React）。DOM を触る側で、vscode も node も import しない。tsconfig.webview.json で型を見る
    ...
    projects/Project.css Project.tsx の CSS（カード 1 枚・札・検証）
    ...
    rules/Rule.css    Rule.tsx の CSS（ルール 1 件の行・ツールの札）
