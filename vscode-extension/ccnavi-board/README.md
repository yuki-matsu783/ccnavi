...
src/
  ...
  webview/            画面（React）。DOM を触る側で、vscode も node も import しない。tsconfig.webview.json で型を見る
    ...
    Tour.tsx          吹き出しの案内と、その出し入れ（useTour）。5 画面で 1 本。画面ごとの初回（拡張ホストの tour）と、ツールバーの「？ 案内」（フェーズ管理はヘルプの「案内をもう一度見る」）で出る
