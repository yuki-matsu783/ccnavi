...
| 上部の「承認待ち N 件を承認」、未承認カードの「承認」 | ターミナルに `ccnavi --approve` を送る。束を見せて y/N を取る。絞り込み中は、見えている承認待ちの識別子を並べて送り（`--approve i0002 i0002-01`）、その分だけの束になる。N もその数。絞り込みが無ければ承認待ち全部の束。親を束から外した子は実行ファイルが落とす |
