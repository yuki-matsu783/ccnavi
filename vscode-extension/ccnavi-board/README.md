...
| 操作の一言（clone の欄の下） | 失敗（赤）は次の一覧が届いたら消す。案内（「clone を送った」など）は残す（clone の直後は `.git` の出現で必ず読み直しが走るため） |
...
`project:` の層の和で行う（設計 11.4.2）が、この画面ではそれらを開かない（設計 11.11）。層の配点はエディタで直す。編集中の内容は一時ファイルに書いて `--lint --risk <パス>` で実行ファイルに渡す。保存の往復の間は欄を止める。`--lint` は設定全体を見るので、`rules.yml` などに error がある間は配点も保存できない。
...
| 読み直しの見え方 | 画面は React で、拡張ホストが渡すのは中身（`RiskData`）だけ。入れ物（HTML）は開いたときに 1 度入るだけ（`retainedHost`、ADR-0062）。中身が届くのは編集を捨ててよいときだけで、人が「再読込」を押したときと、保存・作成が通ったとき。外で変わっただけのときは帯が出るだけで、打ちかけの欄も開いた行も残る。絞り込みは読み直しのあとも残る |
| 往復の間 | 「保存」「作る」「再読込」を押した時点で欄と 3 つのボタンを止める（帯の中の「再読込」も同じ）。人が「破棄して読み直す？」をやめたときは、拡張ホストがそう伝えて欄が戻る。往復の間に読み直されていたら、遅れて通った保存は捨てる |
...
| 何 | どう出るか、何が起きるか |
|---|---|
| 種類 | `phases` を 1 件 1 行（id・title・kind のラベル・review・scope）で並べる。既定は全部畳んであり、行を押すとその下に欄が開く。開いた行は id で控え、再読込のあとも開いたまま。欄は id（対応表のキー）・title・kind（work / feedback）・review（none / mr）・scope（inherit か glob の並び）・deliverables と、「ほかの種類との関係・補足」の 1 行に畳んだ overlap・requires・after・agent・when（値がある種類だけ最初から開く）。overlap・requires・after は、このファイルのほかの種類の id を複数選択のセレクトボックスで選ぶ（押すたびに 1 件ずつ付け外しし、Ctrl は要らない。キーは矢印・Home・End で印を動かし、Space か Enter で付け外しする。素の矢印のように選択を 1 件に縮めない。自分の id は候補に出ない。ほかの層の種類は id を打って Enter で足す）。一覧の上にファイルの頭の `order`（全体計画の待ち方。sequential / dag）の選択がある。sequential は既定で、元から欄が無ければ書かない。scope と deliverables は `,` 区切りの 1 欄。足す（足した行は開いて出る。図を見ていたら一覧へ移る）・消す・上下に動かす・改名する。足した種類の既定は `scope: inherit` |
...
| ファイルが無い | 共通層は空の画面と「共通層に種類は無い。種類は各層（ワークスペース自身・プロジェクト）に置く」の帯、「自身の層を開く」ボタン。画面から共通層のファイルは作らない（雛形を置くと層の同じ id と中身が食い違い、その層が空として扱われるため）。組み込みの既定も雛形も無い（実行ファイルも持たない）。層も雛形は置かず、作るボタンも出ない。層の欄はそのまま触れ、種類を足して保存すると、`--lint` を通った最初の保存でファイルが作られる（先頭に説明のコメントが付く）。種類の無いファイル（`phases: {}`）は先に書き出さない |
...
| 読み直しの見え方 | 画面は React で、拡張ホストが渡すのは中身（`PhasesData`）だけ。リスク管理と同じで、入れ物（HTML）は開いたときに 1 度入るだけ（`retainedHost`、ADR-0062）。中身が届くのは編集を捨ててよいときだけで、外で変わっただけのときは帯が出る。往復の間に欄を止めるのもリスク管理と同じ。並びの欄（scope・成果物）は打っている途中の文字を欄が持つので、`,` の直後もそのまま打てる。「ほかの種類との関係・補足」の開閉は行ごとに 1 度だけ値の有無で決め、あとは人の開閉で動く |
...
- それとは別に、承認の側からも読み直す（`approval-machine.ts` の `refresh`）。監視は `CCNAVI_TICKETS_APPROVED` を
  読まない（`core/watch.ts` の綴りは固定）ので、置き場を変えたときや `files.watcherExclude` で外したときも動く。
...
| 39 | フェーズ管理画面が開く | プロジェクト管理画面の「ワークスペース自身」の「フェーズ管理」（40〜44 も同じ画面） | 種類 8 件（このリポジトリの自身の層）。`implement-feedback` の scope が inherit で、feedback の 2 件に左端の色。上部の path が `.ccnavi/config/phases.yml` |
...
| 43 | 共通層に作らせない | サイドパネルの「フェーズ管理」で共通層を開く（このリポジトリには `.ccnavi/common/phases.yml` が無い）。「自身の層を開く」を押す | 「共通層に種類は無い。種類は各層…に置く」の帯と「自身の層を開く」。雛形で作るボタンは無く、欄は無く、追加も押せない。押すと同じタブが「ccnavi フェーズ管理: 自身の層」に切り替わる。`.ccnavi/common/phases.yml` は出来ない |
| 44 | 作業中は保存できない | 子チケットを `start` してから「保存」 | 上部に赤で「作業中のチケットがある」。保存ボタンが押せない。`finish` にすると押せる |
| 44b | 自身の層のフェーズ管理 | プロジェクト管理画面の「ワークスペース自身」の「フェーズ管理」。`docs` の title を同じファイルの別の種類の title と同じにして「保存」 | タブの題が「ccnavi フェーズ管理: 自身の層」、上部の path が `.ccnavi/config/phases.yml`。保存は `--lint` の error（title の重なり）で止まる。戻して別の変更なら保存でき、`ccnavi --explain` の phases の表に `self` の層で出る |
...
| 44d | 層またぎの表示名の重なり | 前提: 共通層の種類を手で用意する（エディタで `.ccnavi/common/phases.yml` を作り、`version: 1` と種類 1 つ（例 `review-only`、`kind: work`、`title: 共通の確認`、`review: mr`、`scope: inherit`）を書く。画面からは作れない）。プロジェクト管理画面の「ワークスペース自身」の「フェーズ管理」で `docs` の title を `共通の確認` にして「保存」。試したら title を戻し、作ったファイルを消す | 保存は `--lint` の error（表示名が層をまたいで重なる）で止まる。`ccnavi --lint` でも同じ error。重なりを見るのは共通層と各層の間で、自身の層とプロジェクトの層どうしは合成しない（`phasetypes.merge`）ので、その 2 つで title を重ねても止まらない |
...
src/
  ...
  phases-panel.ts     フェーズ管理画面の Webview パネル（1 枚。対象（共通層・自身の層・プロジェクト）を切り替える）。検証・保存の受け付け（vscode に依存する）
