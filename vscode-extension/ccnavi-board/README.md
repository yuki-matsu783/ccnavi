...
「利用者がフェーズ N のレビューを終えた。親が親の作業ツリーで `check --phase N` を単体で打て」の文を出し、「コピー」「新しいセッションで開く」で
Claude Code に渡す。拡張はマーカーを置かず、実行ファイルも sh も起こさない。`check` を打ってマーカーを置くのは文を受けた親（メインエージェント）で、
未解決の指摘が残っていれば `check` がその一覧と次の道を返す。文が「連結せず単体で」と言うのは、ゲートの閉じた親の作業ツリーで通るのが
...
開いたままの画面は閉じない（編集中の内容を消さないため）。代わりに帯で「いまは何にも効いていない」と言う。
...
| チケット制御が disable | 画面が開かない（入口も出ない）。開いたまま設定が disable に変わったときだけ、上部に「配点は何にも効かない」と出る。そのときも編集と保存はできる（画面を作り直さないので編集中の内容は消えない） |
...
| チケット制御が disable | 画面が開かない（入口も出ない。対象がどれでも同じ）。開いたまま設定が disable に変わったときだけ、上部に「種類は何にも効かない」と出る。そのときも編集と保存はできる（画面を作り直さないので編集中の内容は消えない） |
...
| フェーズ行の「受け入れ」 | 依頼済みでゲートが閉じたままのフェーズに出る。親の作業ツリーに `cd` して `sh '<ワークスペースルート>/.ccnavi/scripts/ccnavi-review.sh' accept <N>` を送る（プロジェクトの作業ツリーには `.ccnavi/scripts/` が無いので、ルートから綴る） |
| フェーズ行の「レビュー済み連絡」 | 「受け入れ」と同じ条件で出る。何も起こさず、Claude Code に渡す文（親の識別子・フェーズ・依頼のマーカーの MR の URL・親の作業ツリーで `ccnavi-review.sh check --phase <N>` を打つこと）を組んでオーバーレイに出す。「コピー」「新しいセッションで開く」「閉じる」は承認の後と同じ。親の作業ツリーが無い、そのフェーズが人のレビュー待ちでない（ボードが古い）ときは通知で断る。承認のオーバーレイ（読み込み中・一覧・承認中・承認した文）の上には被せない。承認した文は取り返せないので、渡し終えるか閉じるまで残す。文の sh の綴りは実行ファイルの案内と同じく realpath で解いたワークスペースルートから書く |
...
| 「要対応だけ」 | 人が動く必要があるカードだけを出す。条件は、承認待ち（`pending_approval`。新規の未承認と親の改版。札の「未承認」とは違い、改版を含み、承認されずに取り消した提案は含まない）、ゲート閉、未着手・作業中なのに作業ツリーが無い、人のレビュー待ち、HIGH 以上のリスク、本物が決まらない写り、不備、親ならフェーズ行の要約（ゲート閉・レビュー待ち・HIGH 以上）に出るもの。組み立て（`board.ts`）が付けた印を画面が読むだけ。プロジェクト・親の絞り込みと重ねて効き、列の件数と「承認待ち N 件を承認」もその分になる。選択は Webview の状態として覚える |
...
- 提案（`wip/**/tickets/`、全作業ツリーの同じ場所）・承認済みチケットとマーカー（`.ccnavi/tickets/`）・
...
| 5b | レビュー済み連絡 | 同じフェーズの「レビュー済み連絡」を押す。「コピー」で貼る。もう一度押して「新しいセッションで開く」 | オーバーレイに「フェーズ N のレビュー済みを連絡」と、親の作業ツリーで `check --phase N` を打つ文（MR の URL 付き）。マーカーもターミナルも動かない。貼った文でエージェントが `check` を打ち、ゲートが開く。「新しいセッションで開く」で Claude Code の新しいタブに同じ文が埋まる |
...
| 14c | 開いたまま切る | リスク管理画面（またはフェーズ管理画面）を開いたまま、14b の行を書く | 画面は閉じず、上部に「いまは何にも効いていない」の帯が出る。編集中の欄はそのまま残る。行を消すと帯が消える。設定ファイルを見張っているので「ファイルが外で変更された」の帯も一緒に出るが、配点も種類も変わっていないので再読込は要らない（押すと未保存の内容を捨てるか聞かれる） |
