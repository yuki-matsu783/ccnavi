...
| オーバーレイの「この N 件を承認する」 | 見せた識別子をそのまま `--approve --yes <識別子,…> --json` に渡す。一覧が変わっていれば何も置かれず、読み直して「見せた一覧と今の一覧が違った」と出る。承認できたらオーバーレイが「N 件を承認した」に替わり、Claude Code に渡す文と「コピー」「新しいセッションで開く」「閉じる」が出る |
...
| 4c | 1 件だけ承認 | 同じ親の子を 2 枚承認待ちにして、片方のカードの「この 1 件を承認」を押す | オーバーレイに出るのはその 1 件だけ。承認すると、もう片方は承認待ちのまま残る |
