...
列はチケットの置き場（承認待ち / 作業中 / レビュー待ち / 完了。承認待ちとレビュー待ちは `wip/proposals/<状態>/`、作業中と完了は `.ccnavi/approved/<状態>/`。取り消しは完了に入り「取り消し」の属性で分かる）。カードの枠付きの札は人が動く必要がある状態だけ
（未承認・レビュー準備中／レビュー待ち・ワークツリーなし・実績のリスクが HIGH 以上・本物が決まらない写り）。
...
| 「要対応だけ」 | 人が動く必要があるカードだけを出す。条件は、承認待ち（`pending_approval`。新規の未承認と親の改版。札の「未承認」とは違い、改版を含む）、レビュー準備中／レビュー待ち、承認待ち・作業中なのにワークツリーが無い（レビュー待ち・完了は畳んでいてよい）、HIGH 以上のリスク、本物が決まらない写り、不備、親ならフェーズ行の要約（レビュー準備中／レビュー待ち・HIGH 以上）に出るもの。組み立て（`board.ts`）が付けた印を画面が読むだけ。プロジェクト・親の絞り込みと重ねて効き、列の件数と「承認待ち N 件を承認」もその分になる。選択は Webview の状態として覚える |
...
| 3 | 自動更新 | ボードを開いたまま、別のターミナルで `sh .ccnavi/scripts/ccnavi-ticket.sh done <子>` | コマンドを打たなくてもカードがレビュー待ち（レビュー不要なら完了）へ動く |
