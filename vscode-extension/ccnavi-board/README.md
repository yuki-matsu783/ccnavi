...
| オーバーレイの「この N 件を承認する」 | 見せた識別子をそのまま `--approve --yes <識別子,…> --json` に渡す。一覧が変わっていれば何も置かれず、読み直して「見せた一覧と今の一覧が違った」と出る。読み直しは、見せたときと同じ絞り（カードの 1 件、絞り込みで見えていた分）で行い、その絞りが通らない（識別子がもう承認待ちに無い、親の改版が承認待ちに入った）ときだけ承認待ち全部に戻す。承認できたらオーバーレイが「N 件を承認した」に替わり、Claude Code に渡す文と「コピー」「新しいセッションで開く」「閉じる」が出る |
