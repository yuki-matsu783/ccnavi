...
| 3f | 裏にいる間の見た目の切り替え（保持しない画面） | ボードのタブを裏に回し、コマンドパレットの「ccnavi ボード: 見た目を切り替える」で配色を変える。ボードを表に戻す | 戻したボードが新しい配色で出る（裏にいる間の切り替えは段取りが落とし、作り直された画面が組み上がったところで渡し直される。issue #87） |
...
| 38c | 裏にいる間の見た目の切り替え（保持する画面） | 項目を 1 つ開いて points を打ちかけ、タブを裏に回してから「見た目を切り替える」で配色を変え、表に戻す | 戻した画面が新しい配色で、打ちかけの値も開いた行もそのまま（作り直されないので、表に戻ったところで送り直したものが効いている。issue #87） |
