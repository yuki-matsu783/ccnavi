...
枠の無い薄い文字で 1 行に並ぶ。親カードにはフェーズの一覧が段階ごとに付き、左の丸が段階
（終了は緑の塗り、進行中は青の輪）、状態にはゲート閉、マーカー、人レビュー要、リスクなど人が見るべきことだけが出る。状態はカードが狭いとき（既定の 4 列など）は段階名の下に折り返して置き、他の列を畳むかドラッグで広げてカードの内寸が 480px 以上になると段階名の右に寄せる。ゲートが閉じた段階は名前も状態も赤になる。
