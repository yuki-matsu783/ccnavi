...
（終了は緑の塗り、進行中は青の輪）、右が状態。カードの内寸が 480px 未満（1920px の画面で 4 列のときも含む）では、状態を札と同じ項目の要約に絞る（レビュー準備中／レビュー待ち、HIGH 以上のリスク。順調なフェーズは空）。他の列を畳むかドラッグで広げて 480px 以上になると、フェーズの状態、マーカー、レビューの要否、リスクの点と理由まで出る全文に替わる。狭いときも全文は読み上げに渡り、行にマウスを載せるとツールチップに出る。止めているフェーズは名前も状態も赤になる。
