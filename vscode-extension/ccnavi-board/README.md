...
| 4e | 承認できない | 3c と同じ実行ファイルを指して「承認待ち N 件を承認」を押す | オーバーレイが 60 秒「承認待ちの内容を読み込み中…」のまま。そのあと「ccnavi --approve --preview --json を 60 秒で打ち切りました。まだ 1 件も承認していません」と出て、「閉じる」で閉じられる。この 60 秒の間に監視が読み直しを起こすと、そちらも打ち切られてエラー画面が画面ごと覆う。そのときは binPath を戻して「チケット管理を更新」すると、残っていたオーバーレイが出てくる |
...
| 22 | 未保存の再読込 | 何か変えてから「更新」 | 「破棄して更新しますか？」の確認。「更新」で編集が消える |
...
| 25d | 読み直しの失敗が残らない | 一覧が出ている状態で `ccnaviBoard.binPath` を存在しない綴りに変える（読み直しが失敗するようにする）→ タブを裏に回して表に戻す | 「プロジェクトの一覧を更新できませんでした」の文と理由が出る。古い一覧が残らない。綴りを戻して「ccnavi ボード: プロジェクト管理を開く」を打つと一覧に戻る |
