...
受けない」の見張り（承認中は閉じない・打てるのは一覧を見せているときだけ・承認の途中は二重に開かない・
一覧を受けるのは頼んだときだけ・文を渡せるのは渡す文があるときだけ・承認の文の上に連絡を被せない・
絞り込みの識別子がいまのボードでも承認待ちか）はそこに集めてあり、`board-panel.ts` は入力を入れて
返ってきた「やること」を行うだけ。VS Code に触れないので単体で試せる（CB-T169〜181。
**CB-T181 は変異テストで、見張りを 1 つ消したらテストが落ちることまで見る**）。
...
切り替えは `src/core/appearance.ts` と `src/webview/appearance.ts`）。ハイコントラストのテーマでは Claude の配色を
