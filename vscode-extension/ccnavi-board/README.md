...
`accept` は今までどおり統合ターミナルへコマンドを送り、y/N は人がターミナルで押す。
親を早期に締める `wrapup` はボードに出さない。端末で `ccnavi-review.sh wrapup` を打つ。

レビューを終えたことをエージェントに伝えるのは、フェーズ行の「レビュー済み連絡」。承認と同じオーバーレイに
「利用者がフェーズ N のレビューを終えた。親は自分のワークツリーで `check --phase N` を単体で打て」の文を出し、「コピー」「新しいセッションで開く」で
Claude Code に渡す。拡張はマーカーを置かず、実行ファイルも sh も起こさない。`check` を打ってマーカーを置くのは文を受けた親（メインエージェント）で、
未解決の指摘が残っていれば `check` がその一覧と次の道を返す。文が「連結せず単体で」と言うのは、レビューで止まった親のワークツリーで通るのが
...
| フェーズ行の「受け入れ」 | 依頼済みで止まったまま（レビュー待ち）のフェーズに出る。親のワークツリーに `cd` して `sh '<ワークスペースルート>/.ccnavi/scripts/ccnavi-review.sh' accept <N>` を送る（プロジェクトのワークツリーには `.ccnavi/scripts/` が無いので、ルートから綴る） |
| フェーズ行の「レビュー済み連絡」 | 「受け入れ」と同じ条件で出る。何も起こさず、Claude Code に渡す文（親の識別子・フェーズ・依頼のマーカーのマージリクエストの URL・親のワークツリーで `ccnavi-review.sh check --phase <N>` を打つこと）を組んでオーバーレイに出す。「コピー」「新しいセッションで開く」「閉じる」は承認の後と同じ。親のワークツリーが無い、そのフェーズが人のレビュー待ちでない（ボードが古い）ときは通知で断る。承認のオーバーレイ（読み込み中・一覧・承認中・承認した文）の上には被せない。承認した文は取り返せないので、渡し終えるか閉じるまで残す。文の sh の綴りは実行ファイルの案内と同じく realpath で解いたワークスペースルートから書く |
...
| 5 | 受け入れ | レビュー待ち（依頼済みで止まったまま）のフェーズの「受け入れ」を押す | ターミナルで親のワークツリーに `cd` してから `accept <N>` が走る |
| 5b | レビュー済み連絡 | 同じフェーズの「レビュー済み連絡」を押す。「コピー」で貼る。もう一度押して「新しいセッションで開く」 | オーバーレイに「フェーズ N のレビュー済みを連絡」と、親のワークツリーで `check --phase N` を打つ文（MR の URL 付き）。マーカーもターミナルも動かない。貼った文でエージェントが `check` を打ち、止まっていたのが解ける。「新しいセッションで開く」で Claude Code の新しいタブに同じ文が埋まる |
