...
| `ccnavi ボード: ルール設定を開く` | ワークスペースのルール設定画面を開く。既に開いていれば前面に出す（別の層を見ていれば、そのタブの中身をワークスペースのルールに入れ替える） |
...
| `ccnavi ボード: フェーズ管理を開く` | フェーズ管理画面を開く。既に開いていれば前面に出す（別の層を見ていれば、そのタブの中身を共通層に入れ替える）。チケット制御が `disable` なら一覧に出ず、呼ばれても開かずに理由を言う |
...
| 何 | どう出るか、何が起きるか |
|---|---|
| 種類 | `phases` を 1 件 1 行（id・title・kind のラベル・review・scope）で並べる。既定は全部畳んであり、行を押すとその下に欄が開く。開いた行は id で控え、更新のあとも開いたまま。欄は id（対応表のキー）・title・kind（work / feedback）・review（none / mr）・scope（inherit か glob の並び）・deliverables と、「ほかの種類との関係・補足」の 1 行に畳んだ overlap・requires・after・agent・when（値がある種類だけ最初から開く）。overlap・requires・after は、このファイルのほかの種類の id を複数選択のセレクトボックスで選ぶ（押すたびに 1 件ずつ付け外しし、Ctrl は要らない。キーは矢印・Home・End で印を動かし、Space か Enter で付け外しする。素の矢印のように選択を 1 件に縮めない。自分の id は候補に出ない。ほかの層の種類は id を打って Enter で足す）。一覧の上にファイルの頭の `order`（全体計画の待ち方。sequential / dag）の選択がある。sequential は既定で、元から欄が無ければ書かない。scope と deliverables は `,` 区切りの 1 欄。足す（足した行は開いて出る。図を見ていたら一覧へ移る）・消す・上下に動かす・改名する。足した種類の既定は `scope: inherit`（glob を埋め忘れて「何も書けない」種類にならないように） |
...
| 7d | 切り替えの確認 | ルール設定で何か打ちかけたまま、プロジェクト管理から別の層の「ルール設定」を押す。「キャンセル」。もう 1 度押して「切り替える」。フェーズ管理でも同じ | 「未保存の変更があります。破棄して「ccnavi ルール設定: <名前>」に切り替えますか？」が出る。キャンセルなら前の対象と打ちかけが残る。切り替えると「…を読み込み中…」を挟んでその層の中身と題になる。打ちかけが無ければ聞かずに切り替わる |
| 7e | 往復の途中で切り替える | 7b の遅い実行ファイルで、ルール設定の「判定」か「保存」を押した直後に別の層を開く。確認が出ている間にもう 1 つ別の層を開く | 切り替え先に「更新したので、…は取りやめました」は出ず、前の対象の判定の結果も残らない。確認は 1 枚だけで、2 つ目の押下は前面に出すだけ。切り替え先が読めずエラーになったら、前の対象のファイルを触っても「ファイルの変更を検知しました」の帯は出ない |
...
| 14b | チケット制御を切る | `.claude/settings.json` の env に `"CCNAVI_TICKET_CONTROL": "disable"` を書く | サイドパネルが「プロジェクト管理」「ルール設定」の 2 つになり（タイトルバーの歯車は残る）、コマンドパレットから「チケット管理を開く」「チケット管理を更新」「リスク管理を開く」「フェーズ管理を開く」が消える。プロジェクト管理の各行から「チケット管理」と「フェーズ管理」が、「ワークスペース自身」の枠から「自身の層のフェーズの種類」が消える。行を消すと戻る |
...
| 44c | プロジェクトの層に作る | 層の種類が無いプロジェクトの行の「フェーズ管理」→ 種類を 1 つ足して「保存」。次にそのプロジェクトの子を `start` して「保存」、別のプロジェクトの子だけを `start` にして「保存」 | 「… がありません。無い層は空で、共通層の種類だけが使われます…」の帯が出るが作るボタンは無く、欄は触れる。保存で `projects/<名前>/.ccnavi/config/phases.yml` が先頭の説明コメント付きで出来て、足した種類が入る。前者は「プロジェクト <名前> に作業中のチケットがあります（…）」で止まり、後者は保存できる |
