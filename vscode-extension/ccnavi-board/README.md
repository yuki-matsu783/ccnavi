...
承認済／レビュー待ち、人間レビューの要否、ワークツリーの名前、レビュー済／省略、MEDIUM 以下のリスク、base は
枠の無い薄い文字で 1 行に並ぶ。列やバッジと同じことは重ねない（クローズは完了の列で、終了の印はバッジで分かる）。
閉じたカードには人間レビューの要否も出さず、レビュー済／省略とリスクの点だけを残す。親カードにはフェーズの一覧が 1 フェーズ 1 行で付き、左の丸がフェーズ
