...
| 「要対応だけ」 | 人が動く必要があるカードだけを出す。条件は、承認待ち（`pending_approval`。新規の未承認と親の改版。札の「未承認」とは違い、改版を含む）、レビュー準備中／レビュー待ち、未着手・作業中なのにワークツリーが無い、HIGH 以上のリスク、本物が決まらない写り、不備、親ならフェーズ行の要約（レビュー準備中／レビュー待ち・HIGH 以上）に出るもの。組み立て（`board.ts`）が付けた印を画面が読むだけ。プロジェクト・親の絞り込みと重ねて効き、列の件数と「承認待ち N 件を承認」もその分になる。選択は Webview の状態として覚える |
