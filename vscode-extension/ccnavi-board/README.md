...
同じ拡張に「リスク管理画面」がある。実績で測るリスクの配点（`risks.yml`）の閾値と項目を画面で直し、
...
| 何 | どう出るか、何が起きるか |
|---|---|
| 等級の閾値 | `levels` の medium / high / critical の 3 欄を 1 行に。空ならその等級は組み込みの値（20 / 40 / 70）で、保存しても欄を書かない。説明は見出しの下の「この欄の説明」を開くと出る |
...
| 欄名 | 日本語で、`risks.yml` のキー名は欄名にマウスを重ねると出る。id = `id`、点 = `points`、当て方 = `lines_over` / `files_over` / `deleted_over` / `glob` / `script` / `judge`、しきい値・glob・スクリプト・問い = 当て方の値、上限 = `max`、文面 = `message`、MEDIUM / HIGH / CRITICAL = `levels` の各等級 |
...
| 34 | リスク管理画面が開く | サイドパネルの「リスク管理」 | 閾値 3 欄と項目 4 件（このリポジトリの `risks.yml`）。上部の path が `.ccnavi/common/risks.yml` |
...
src/
  ...
  webview/            画面（React）。DOM を触る側で、vscode も node も import しない。tsconfig.webview.json で型を見る
    ...
    risk/App.tsx      帯・ツールバー・等級の閾値・項目の一覧と、拡張ホストからのメッセージの受け
