...
承認済／クローズ、人間レビューの要否、ワークツリーの名前、レビュー済／省略、MEDIUM 以下のリスク、base は
枠の無い薄い文字で 1 行に並ぶ。親カードにはフェーズの一覧が 1 フェーズ 1 行で付き、左の丸がフェーズ
...
行き先（対応しない・このフェーズで直す・issue に回す）を選ぶ。拡張は `ccnavi-review.sh decide <N> --preview` で
...
| 何 | どう出るか、何が起きるか |
|---|---|
| リスクレベルが上がる点数 | `levels` の medium / high / critical の 3 欄を 1 行に。空ならそのリスクレベルは組み込みの値（20 / 40 / 70）で、保存しても欄を書かない。説明は見出しの下の「この欄の説明」を開くと出る |
...
| bash | clone / fetch / pull を送るターミナル。承認と残った指摘の行き先は通らない（子プロセスで打つ。sh はこのシェルで走らせる）。Windows は Git Bash（`C:\Program Files\Git\bin\bash.exe`、無ければ PATH の `bash`） |
...
| 4e | 承認できない | 3c と同じ実行ファイルを指して「承認待ち N 件を承認」を押す | オーバーレイが 60 秒「承認待ちの一覧を読み込んでいる…」のまま。そのあと「ccnavi --approve --preview --json を 60 秒で打ち切った。承認済みチケットは置かれていない」と出て、「閉じる」で閉じられる。この 60 秒の間に監視が読み直しを起こすと、そちらも打ち切られてエラー画面が画面ごと覆う。そのときは binPath を戻して「ボードを更新」すると、残っていたオーバーレイが出てくる |
...
| 5 | 残った指摘を決める | レビュー待ち（依頼済みで止まったまま）で未解決の指摘が付いたフェーズの「決める」を押し、指摘ごとに行き先を選んで「この行き先で決める」 | 全部に選ぶまで押せない。押すと「置いている…」で閉じられず、置けたら決めた文とコピー・新しいセッションで開くが出る。「このフェーズで直す」を選んだ分は続きの子チケットがボードに現れ、issue に回した分はマージリクエストのコメントに issue の綴りが残る |
...
src/
  ...
  core/
    ...
    commands.ts       ターミナルに送るコマンド行と、承認・残った指摘の行き先を子プロセスで打つ引数の並び
    decidemodel.ts    残った指摘の JSON（実行ファイルとの契約）の読み取りと、選んだ行き先の確かめ
  ...
  webview/            画面（React）。DOM を触る側で、vscode も node も import しない。tsconfig.webview.json で型を見る
    ...
    board/Decide.tsx   残った指摘の行き先のオーバーレイの中身（指摘ごとの選択）
    ...
    risk/App.tsx      帯・ツールバー・リスクレベルが上がる点数・項目の一覧と、拡張ホストからのメッセージの受け
