...
タブは画面の種類ごとに 1 枚で、5 種類は同時に開ける。どの画面も、押したらまずタブを出して「読み込み中」を見せ、
中身を読み終えたら差し替える（実行ファイルへの問い合わせを待つ間に押し直しても、タブは増えない）。
...
| 欄名 | 日本語で、`phases.yml` のキー名は欄名にマウスを重ねると出る。id = `id`、題 = `title`、区分 = `kind`、レビュー = `review`、範囲 = `scope`、成果物 = `deliverables`、並行できる種類 = `overlap`、一緒に必要な種類 = `requires`、先に済ませる種類 = `after`、担当エージェント = `agent`、使う場面 = `when` |
...
| 7b | 先にタブが出る | 返るのが遅い実行ファイル（3c と同じ、`sleep 5` の sh）を `ccnaviBoard.binPath` で指して、サイドパネルの「チケット管理」を押す。待っている間にもう 1 度押す。プロジェクト管理でも同じ | 押した直後にタブが出て「ccnavi チケット管理を読み込んでいる…」。2 度目は前面に出るだけでタブは 1 枚。返ったら中身かエラーに替わる |
