...
上部の「要対応だけ」を入れると、人が動く必要があるカード（承認待ち、バッジが付くもの、不備、レビュー準備中やレビュー待ちのフェーズを持つ親）だけが残る。
...
| 「要対応だけ」 | 人が動く必要があるカードだけを出す。条件は、承認待ち（`pending_approval`。新規の未承認と親の改版。バッジの「未承認」とは違い、改版を含む）、レビュー準備中／レビュー待ち、書き込み停止中（`blocked`。不備としても出る）、未着手・作業中なのにワークツリーが無い、HIGH 以上のリスク、本物が決まらない写り、不備、親ならフェーズ行の要約（レビュー準備中／レビュー待ち・HIGH 以上）に出るもの。組み立て（`board.ts`）が付けた印を画面が読むだけ。プロジェクト・親の絞り込みと重ねて効き、列の件数と「承認待ち N 件を承認」もその分になる。選択は Webview の状態として覚える |
...
| 5d | 要対応だけ | 「要対応だけ」を入れる。親の計画を書き換えて改版を承認待ちにする。「更新」する。外す | 順調な子（承認済みで作業中、閉じてレビューも済んだもの）が消え、承認待ち・レビューで止まった子・レビュー待ちの親などだけが残る。改版の親は承認済のまま要対応に残り、「承認待ち N 件を承認」の N に入る。列の件数がその数になる。更新しても入ったまま。外すと全部戻る |
