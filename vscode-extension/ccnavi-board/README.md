...
列は 未着手 / 作業中（承認済み・レビュー待ちを含む）/ 完了 / 取り消し。置き場（ADR-0055）との対応は、未着手 = `wip/proposals/todo/`、作業中 = `.ccnavi/approved/doing/` と `wip/proposals/review/`（レビュー待ちはカードの属性「レビュー待ち」で分かる）、完了 = `.ccnavi/approved/done/`、取り消し = 同じ `done/` で `cancelled_at` を持つもの。カードの枠付きの札は人が動く必要がある状態だけ
（未承認・レビュー準備中／レビュー待ち・書き込み停止中・ワークツリーなし・実績のリスクが HIGH 以上・本物が決まらない写り）。
レビューが済むまで止めている間は、その段の名前を 1 枚だけ出す。**依頼を出すまでが「レビュー準備中」**
...
「利用者がフェーズ N のレビューを終えた。親が親のワークツリーで `check --phase N` を単体で打て」の文を出し、「コピー」「新しいセッションで開く」で
...
「プロジェクト管理」「ルール管理」「リスク管理」「フェーズ管理」「チケット管理」「見た目」の入口が名前だけで並ぶ（何ができるかはマウスを重ねると出る）。「チケット管理」「リスク管理」「フェーズ管理」が出るのは、ワークスペースが
...
サイドパネル（左端の ccnavi アイコン）の「プロジェクト管理」「ルール管理」「リスク管理」「フェーズ管理」「チケット管理」「見た目」は、
...
切り替えは `src/core/appearance.ts` と `src/webview/appearance.ts`）。**送り方は、ほかの中身と同じ段取り（`ScreenHost`）を通る。** 組み上がっていない画面と捨てられた画面には送らず、保持しない画面は作り直されたあと `ready` で、保持する画面は表に戻ったところで送り直す（issue #87）。ハイコントラストのテーマでは Claude の配色を
...
| 保存 | 一時ファイルへ書いて `--lint --phases` を通し、error があれば保存しない。識別子に使えない id、title の重なり、feedback で `review: none`、overlap / requires が指す先の無い種類、提案がワークツリーに在る親の計画が指す種類を消したとき、はここで止まる（承認済みチケットの計画は照合しない。定義が読めなくなるときだけ止まる。チケット制御が disable なら照合は走らない）。同じ id が 2 つあるときは画面が先に止める（実行ファイルは後ろの定義で黙って上書きするため） |
...
| 5c | MR のリンク | 依頼済みのフェーズ行と親カードの「MR #N」を押す | 既定のブラウザで、フェーズ行は依頼のコメント、親カードはマージリクエストが開く。提案のプレビューは開かない |
...
| 6b | 絞って承認 | 承認待ちが 2 つの親にまたがるボードで、片方の親で絞って「承認待ち N 件を承認」を押す | N はその親の分だけ。ターミナルの `--approve` に識別子が並び、承認の対象もその分だけ。y の後、もう片方の親の承認待ちは残る |
...
| 6d | フェーズ行の要約と全文 | 4 列のまま親カードのフェーズ行を見る。行にマウスを載せる。他の 3 列を畳む | 順調な段階は段階名だけで、レビュー準備中／レビュー待ち・HIGH 以上のリスクだけが右に出る。ツールチップに「終了 · レビュー依頼済 · … · リスク: …」の全文。畳んで広がると右が全文に替わる |
...
| 12 | 読めない承認済みチケット | ボードを開いたまま `.ccnavi/approved/doing/<id>.md` の frontmatter を壊す | 上部の問題の一覧にその承認済みチケットが出て、他のカードはそのまま |
| 13 | プロジェクト | `projects/<repo>` を持つワークスペースで開く | `project` バッジと絞り込みが出る |
| 14 | 左端のアイコン | 拡張を入れる | アクティビティバーに ccnavi のアイコン。押すと「プロジェクト管理」「ルール管理」「リスク管理」「フェーズ管理」「チケット管理」「見た目」の順で 6 つ |
| 14b | チケット制御を切る | `.claude/settings.json` の env に `"CCNAVI_TICKET_CONTROL": "disable"` を書く | サイドパネルが「プロジェクト管理」「ルール管理」「見た目」の 3 つになり、コマンドパレットから「ボードを開く」「ボードを更新」「リスク管理画面を開く」「フェーズ管理画面を開く」が消える。プロジェクト管理の各行から「チケット管理」と「フェーズ管理」が、「ワークスペース本体」の枠から「自身の層のフェーズの種類」が消える。行を消すと戻る |
