...
| `ccnavi ボード: フェーズ管理画面を開く` | フェーズ管理画面を開く。既に開いていれば前面に出す。チケット制御が `disable` なら一覧に出ず、呼ばれても開かずに理由を言う |
...
自身の層とプロジェクトの層はプロジェクト管理画面の「ルール設定」から開き、対象ごとに 1 パネルで並べて開ける。
...
プロジェクト管理画面の「ワークスペース自身」の枠と各行の「フェーズ管理」から層を開く。対象ごとに 1 パネルで並べて開ける。
...
| 10 | 実行ファイルが無い | `dist/ccnavi/` の名前を一時的に変え、ソースも無いフォルダで `ボードを開く` | 「実行ファイルが見つからない」の通知 |
...
| 29 | プロジェクトのルール設定 | 行の「ルール設定」。glob を変えて保存せずに、`Write` と `projects/<名前>/docs/x.md` で「判定」。そのあと保存し、ターミナルで `ccnavi --explain` | タブの題が「ccnavi ルール設定: <名前>」。上部の path が `projects/<名前>/.ccnavi/config/rules.yml`。変えた後のルールで判定され、当たったルールの id が `<名前>:...`。保存したルールが `--explain` の `■ rules <名前>` に出る（判定に効いている）。ワークスペース版のパネルも同時に開いたままにできる |
...
| 42d | 図が描かれる | フェーズ管理画面で「図」のタブを押す | 種類が点で出て、`requires` は実線、`overlap` は破線で結ばれる。**矢印は付かない**。図の下に「線に向きは無い…」の一言が出る。開発者ツールの Console に CSP の拒否が出ていない（点の位置と拡大縮小はインラインの style で当たるので、拒否されると図が動かない） |
...
src/
  ...
  phases-panel.ts     フェーズ管理画面の Webview パネル（対象ごとに 1 つ。共通層・自身の層・プロジェクト）。検証・作成・保存の受け付け（vscode に依存する）
