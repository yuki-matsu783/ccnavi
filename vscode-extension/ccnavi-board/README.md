...
| 動いたカードの印 | 前の読み直しから列が変わったカード（承認された・着手された・閉じた）と、新しく現れたカードに緑の輪と帯（「未着手 → 作業中」「新しく出た（作業中）」）が出る。**時間では消さない。** 承認の直後は渡す文のオーバーレイがボードを覆っていて動く瞬間を見られないので、次に何かが動くまで残す。開いた直後は比べる相手が無いので出ない。列が動いていないボードが渡り直っても（オーバーレイの出し入れ、何も変わらなかった「更新」）作り直さない。読み直せなかった画面を挟んでも、最後に読めたボードと比べる。色だけに頼らず帯の文で言い、`prefers-reduced-motion` では光らない。**覚えるのは画面が生きている間だけ**で、別のタブへ移って戻る（画面が作り直される）と印は消える（`core/board-moved.ts`） |
...
- **承認の後は、監視と別に承認の側からも読み直す**（`approval-machine.ts` の `refresh`）。承認済みチケットが
  置かれれば監視も拾うが、拾えない置き方（`CCNAVI_TICKETS_PROPOSAL` が既定と違う、`files.watcherExclude` を
  足した、監視の効かないファイルシステム）では承認したのに画面が変わらないままになる。重なった読み直しは
  1 回に畳まれる（`board-panel.ts` の `again`）ので、画面はちらつかない
...
src/
  ...
  core/
    ...
    board-moved.ts    前の読み直しから列が変わったカードと、新しく現れたカード。時間では消さず、次に何かが動くまで残す印のもと
