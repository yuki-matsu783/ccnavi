...
| 4g | 承認が途中で書けない | 子の承認済みチケットの置き場（`.ccnavi/tickets/`）に、その子と同じ名前のディレクトリ（例 `i0001-01.md`）を作ってから、親と子をまとめて承認する | 「ccnavi --approve --yes が i0001-01 で止まった: 書けない（…）。i0001 の 1 件は承認済みチケットに入っている。コミットと push は送っていない…」と出る。ボードを更新すると、親だけが承認済で出る |
