...
async function lintJson(root: string, setting: string, extra: readonly string[], what: string): Promise<RunResult<LintJson>> {
  ...
  if (ran.code < 0 || ran.code > 1) {
    return { ok: false, error: `${what} が失敗した: ${firstLine(ran.stderr)}` };
  }
  ...
  if (unknown !== undefined) {
    return { ok: false, error: `実行ファイルが ${unknown} を知らない（古い）。${what} で確かめられないので進めない。実行ファイルを新しくする` };
  }
  ...
  return { ok: false, error: said === "" ? `${what} の出力を読めない（${parsed.error}）` : `${what} の出力: ${said}` };
}
