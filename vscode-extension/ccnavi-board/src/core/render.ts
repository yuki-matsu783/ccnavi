...
function renderActionButton(action: Action, id: string): string {
  switch (action.kind) {
    case "approve":
      // このカードだけの束（`--approve --preview --json <識別子>`）。同じ親の承認待ちが他にあっても巻き込まない。
      return `<button type="button" class="action" data-action="approve-one" data-ticket="${escapeHtml(id)}" title="このチケットだけを束にして承認する（ccnavi --approve ${escapeHtml(id)}）。まとめて承認するなら上部のボタン">この 1 件を承認</button>`;
    case "accept":
