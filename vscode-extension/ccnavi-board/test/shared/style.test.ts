 ...
 * React に移した 4 画面（ボード・プロジェクト管理・リスク管理・フェーズ管理）は中身を画面の中で
 * 組むが、CSS と body は拡張が入れるので同じに見る。
 */
...
/** React に移した 4 画面。中身は画面が組み、CSS と body だけを拡張が入れる */
...
test("CB-T127 5 つの画面は同じ骨組みの CSS（ツールバー・帯・欄・脚注）を 1 つの定数から持つ", () => {
  ...
  // 切り替えの受け口。文字列で組む 1 画面は埋め込みのスクリプト、React の 4 画面は画面の中
