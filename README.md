...
「承認済みチケット」「マーカー」「作業ツリー」「直接作業」のような呼び名は用語集で定義している。
...
- **読むのは git プロジェクトルートに checkout されている版だけ。** 作業ツリーの中の `.ccnavi/` は読まない。
  作業ツリーで層を直しても、そのブランチが統合されるまで効かない。`--lint` が「作業ツリーにしかないファイル」を warn で言う
...
雛形として作れる（組み込みの既定は無い。既定を組み込むと、意図せずレビューの要否が決まる）。
...
実行ファイルだけは作業ツリー側を持たない（置き場が `.gitignore` の中にあり、統合で入る道が無い）。
...
エージェントは直接書けない。見本の下書きは作業ツリーの `scratchpad/` に置き、ルールの下書きと一緒に利用者に渡す。
...
| この呼び出しが触っていないツリー | 呼び出しごとの監視が見るのは、ワークスペースルートと、この呼び出しの行き先（Bash は cwd）が属するツリーの 2 つだけ。触っていないプロジェクトや作業ツリーはそのとき見ない。ターンの区切り（`UserPromptSubmit` と `Stop`）では全部のツリーを見る |
...
「レビュー済み連絡」が「親の作業ツリーで `ccnavi-review.sh check --phase <N>` を打て」の文を同じ 2 ボタンで渡す。
...
親のツリーへ書けば親のチケットで判定される。ワークスペースルート直下と、チケットの無い作業ツリーは
ルールだけで判定する。
...
下書きの置き場（作業ツリーのルートの直下の `scratchpad/`）も、**実行前の判定だけは**範囲の外として扱わない。塞ぐと、
範囲を宣言した作業ツリーほど手元に何も置けなくなり、承認が要る作業だけが下書きの場所を失う。外すのは
...
子の作業ツリーについては、サブエージェントの終了時に基準点（`base_sha`）からのコミット済みの差分と
未コミットの両方を見て、範囲外が残っていれば 1 回だけ差し戻す。

**更新したときに変わるもの。**

- 作業ツリーを `allow` で開けているルール（`allow: "*/.claude/worktrees/*"` など）でも、承認済みチケットに
  結び付いた作業ツリーの書き込みはチケットの範囲に縛られる。止まったら、ルールを緩めるのではなく、その場所を
  範囲に持つチケットを提案する
- チケットの無い作業ツリー、ワークスペースルート直下、チケット制御が `disable` のワークスペースは変わらない
...
`note` はチャットで受けた承認や判断をマージリクエストの通常コメントに写す。作業ツリーの記録はマージで消えるので、
...
作業ツリーを片付ける」。`ready` が置くマーカーは push の後に書かれるので、コミットせずに作業ツリーを消すとマーカーが消える。
作業ツリーの片付けはマージを待たない。ブランチはリモートをホストがマージのときに消し、ローカルは残る（squash で入るので `branch -d` が通らない）。
...
親の作業ツリーからならその親の子、子の作業ツリーからならその子自身。ワークスペースルートや、チケットの無い
...
作業ツリーの `scratchpad/` に置き（追跡しないので、作業ツリーを畳めば一緒に消える）、ルールの下書きと一緒に利用者に渡す。
...
| warn | 作業ツリーの ccnavi ディレクトリに、切り元の git プロジェクトルートに無いファイルがある（統合されるまで効かない） |
...
| warn | チケットの無い作業ツリー、識別子と名前の一致しない作業ツリー |
| warn | 作業ツリー側に置かれた承認済みチケット（読まれない） |
| warn | 承認済みチケットはあるが作業ツリーが無い（範囲が効かない） |
| warn | リモートの種類に合うトークン（`GITHUB_TOKEN` / `GITLAB_TOKEN`）が無い、`origin` が読めない |
| warn | 作業ツリーでもワークスペースルートでもないのに `.claude/` を持つディレクトリがある |
