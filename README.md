...
まず判定だけを見たいなら `PreToolUse` と `PostToolUse` の 2 つでも動くが、残り 5 つを
...
ここで言う「プロジェクト」は**ワークスペース単位**の意味で、Claude Code を開いた場所
（ワークスペースルート）1 つを指す。`projects/<名前>` の下の各プロジェクトの単位ではない。
...
- チケット作業（設計に触れる・複数の段階になる・人のレビューが要る）: wip/proposals/ に
...
識別子が混じっていれば何も承認せず、親の改版が承認待ちなのに対象に無い子も何も承認しない
...
超えていれば「範囲にあるのに、判定で止まる場所（承認しても書けない）」の見出しで出る。親が `issue:` を持てば、
その番号も出る（マージリクエストの本文で `Closes` に使う）。承認すると
チケットごとに承認済みチケットを置く。承認後に提案を書き足しても効く範囲は変わらない。

**承認済みチケットは親チケットのブランチに乗って他の機械へ届く。** 運ぶのは
`sh .ccnavi/scripts/ccnavi-push-approved.sh` で、承認の 2 つの場所がどちらもこれを通す。
...
`note` はチャットで受けた承認や判断をマージリクエストの通常コメントに写す。ワークツリーの記録はマージで消えるので、
...
ワークツリーの片付けはマージを待たない。ブランチはリモートをホストがマージのときに消し、ローカルは残る（squash で入るので `branch -d` が通らない）。
...
`tree` と `project` は、この呼び出しがどのツリーのものと判定されたか。`source` は、判定を下した
...
捨てた定義は出ない。ここに出ているものが、Bash の和に実際に入っているものと一致する。
...
| error | 配点の `script:` が層の外を指す（共通層から `.ccnavi/scripts/`、層から `.ccnavi/common/scripts/`）、または指す先が git プロジェクトルートに無い |
...
引数を後ろに繋ぐので、後ろに 1 本足すと後勝ちで上書きできた。2 本目が在ったら止める。
