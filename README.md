...
**0 が「はい」、3 が「いいえ」。** いいえの理由は 3 つ（承認待ちに無い識別子、承認待ちが 1 件も
無い、承認の対象にしない提案）。範囲の超過と読めない提案では落とさない。どちらも `--approve` が
落とさないもので、ここで落とすと確かめと承認の答えが割れる。落とさないが本文には出す。
...
「確かめでは いいえ、承認は 通る」という食い違いになる。範囲の超過（`batch[].overflow`）と、
