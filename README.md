...
`--yes` は端末を求めない代わりに、見せた一覧と今の一覧が同じであることを求め、違えば何も置かない。
...
ccnavi --approve --yes <識別子,…> --json [<絞り>...]    # 見せた一覧をそのまま承認する
```
...
| `root` / `generated_at` | ワークスペースルートと、出した時刻 |
| `batch[]` | 承認の対象。`{ticket, title, parent, phase, revision, tree, path}`。`parent` と `phase` は子だけ（親は `null`）。`revision` は親の改版。空なら承認待ちが無い |
...
| `mismatch` | 一覧が変わっていたとき。`{expected[], current[]}`。このとき承認済みチケットは置かれず、終了コードは 1 |

`--yes` は端末を求めない（`--preview` も同じ）。代わりに、見せた一覧と今の一覧が同じであることを
