...
エージェントは直接書けない。見本の下書きは作業ツリーの `tmp/` に置き、ルールの下書きと一緒に利用者に渡す。
...
下書きの置き場（作業ツリーのルートの直下の `tmp/`）も、**実行前の判定だけは**範囲の外として扱わない。塞ぐと、
範囲を宣言した作業ツリーほど手元に何も置けなくなり、承認が要る作業だけが下書きの場所を失う。外すのは
**ルートの直下 1 段の `tmp/` だけ**で、綴りの大文字小文字は区別する（`TMP/`・`tmpX/`・`docs/tmp/`・`tmp` という
名前のファイルは、どれも普通の作業対象）。

外してよい根拠は「そのツリーの git が追跡しないので統合先へ乗らない」ことの 1 つだけで、`.gitignore` は
リポジトリごとに違うので保証ではない。だから**実行後の監視とサブエージェント終了時の検査は `tmp/` を外さない**。
あの 2 つが見るのは `git status`（`--ignored` 無し）と `base_sha..HEAD` の差分（追跡ファイルだけ）なので、
追跡されていない `tmp/` はそもそも現れない。現れたら、それは追跡されているということ、つまり根拠が崩れて
...
作業ツリーの `tmp/` に置き（追跡しないので、作業ツリーを畳めば一緒に消える）、ルールの下書きと一緒に利用者に渡す。
...
| warn | 下書きの置き場（`tmp/`）に追跡されているファイルがある、または `tmp/` が git で無視されていない（ワークスペースと各プロジェクトのそれぞれ。追跡されていると、実行後の監視が下書きを範囲外として報告しはじめる） |
