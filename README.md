...
| `scattered[]` | どれが本物か決まらない写りの全部。`{tree, state, path}`。決まっていれば空。入るのは 2 つの形で、権威のツリー（親のツリー → 元ツリーの順。ADR-0073）で畳んで 2 つ以上残ったとき（状態の操作が「複数の場所にある」で止まる）と、1 つのツリーの中で 2 つの置き場に在るとき（`--lint` が ERROR で言うのと同じ条件）。写りがあること自体は普通なので `seen_in` の数は食い違いを意味しない |
