...
- **区別が要る `regex` は `(?-i:...)` で囲む。** 囲むのは区別したい部分だけでよい。`[\\/](?-i:strict)[\\/]` は
  ...
  `\.[^c\\/]` のような除外は、畳むと `.C` まで除外されて**除外の側が広がる**。そこは `\.(?-i:[^c\\/])` と囲む
- **`allow` でコマンドの名前に当てるときは囲む。** `deny` と `ask` は畳んでよい（止まる側に倒れる）が、
  `allow` を畳むと網が広がり、綴りを変えた呼び出しまで通る。通ると、どのルールも言及しない呼び出しを
  権限モードへ渡す安全網（`UNDECLARED`）が外れ、確認できる者が居ないモードではその拒否ごと消える。
  共通層の `prefer-read-grep` と `prefer-glob` は `(?-i:(cat|grep|...))` の形で囲んである
...
- 深さは 2 段。範囲は 20 件まで
- 綴りは書いたまま持つ。`src/Components/*` は大文字のまま記録と承認画面に出る。畳むのは当てるときだけ
- 大文字小文字は、どの機械でも区別しない。`docs/Design/*` は `docs/design/plan.md` にも
  ...
  `regex` で書いた範囲も同じに畳む。区別が要るなら `(?-i:...)` で囲む
