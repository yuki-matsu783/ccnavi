...
（既定 15）で切り、一度落ちた origin には同じ回でもう取りに行かない。取ってこられなかったときは、
確かめる対象（資格情報を伏せた origin・ブランチ・手元の SHA）をモデルに返す。git の通信が許されず、
ホストには MCP のツールでしか届かない環境で、モデルがホストの先頭のコミットと比べ、遅れを利用者に
伝えられるようにするため。遅れを埋めるために承認済みチケットを取ってきて書く道は案内しない（そこを
動かすのは人）。
