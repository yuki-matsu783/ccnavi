...
`ccnavi-git.sh`、実行ファイル、それに記録と控え（`logs/log.jsonl` と `logs/state`）。記録と控えは名前を絞って守る。
...
見本の下書きは scratchpad に置き、ルールの下書きと一緒に利用者に渡す。env（`CCNAVI_RULES` など）で共通層を ccnavi ディレクトリの外に
