...
| `CCNAVI_TICKETS_APPROVED` | 承認済みチケットとフェーズの印の置き場。各ツリーのルートからの相対。既定は `.ccnavi/tickets`（層の傘の下）。そのツリーの git が追跡し、親チケットのブランチに乗って他の機械へ届く。空文字は受けず、既定の置き場に戻る（切るのは `CCNAVI_TICKET_CONTROL` の仕事。空で書いてあれば `--lint` が言う） |
...
設定 3 本になったので、1 本ずつのパスではなく傘の名前を渡す形にした。旧の置き場（`config/rules.yml`）は
...
傘の名前（`.ccnavi`）は `CCNAVI_PROJECT_HOME` の既定値で、変えられるのはそこだけ。`config/` と 3 本のファイル名は固定。
...
層の傘の下のスクリプト（`.ccnavi/scripts/`）はこの一式に入れない。スクリプトは配点から `script:` で参照されて初めて効き、
...
傘の下のファイル数が定まらず、呼び出しのたびに全部を読んで比べると判定の期限に効くため。
...
`mv .ccnavi .ccnavi.bak` も止まる。傘ごと消えれば、下のファイルを 1 本ずつ書き換えるのと
...
| warn | 作業ツリーの層の傘に、切り元の git プロジェクトルートに無いファイルがある（統合されるまで効かない） |
