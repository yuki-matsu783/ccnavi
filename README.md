...
3. 旧 `.claude/ccnavi/phases.yml` を消す。**ただし VS Code 拡張のフェーズ管理画面を使っているなら、画面が層に
   追従するまで消さない**

1 と 2 のあいだは、共通層と自身の層に同じ種類が並ぶ。全欄一致の重複として自身の層の側が捨てられるだけで、判定は変わらない
（`--lint` が info で言う。「ルールは 3 層の和で当たる」）。3 を行わずに残し続けても同じで、判定は変わらない。

3 に条件があるのは、拡張のフェーズ管理画面が共通層の `.claude/ccnavi/phases.yml`（`CCNAVI_PHASES`）の 1 本だけを開くため。
消すと画面は存在しないファイルを開こうとし、種類が実際に置かれている自身の層を編集できない。両方に同じ中身を
置いたままにするなら、画面で直した内容を自身の層にも写し、2 本を揃えておく（揃っていないと、同じ `id` で中身の違う種類が
並んで `--lint` が error を出し、自身の層が空になる）。リスク管理画面も共通層の `.claude/ccnavi/risk.yml` だけを開くが、
配点は共通層に残るので今のまま使える。ルール設定画面は層に追従した（自身の層とプロジェクトの層を開ける）が、
フェーズ管理画面とリスク管理画面の追従は別の issue で扱う。
