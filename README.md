...
その下の子が複数、という形が普通。識別子を並べればその分だけの束になる（VS Code 拡張が
絞り込みで見えている承認待ちを渡す）。承認待ちに無い識別子が混じっていれば何も承認せず、
束から外した親を持つ子は落ちる。親の改版が承認待ちなのに束に無い子も、旧計画で検証しないよう何も承認しない。画面に出るのは、親が新たに書けるようにする領域、
