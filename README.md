...
まず判定だけを見たいなら `PreToolUse` と `PostToolUse` の 2 つでも動く。残り 5 つを
登録しないと何が落ちるかは、「設定」の節の表にある。`CCNAVI_MODE` は `dry-run` にする。
...
`dry-run` の間は `warn` が出続ける。モードの 1 件（「`dry-run` なので判定しても呼び出しに
手を出さない」）と、`allow` を書くまでのもう 1 件（「`allow` が空」）。終了コードは 0 のままで、
切り替えが済んでいない合図としてそこに残る。
...
分かる。「止めた」としか言っていないルールは、`enable` にしたとたんにエージェントの手を止める。
...
気づけない。`/ccnavi-config` スキルがこの流れをまとめて回す。
...
`Write|Edit|NotebookEdit`）が走り、整形と検査をかける。テストはそこでは走らせない。
1 ファイル直すたびに全件を走らせると、複数ファイルにまたがる変更では途中の状態が必ず落ちる
（ADR-0036）。

テストは `.claude/hooks/test-py.sh` で、`Stop` に登録してターンの終わりに 1 回だけ回す。
落ちていたら止まらせずに差し戻すが、差し戻しは 3 回まで。上限に達したら止まらせて判断を
人へ返す。このリポジトリの `.claude/settings.json` が `Stop` に登録しているのは ccnavi の
...
ワークツリーで組み立てると、写す先はそのワークツリーの `.ccnavi/bin/` になる。hook が起動するのはワークスペースルートの
...
設定は環境変数で渡す。書き場所は `.claude/settings.json` の `env` ブロック。
...
`dry-run` に落ちる（「動作モード」の節）ので、様子を見ている間に手元のファイルが戻ることは
無い。裏を返すと、`enable` へ切り替えた瞬間にこの 2 つも本気になる。同じ強さで並べておけば、
切り替えの前に「何が戻るはずだったか」を記録の `would-restore` で確かめられる。代償として、
`dry-run` で入れたまま忘れるとこの 2 つも `dry-run` のまま残る。`--mode enable` で打ち直すか、
...
| `CCNAVI_GUARD_TICKET_APPROVAL` | `enable`（既定）、`disable`。チケットの承認の経路を守るか。enable なら、シェルから ccnavi の実行ファイルを `--approve` / `--reviewed` / `--close-early` / `ticket …` / `review …` 付きで打つ形を止め（`DENY_TICKET_APPROVAL_CLI`）、`--approve` と `--reviewed` と `--close-early` は標準入力が端末であることを求める。エージェントのコマンド行でこの変数を置く形と `--guard-ticket-approval` に `enable` 以外を渡す形も、実行ファイルをどう呼んでいても同じ理由コードで止める。テストや、端末を持たない実行環境（CI など）で切る。`dry-run` は取らない（承認は通れば済んでしまうので、止めずに報告する段が無い）。書かれていたら `enable` に倒し、`--lint` が error にする |
...
実行ファイルを指すこともできない。設定を書く 1 回で、そのまま配布物も置く。
...
配布元は既定で、打ったスクリプトの置き場の 1 つ上――つまり ccnavi のリポジトリ。
...
sh は自分の隣（`.ccnavi/scripts/<os>-<arch>/`）は探さない。そこは配る場所ではなく、探すと自己防衛の綴りが
...
既定の配布元が使えないとき――組み立てていない、配布先が ccnavi 自身――は断らない。
...
- **`id` にコロンは書けない。** 層の名前を添えた形（`self:id` / `<プロジェクト名>:id`）と見分けが付かないため。
  共通層に `id: "lib:custom"` と書けると、プロジェクト `lib` の宣言に見えて、記録を読んだ人がどのファイルを直すのか
  決められない。`--lint` が error にし、その 1 件は読み込みから落ちる。ルール・フェーズの種類・リスクの配点の 3 本とも同じ
- **`glob` も `regex` も大文字小文字を区別せずに当てる。** Windows でも macOS でも Linux でも同じ。`*/.ccnavi/*` と書いた 1 行は
  `.Ccnavi/` にも当たる。当たり方が機械で変わると、同じルールファイルがある機械では止め、別の機械では通す。
  代償として、`allow` も綴り違いに広がる。既存のルールを持ち込むときは `--test-samples` を回して、通したくないものが
  通っていないことを確かめる
- **区別が要る `regex` は `(?-i:...)` で囲む。** 囲むのは区別したい部分だけでよい。`[\\/](?-i:strict)[\\/]` は
  `/strict/` にだけ当たり、`/STRICT/` には当たらない。**否定の文字クラスを持つルールは要注意で**、
  `\.[^c\\/]` のような除外は、区別せずに当てると `.C` まで除外されて**除外の側が広がる**。そこは
  `\.(?-i:[^c\\/])` と囲む
- **コマンドの名前も区別しない。** `CAT file` は `cat file` と同じ判定になる。区別しない機械ではどちらも
  同じコマンドが走るので、綴りだけで別扱いにしない。代償として、区別する機械で `CAT` という別の実行ファイルが
  在れば、`cat` 向けに書いた `allow` がそれにも掛かる（ADR-0051）
- **`common` と `self` は層の名札に予約してある。** `projects/common/` や `projects/self/` は、綴り違い（`projects/Self/`）を
  ...
  `--lint` が error で名指しし、`--approve` はその `project:` を承認しない。別の名前に変える
...
- **読むのは元リポジトリに checkout されている版だけ。** ワークツリーの中の `.ccnavi/` は読まない。
  ワークツリーで層を直しても、そのブランチが統合されるまで効かない。`--lint` が「ワークツリーにしかないファイル」を warn で言う
- **無い層は空。** 3 本は独立に無くてよく、無いことは咎めない。壊れている層も空として扱うが、そちらは記録に層の名前が残り、
  `--lint` が error にする。組み込みの既定へは落ちない（共通層の deny が消える側に倒れるため）。共通層のルール自身が
  読めないときだけ、今までどおり組み込みの既定に落ち、そのとき層は足さない
- **id には層の名前が付く。** 共通層は裸の `id`、自身の層は `self:id`、プロジェクトは `<名前>:id`。記録と文面がこの形で出すので、
  id だけで直しに行くファイルが分かる
- **同じ宣言の重複は 1 本にまとめる。** 裸の `id` が同じで全欄（`{root}` を置き換えた後）が一致する定義は、後ろの層のものを捨てる。
  `--lint` が info で言う。見本をそのまま持ち込んで始めたプロジェクトが共通層と同じ行を持つのは普通の形
- **同じ `id` で中身が違うルールは両方効く。** `--lint` が warn で言う。`deny` と `ask` は増える側に倒れる。
...
渡す回になる。書かなければ刻みは 1 で、当たるたびが渡す回。
...
編集がたまったところで一度だけ声を掛ける、といった書き方ができる。
...
- **この例は `--lint` の warn を 1 件出す**（「広い allow に additionalContext がある」）。
  苦情の理屈は「当たるたびに同じ文が積まれる」で、`every` を書いた時点で成り立たない。
  対象を絞るためではなく、たまったところで声を掛けるためのルールなので、`glob` は広いままでよい。
  ソースの下だけに絞ると、`CLAUDE.md` や `docs/adr/` を直しているときに当たらなくなる。
  いちばん突き合わせが要る編集が、そこで落ちる
- **数えるのは「そのルールが当たった回数」。** 変わったファイルの本数でも、経った時間でもない。
  同じファイルを 5 回直せば 5 と数える。`match` はツール名なので、`Bash` を書かない限り
  シェルやビルドが書いたぶんは数えない
- **刻みは `SessionStart`（起動・再開・compact の後）で 0 に戻る。** 数えは進みを捨てるだけで
  先へは進まないので、`additionalContext` に添えたときは**届くのが遅れる**（戻りが続けば
  一度も届かない）。`additionalContextOnce` に添えたときは「渡す回の最初の 1 回」が改めて
  成り立つので、**セッション全体では 1 度より多く届く**。長い作業で正確に N 回ごとを守る欄ではない
- **`additionalContextOnce` に添えると、1 回目ではなく N 回目に届く。** 「たまったところで
  1 度だけ言う」はこの形で書く。逆に「1 回目に言いたいこと」と同居はできないので、
  両方要るならルールを 2 件に分ける
- 数えは文脈ごと（セッションと、サブエージェントならその起動）。`logs/state/once-*.json` に
  「鍵 → 回数」で残る。控えの置き場が無い（`--state ""`）なら刻まず毎回届く
- `every: 1` は既定を明示しただけなので `--lint` は何も言わない。`0`・負・整数でない値は
  error で名指しし、判定は 1（毎回渡す）に倒して通す。渡すものが 1 つも無い `every` は warn
...
- 行き先（Bash なら cwd）がワークツリーの中なら、まずワークツリーの同じパスを見る。
  無ければその元リポジトリ、最後にワークスペースルート。ワークツリーで直している最中の
  案内文がそのまま効く
...
打つ（形は下の「承認の JSON」）。`decide` はボタンから統合ターミナルへコマンドを送り、y/N は人が
ターミナルで押す。`close-early` はボードに置かず、端末で打つ。組み立て方と使い方はそこの README、
出力の形は下の「ボードの JSON」。
...
雛形として作れる（組み込みの既定は無い。既定を組み込むと、意図せずレビューの要否が決まる）。リスク管理画面と同じく、
`CCNAVI_TICKET_CONTROL` が `disable` なら入口ごと出ない。
...
切れ目にはルールから見える目印が置かれ、目印は 2 つある。コマンドとコマンドの間は `\x00`、
...
語の中に入った `>` `<` `|` `&` `(` `)` も同じ扱いで、両側に語の中の目印が置かれ、演算子としては読まれない。
...
静的に読めないコマンドは、生の文字列との一致に縮退する。縮退した拒否は文面が変わり、
...
**`Grep` と `Glob` は ccnavi が受け持たない。** 守りは 3 段に分かれていて、探すツールが読む
ファイルを見るのは、下の表の上 2 つ。
...
読めない値は報告して `enable` に落ちる。緩む側には倒れない（ADR-0007）。
...
上の実行後の監視は、保護領域をルールファイルの `deny` と `ask` から導く（戻すのは
そのうち `deny` のぶんだけ）。だからルールファイル自身をそこで守ることはできない。書き換えられた瞬間に「何を守るか」
の一覧ごと消えるので、`deny` を空にされた形も、壊れて組み込みの既定に落ちた形も、
保護領域は 0 件になり、監視は何も検知しない。

そこで次のものだけは、ルールファイルの外に、組み込みで持つ。
...
上のうち git が追跡しているものは、その元リポジトリから切ったワークツリーが持つ同じファイル（ワークツリー側の設定）も対象に入る。
ワークツリー側の設定は今この瞬間には誰にも読まれないが、ブランチを統合すればそのまま hook の登録と設定になる。実行前の `deny` は
「今の 1 回」を止めるだけで、統合は別の日の別のセッションが行うので、そこには届かない。片方だけ守っても時間差で効く。
実行ファイルだけはワークツリー側を持たない（置き場が `.gitignore` の中にあり、統合で入る道が無い）。

守る場所は層の数だけ増えるが、守る対象の種類は「設定 3 本」の 1 種のまま。ルールは何を止めるかを、フェーズの種類はレビューが
要るかを、配点は点が幾つかを決める。3 本とも、ここが書けるとエージェントが自分の判定を緩めて次の呼び出しから効く。
ファイル単位で見るので、無いものは対象から外れる。

ccnavi ディレクトリの下のスクリプト（`.ccnavi/scripts/`）はこの一式に入れない。スクリプトは配点から `script:` で参照されて初めて効き、
その配点がここで守られているので、新しい参照は足せない。残る道は「既に参照されているスクリプトの中身を書き換える」だけで、
そこは組み込みの `deny`（`*/.ccnavi/*`）と `CCNAVI_RESTORE_IF_DENY` に任せる。止まり、抜けても戻る。控えと復元まで持たせないのは、
ccnavi ディレクトリの下のファイル数が定まらず、呼び出しのたびに全部を読んで比べると判定の期限に効くため。

実行ファイルだけ控える時点が違う。PyInstaller が作るものは 17 MB ほどあって、
...
git は控えが無いときの代わりで、そのときだけ使う。実行前に対象が消えていて控えも
...
シェルから書き込む形は、ルールに書いていなくても拒否される。
戻せるだけでは足りないからで、実行後に戻すまでの間、書き換わった設定はそのまま
効いている。止めるほうが本筋で、戻すほうは止めきれなかったぶんの受け皿になる。

止める綴りは「区切りが続くか、そこで終わる」形で持つ。`rm -rf .ccnavi` と
`mv .ccnavi .ccnavi.bak` も止まる。ccnavi ディレクトリごと消えれば、下のファイルを 1 本ずつ書き換えるのと
同じだけ守りが消えるため。場所の綴りはどの機械でも大文字小文字を区別せずに当てる。コマンドの名前
（`rm` / `cp`）も、ルールを読み込むときに式ごと区別せずに当てる形になる（ADR-0051）。`RM` が走る保証は
...
組み込みは、ルールファイルに何が書いてあっても足す。`builtin-guard-` で始まる id はどの層のルールファイルにも書けず、
書けばそのルールは読み込まれずに error になる（`--lint` も言う）。組み込みを外したいなら env で面ごと切る。

ルールファイルが壊れて組み込みの既定に落ちている間は、共通層のルールファイルを Write / Edit で直した結果を戻さない。
実行前の時点で読めなかったときに限る。シェルからの書き込みと他の設定ファイルは今までどおり戻す。
...
| この呼び出しが触っていないツリー | 呼び出しごとの監視が見るのは、ワークスペースルートと、この呼び出しの行き先（Bash は cwd）が属するツリーの 2 つだけ。触っていないプロジェクトやワークツリーはそのとき見ない。ターンの区切り（`UserPromptSubmit` と `Stop`）では全部のツリーを見る |
...
ここで言う単位は**ワークスペース**で、Claude Code を開いた場所（ワークスペースルート）
1 つを指す。`projects/<名前>` の下の各プロジェクトの単位ではない。
`CCNAVI_TICKET_CONTROL` は `.claude/settings.json` の `env` に 1 つだけ書くつまみで、
...
効いているワークスペースでも、すべての作業にチケットが要るわけではない。作業の進め方は 2 つ。

- **直接作業**: 調査や小さな修正（触るファイルが少ない、振る舞いが変わらない、人のレビューが
  要らない）。チケットを起こさず、そのまま進める。判定は全体ルールだけ。ワークスペースルート直下と、
  承認済みチケットの無いワークツリーがこれにあたる
...
改めて届くものは頭では言わない。レビューの sh の綴りはフェーズの終わりに来たときと `ready` の
...
最後の行は dry-run のときだけ。「止まらない」だけで終えずに、通ったことを許可と読まないこと
まで書く。dry-run は判定に手を出さないモードなので、deny に当たった呼び出しもそのまま実行
される。そこで文が黙ると、通った事実が許可の証拠として読まれる。
...
- 子は親の部分集合として書く。親やフェーズの種類の `scope` を超える項は承認で warn に出るだけで、
  判定がその上限で切り詰めるので、超えた場所には書けない（「判定の鍵はファイルの行き先」）
- 書いていない場所は範囲外。親子は厳しい側が勝つ
- 深さは 2 段。範囲は 20 件まで
- 綴りは書いたまま持つ。`src/Components/*` は大文字のまま記録と承認画面に出る。
  大文字小文字を無視するのは当てるときだけ
- 大文字小文字は、どの機械でも区別しない。`docs/Design/*` は `docs/design/plan.md` にも
  当たる。機械に任せると、同じチケットと同じ綴りで、止まる場所が Windows と Linux で
  食い違う。範囲は人が宣言する意図なので、機械の都合ではなく綴りの意味で読む。
  `regex` で書いた範囲も同じに扱う。区別が要るなら `(?-i:...)` で囲む
...
全ツリー（ワークスペース、プロジェクト、ワークツリー）の `wip/proposals/` を走査し、未承認のものをまとめて
見せる。親が 1 本、その下に子が複数、というのが普通。識別子を並べた
`ccnavi --approve <識別子>...` はその分だけを承認の対象にする（VS Code 拡張が、絞り込みで見えている
承認待ちを渡す）。絞りは対象を狭めるだけで、絞らないときに落ちるものは通さない。承認待ちに無い
識別子が混じっていれば何も承認せず、親の改版が承認待ちなのに対象に無い子が混じっていても、
やはり何も承認しない
（外すと承認済みチケットの旧い計画で検証してしまう）。対象から外した親を持つ子は「親が承認されていない」で
落ちる。同じ親の子がフェーズをまたいで並ぶと、番号の小さい子から数える。終わったフェーズに子を足す
と、そのフェーズは開き直るので、それを前提にしていた後のフェーズの子は落ちる（1 本ずつ承認したときと
同じ答え）。まだ承認済みチケットの無い親と一緒に出した子も、その親の計画で順序を数える。親と
2 番目以降のフェーズの子を一緒に出しても、前のフェーズが閉じていないので子は落ちる。画面に出るのは、親が新たに書けるようにする領域、
子が親の範囲をどこまで絞ったか、子ごとの人間レビュー要否、計画。子の範囲が親の範囲やフェーズの種類の上限を
超えていれば「チケットで編集対象としているが、書き込めない場所」の見出しで出る。親が `issue:` を持てば、
その番号も出る（マージリクエストの本文で `Closes` に使う）。承認すると
チケットごとに承認済みチケットを置く。承認後に提案を書き足しても効く範囲は変わらない。

**承認済みチケットは親チケットのブランチに乗って他の機械へ届く。** 運ぶのは
`sh .ccnavi/scripts/ccnavi-push-approved.sh` で、端末とボードの 2 つの承認の経路がどちらもこれを通す（ファイルを動かす 3 つめの経路は除く）。
端末から打つときは `sh .ccnavi/scripts/ccnavi-approve.sh` を使い、承認が通るとそのまま
呼ばれる。識別子を並べた `sh .ccnavi/scripts/ccnavi-approve.sh i0002-03` は、`--approve i0002-03` と
同じくその分だけを承認の対象にする（同じ親の承認待ちのうち 1 本だけを先に承認するとき）。`-` で始まる語と
空の語は識別子として受けず、何も打たずに 2 で終わる（`--yes` や `--root` が混ざると、y/N を経ない経路や
別のワークスペースの承認に化けるため）。ボードでは未承認カードの「この 1 件を承認」が同じ役をする。
ボードで承認したときは、承認のあとボードがこの sh を絶対パスで統合ターミナルへ
Enter まで送る（y/N は無い）。A が承認して B の機械で作業し A がレビューする、
...
- シンボリックリンクは辿らない。`projects` と `.claude/worktrees` そのものか、その下の 1 件が
  リンクなら、名指しして飛ばす（リンクの先はワークスペースの外のリポジトリでありうる）
- `main` / `master` / `develop` / `release` / `release/*` と、ブランチの上に居ないツリーは
  push せず、コミットまでで止める。そこへ直接送る判断は人のものだから
- 運ぶものがどのツリーにも無ければ `運ぶ承認済みチケットは無い。` と 1 行出す
- エージェントが打つ形は組み込みの deny（`DENY_TICKET_APPROVAL_CLI`）が止める。運ぶ時機は人が決める
...
`ccnavi-approve.sh` は承認が通れば、運ぶ段が 1 で終わっても 0 を返す（運ぶ失敗で承認が失敗に見えないように）。
...
git でリモートに届かないときも止めず、手元の版で判定する。認証を尋ねる画面は出さず
（`GIT_TERMINAL_PROMPT=0`・`GCM_INTERACTIVE=never`）、fetch 1 回を `CCNAVI_FETCH_TIMEOUT` 秒
（既定 15）で切り、一度落ちた origin には同じ回でもう取りに行かない。hook の上限（60 秒）に
当たると、報せごと捨てられるため。
認証で落ちたとき（401・403、資格情報が無い、公開鍵が通らない）は、そう 1 行添える。尋ねないので
落ち続けるが、人が端末で一度 `git fetch origin`（承認の sh でもよい）を打って認証を済ませれば、
保存された資格情報で次のセッションから通る。見分けは git の文言に頼るので、`LC_ALL=C` で揃えて見る。
...
**承認を頼む前に、エージェントが自分で確かめる。** `--approve --preview --verify [<識別子>...]` は、
承認済みチケットを置かずに「いま `--approve` を打てば、その提案が承認の対象に入るか」を返す。
**0 が「はい」、3 が「いいえ」。**「いいえ」になるのは 3 つで、承認待ちに無い識別子、承認待ちが
1 件も無い、承認の対象にしない提案がある。範囲の超過と読めない提案は「いいえ」にしない。
どちらも `--approve` が落とさないもので、ここで落とすと確かめと承認の答えが割れる。ただし本文には出す。
**1 はほかと同じで「使い方か設定の誤り」**（綴り違い、`--preview` の付け忘れ、読めない設定）。
答えと分けてあるので、打ち方の誤りを提案の落ち度と読み違えない。`--json` を足すと
「承認の JSON」の形に `verify` が付く。`--preview` に相乗りさせてあるのは、組み込みの deny が
免除するのが `--preview` の付いた `--approve` だけだから。提案を `wip/proposals/todo/` に書くと、
この確認を勧める文が 1 つの文脈につき 1 度だけ届く（承認の知らせと同じ経路。判定には足さないので、
`todo/` の扱いは何も変わらない。ADR-0059）。

承認されたことは、次の `UserPromptSubmit` か `PreToolUse` で 1 度だけモデルに届く
（`additionalContext`）。セッションごとに、最初の hook より後に置かれた承認済みチケットと、版
...
識別子の承認済みチケットで判定する。呼び出し元の cwd も、サブエージェントかどうかも見ない。
親の cwd から子のツリーへ絶対パスで書いても子のチケットで判定され、子のサブエージェントが
親のツリーへ書けば親のチケットで判定される。ワークスペースルート直下と、チケットの無いワークツリーは
ルールだけで判定する。

ルールの判定とチケットの判定を両方出し、**強い側を採る**（`deny` > `ask` > `allow` > 何も言わない）。
範囲の外とチケットの `deny` の項は `deny`。同じ強さならルールの判定と文面を使う。
...
チケットがルールより強かった回は、文面の頭に
`rule: <id> (allow) lets this through, but the ticket for this worktree narrows it` が載り、チケットの `deny` の項に
当たったなら `ticket entry: deny <綴り>` も載る。理由コードは `DENY_TICKET_SCOPE` / `TICKET_ASK` のまま。記録では
`rules` が `(ticket-scope)` とルールの id の並びになり、`source` は空になる。

```sh
jq -r 'select(.rules[0]? == "(ticket-scope)" and (.rules | length) > 1) | .subject' logs/log.jsonl
```

子の「範囲」は、子の宣言を親の範囲とフェーズの種類の `scope` の両方で切り詰めたもの
（種類が `inherit` か、`phases.yml` がどの層にも無ければ親だけ）。子に書いた項が上限を超えていても
承認は warn で通り、判定がここで止める。regex の項は実際のパスを親と種類に当てる。
上限で止めたときは、文面の頭に超えた上限が `limit: phase type 設計 (design): wip/design/*, docs/*` や
...
**ルートの直下 1 段の `scratchpad/` だけ**で、綴りの大文字小文字は区別する（`SCRATCHPAD/`・`scratchpadX/`・`docs/scratchpad/`・`scratchpad` という
名前のファイルは、どれも普通の作業対象）。

外してよい根拠は「そのツリーの git が追跡しないので統合先へ乗らない」ことの 1 つだけで、`.gitignore` は
リポジトリごとに違うので保証ではない。だから**実行後の監視とサブエージェント終了時の検査は `scratchpad/` を外さない**。
あの 2 つが見るのは `git status`（`--ignored` 無し）と `base_sha..HEAD` の差分（追跡ファイルだけ）なので、
追跡されていない `scratchpad/` はそもそも現れない。現れたら、それは追跡されているということ、つまり根拠が崩れて
いるということなので、範囲外として報告する。

シェルが書いたものは実行後の監視が `POST_TICKET_SCOPE` で言う。ルールの `allow` に当たる場所でも同じ。
子のワークツリーについては、サブエージェントの終了時に基準点（`base_sha`）からのコミット済みの差分と
未コミットの両方を見て、範囲外が残っていれば 1 回だけ差し戻す。

**更新したときに変わるもの。**

- ワークツリーを `allow` で開けているルール（`allow: "*/.claude/worktrees/*"` など）でも、承認済みチケットに
  結び付いたワークツリーの書き込みはチケットの範囲に縛られる。止まったら、ルールを緩めるのではなく、その場所を
  範囲に持つチケットを提案する
- チケットの無いワークツリー、ワークスペースルート直下、チケット制御が `disable` のワークスペースは変わらない
...
人）。拒否の文面・`--explain`・ボードのバッジはこの名前で言い、どちらの段かで「やること」も変わる。
...
番号だけのフェーズは「2 番目のまとまり」以上のことを言わない。`.ccnavi/common/phases.yml` に
...
足すのは何度でもできる。それでも残る指摘は `ccnavi-review.sh to-issue` で別の issue に切り出す。
...
フェーズが終わっている・そのフェーズのレビューが延期されていない・そのフェーズの子ブランチが
親に取り込まれている・未コミット無し（未追跡は数えない）・push 済み・未依頼、の 6 つ。
1 つでも欠けたら全件を列挙して何もしない。
...
自分で自分の首を絞めることになる。人がレビューで見るものは変わらない。未コミットの側も同じ理由で前提から外してある。
...
未解決の扱いは人が端末で選ぶ（`decide <N>`）。選べるのは 2 つ。
...
- **`f` 続きの子チケットを起こす。** 見た子を `done/` へ動かし、同じフェーズの番号で
  ...
  マーカーは消える（子を足して承認したときと同じ）。指摘は受け入れない（次の `confirm` がまた数える）
...
見るのは作業ツリーの中（フェーズ・ブランチ・未コミット・push・マーカー）だけで、マージリクエストの中身は
...
| `decide N` | sh が取ってくる → `--reviewed N --accept-unresolved`（人に見せ、受け入れて進むか続きの子を起こすかを選ばせる）→ 受け入れた一覧か起こした子を sh がコメントに写す |
| （`chat` のフェーズ） | sh は動かない。人が端末で `ccnavi --reviewed <N> --chat --cwd <親のワークツリー>` を打つ。依頼も写しも無い |
| `comment` | sh が投稿する。実行ファイルは関わらない |
| `to-issue --body-file <題と本文>` | 残った指摘を別の issue に切り出す。`review to-issue`（局面を確かめ、残ったスレッドの URL を添えた下書きを書く）→ sh が issue を作り、マージリクエストに引き継ぎのコメントを残す |
...
`SubagentStart` で、承認済みで開いている子の一覧（識別子・ワークツリー・範囲・先行の状態）を
渡す。判定は行き先で決まるので、これは案内でしかない。渡すのは cwd のワークツリーに関わる子だけで、
親のワークツリーからならその親の子、子のワークツリーからならその子自身。ワークスペースルートや、チケットの無い
ワークツリーから起動したサブエージェント（別セッションの調査など）には何も渡さない。
...
`decision` は下した判定で `allow` `ask` `deny` `handover` `skip` のどれか。`handover` は
権限モードに委ねた回（「ルールが言及していない呼び出し」の節）。`enforced` は
実際に適用したか。`dry-run` は `enforced:false` になるので、1 つのファイルで
「何を止めるはずだったか」と「何を実際に止めたか」の両方が数えられる。

`code` は判定の根拠の種別で、返した文の先頭に載るものと同じ。一覧は
[ccnavi.md](ccnavi.md) の付録 A。よく出るのは `DENY_COMMAND_PATTERN`、`DENY_PATH`、
`RULE_ASK`、`UNDECLARED`、`PARSE_UNCERTAIN`、`DENY_TICKET_SCOPE`。止めた回のうちどれだけが
...
`worktree-unreadable`、`no-turn-baseline` の 9 つ。
通した回も残すのは、記録が無いことを「ccnavi が動かなかった」と読めるようにするため。
後ろの 3 つは実行後の監視が働かなかった回で、`worktree-unreadable` の
`detail` には見に行けなかった理由が入る。これが残らないと、違反が無かったのか
監視が動いていなかったのかが同じ見た目になる。`no-turn-baseline` は、ターンの基準が
無くて `Stop` が黙った回。
...
これでしか数えられない。共通層ではなく自身の層やプロジェクトの層が読めなかった回は、
組み込みには落ちず、`fallback` にその層の名前（`self`、`lib` など。複数ならカンマ区切り）が入る。
その層を空として判定した回、という意味になる。

`source` は、判定を下した
ルール（`rules` の先頭）がどの層から来たか。値は `common`（共通層）/ `self`（ワークスペース自身の層）/
プロジェクトの名前。ルールが当たらなかった行は空。`rules` の id も層の名前付き（共通層は裸、
...
いま効いている宣言を数え上げるには `--explain`。層ごとのルール一覧と、フェーズの種類と
...
（件数と中身は例。）並びは判定と同じ 共通層 → 自身の層 → プロジェクト（名前順）で、重複として
捨てた定義は出ない。ここに出ているものが、Bash の層の和に実際に入っているものと一致する。
読めない層は「読めない: <理由>。この層は空として扱う」、置いていない層は「この層は置いていない（無い = 空）」と出る。
`levels` の行は共通層の値。層で `levels` を書くとキーごとに小さいほうが勝つので、チケットに効く値は
その `project:` の層で変わる。
...
見本は ccnavi ディレクトリの下にあるので、エージェントは直接書けない（「コアファイルを守る」）。足す見本の下書きは
ワークツリーの `scratchpad/` に置き（追跡しないので、ワークツリーを畳めば一緒に消える）、ルールの下書きと一緒に利用者に渡す。
...
| `unwrapped` | 実行役のコマンドが中で実行するコマンド、または `cd` で移った先から見た綴りでルールに当たったときの、そのコマンド。複数なら `\x00` でつなぐ。元の形で当たったとき・当たらなかったときは空。後から足した欄 |
...
先頭の数行は「何を見て検証したか」。「チケットの承認の経路を守る」の行は、チケット制御が
有効なときだけ出る。
...
拾うものを群に分けて並べる。全件を機械で読むなら `--lint --json` の `problems[]`。
...
| error | 配点の同じ `id` が層をまたいで在る、合成後の `levels` が `medium <= high <= critical` になっていない（その層を空として扱っている） |
...
| warn | ワークツリーの ccnavi ディレクトリに、元リポジトリに無いファイルがある（統合されるまで効かない） |

層のファイルが無いことは言わない。無い層は空で、それが正常な形だから。言うと、本当に言うべき 1 行が埋もれる。
...
プロジェクト 1 つに対して行う。VS Code の拡張が、編集中のプロジェクトのルールを保存する前に
判定と検証に掛けるためのオプションで、`--test` / `--test-samples` / `--lint` / `--explain` でだけ効く。
診断の外（hook からの判定、`ticket` / `review` の副命令）に渡しても無視する（標準エラーにその旨を出す）。
保存していないルールが実運用の判定に効く道を作らないため。守る対象（コアファイル）も差し替えを見ず、
本来の場所を守る。

**この門は、層の置き場を動かす残りのフラグにも同じように掛かる**（ADR-0067）。共通層の中身の
`--rules` / `--phases` / `--risk` と、層を探す先の `--projects` / `--project-home`。7 本とも
「診断でだけ効く」で揃っていて、落とすときの文面も同じ。層を探す先も載せるのは、外せば
その層の配点も種類も消えて、中身を差し替えるのと結果が同じになるから。

`--root` と `--cwd` は別で、**1 度しか渡せない。** どちらも「いまどこで動いているか」で、
`.ccnavi/scripts/` の sh が自分で計算して渡す。sh は自分のぶんを先に置いてエージェントの
引数を後ろに繋ぐので、後ろに 1 本足すと後勝ちで上書きできてしまう。だから 2 本目が在ったら止める。

層のフェーズの種類も同じ形で差し替えられる。
...
`--project-phases-file <名前>=<パス>` は、その名前の層（`self` は自身の層）のフェーズの種類のファイルの代わりに
`<パス>` を読み、共通層の種類と合成して確かめる（同じ `id` で中身が違う、表示名が層をまたいで重なる、
`overlap` / `requires` の指す先）。共通層の種類は `--phases` で差し替える。効く経路と守る対象の扱いは
5 本とも同じ。VS Code の拡張のフェーズ管理画面が、編集中の層の種類を保存する前に検証するために渡す。
配点（risk）の層には差し替えがまだ無い。
...
VS Code の拡張が、承認を端末ではなくボードのオーバーレイで行うための 2 本。承認の対象を組むのは
...
`--verify` の答えは `--preview` の答えに `verify` を足したもの。読むのはエージェントで、
終了コードが答えそのものになる（**0 = はい、3 = いいえ**。1 はほかと同じで使い方か設定の誤り）。

| 鍵 | 何 |
|---|---|
| `verify.ok` | 真偽。`--approve` を打てば、指定したもの（指定が無ければ承認待ち全部）がそのまま承認の対象に入るか |
| `verify.reason` | 答えの理由の名前。全部入るなら `ok`、入らないなら `refused`（絞りが通らない）/ `nothing-pending`（承認待ちが 1 件も無い）/ `rejected`（承認の対象にしない提案がある）。読めない提案（`problems[]`）はここに出ない（落とさない。下記） |

`--preview` 自身は、対象にしない提案があっても 0 で返る（オーバーレイを出す側は 0 以外を
失敗として扱うので、そこで 0 以外を返すと一覧が出せなくなる）。答えが要るときだけ `--verify` を足す。

`--verify` が落とさないものが 2 つある。どちらも `--approve` が落とさないもので、ここで落とすと
「確かめは『いいえ』なのに承認は通る」という食い違いになる。範囲の超過（`batch[].overflow`）と、
読めない提案（`problems[]`。走査は絞る前の全ツリーを見るので、他のセッションの書きかけ 1 本で
落ちることになる）。どちらも本文には出す。

承認で落ちるものは `ccnavi --lint` も同じ関数（`approval.candidates`）を通って名指しする。
違うのは終わり方だけで、`--verify` は真偽、`--lint` は severity（error があれば非ゼロ）。
ただし「まだ承認できない」もの（前のフェーズが閉じていない子）は `--lint` では warn。
`--lint` の終了コードは CI と VS Code の設定画面の保存が見るので、編集と関わりのない提案 1 本で
そこが止まらないようにしてある。承認そのものは落ちる。
...
| `partial` | 置いている途中で止まったとき（書けない、など）。`{placed[], ticket, reason}`。`placed[]` はそこまでに承認済みチケットに入ったぶん（新規は置いた、改版は書き換えた）、`ticket` は止まったところ、`reason` は理由、`lines[]` は止まるまでに出た行（端末なら標準出力に出ていたぶん）。**置いたものは戻さない**（戻す途中でまた落ちる）ので、どこまで進んだかをそのまま返す。終了コードは 1。承認済みチケットを運ぶ sh は送られていない |

`--yes` は端末を求めない（`--preview` も同じ）。代わりに、見せた一覧と今の一覧が同じで、
`--digest` の値（大文字小文字は問わない）が承認のときに読み直した中身の指紋と合うことを求める。
人が見ていないものを承認する道を塞ぐため。`--digest` が無ければ承認せず、終了コードは 1。
指紋が覆うのは承認画面の本文と、束のチケットごとに承認済みチケットとして書き出す中身
（新規は提案の frontmatter と本文、改版は計画を差し替えた承認済みチケット。承認のときに付く
`ccnavi_approved` は除く）。画面に出ない欄や Markdown の本文を見せたあとに書き換えても通らない。エージェントが Bash や PowerShell で
`--yes` を打つ道は、組み込みの deny（`builtin-guard-ticket-approval`）が塞ぐ。`--preview` は
一覧を見るだけなので通す。`--json` を付けなければ、どちらも人向けの文面を出す。

後ろに並べる `<絞り>` は `ccnavi --approve <識別子>...` と同じで、承認の対象を狭める（ボードが絞り込みで
...
通す形の一覧に無いものは既定で拒否する。分類だけでは足りないので、閉じる向き
...
別の綴りへ送る refspec も通さない。`main` `master` `develop` `release` `release/*` へ直接は送れない。
レビューはマージリクエストの実物に結ぶので、そこまではエージェントが運べたほうがよい。
統合を決めるのは人なので、マージは人の側に残す。

送るのは親だけ。子チケットのワークツリー（承認済みチケットに `parent:` があるもの）からの
...
- 実行後の監視は git を 1 回起こす。止めていないので遅れは待ち時間にしかならず、
  ...
  すでに持っている情報を作り直すだけになる
- ネットワークへは出ない。起こす外部プロセスはローカルの git と、子を閉じるときに
  リスクの配点の `script` 項目を走らせる `sh` だけ
