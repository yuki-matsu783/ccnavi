...
| `wrapup --reason <理由> [--no-issue]` | まだ残っているが「キリの良いところまでやった」と締める。人が端末で打つ。`review wrapup`（残りを見せて y/N、未着手の子を取り消し、印を置く）→ sh が残りを issue に写し、Draft を外し、note を投稿する |
...
フィードバック計画が承認され、レビューが済んでいる）と同じ条件で通り、Draft を外す。
それが「マージに進んでよい」の合図で、マージするかどうかは人が MR で決める。
親を閉じる前でも後でも打てる。
...
残りを新しい issue に写して Draft を外す。黙って消えるものは作らない。`phases/<親>/wrapup.json` が
その証跡で、これがあれば親はフィードバック計画が無くても閉じられる。
