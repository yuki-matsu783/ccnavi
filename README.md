...
配った実行ファイルは、配り先の `.gitignore` にも足す（配り先が git のリポジトリの
...
配るのを諦めた理由を 1 行出して、`.claude/settings.json` は書く。名指ししていない配り元が
...
ワークスペースルートの中にある作業ツリーが持つ同じファイル（作業ツリー側の設定）も
対象に入る。統合でそのままワークスペースルートの設定になる道を持つので、
片方だけ守っても時間差で効く。
