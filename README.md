...
| `CCNAVI_TICKET_CONTROL` | `enable`（既定）、`disable`。チケット制御（提案の承認・承認済みチケットの範囲・フェーズの足止め・サブエージェントの制限）を使うか。全体ルールは全プロジェクトが使い、チケットまで使うかをここで決める。`disable` なら `--approve` と `ticket` / `review` の副命令は動かず、セッション開始の案内も出ず、VS Code 拡張の「チケット管理」も出ない。それ以外の値は `enable` として動き、`--lint` が error にする |
...
足止めの中で通す形も同じで、当てると `env sh .ccnavi/scripts/ccnavi-review.sh check --phase 1` のような、実行役のコマンドを
前に置いた形が足止めの中で通るようになる。
...
（提案の承認、承認済みチケットの範囲、フェーズの足止め、サブエージェントの制限）まで使うかは
...
- チケット作業（設計に触れる・複数の段階になる・人のレビューが要る）: wip/tickets/ に
...
手順で、人がどこで見るか（`review: mr` / `chat`）はそのフェーズの足止めで、フェーズの種類の
...
提案は `wip/tickets/<状態>/<識別子>.md`（`CCNAVI_TICKETS_PROPOSAL`）。状態は置き場が表す。
...
全ツリー（ワークスペース、プロジェクト、ワークツリー）の `wip/tickets/` を走査し、未承認のものをまとめて
...
提案の置き場（`wip/tickets/`）と承認済みチケットの置き場（`.ccnavi/tickets/`）の下は、範囲の外として扱わない。
...
| 足止め | する。レビュー済みのマーカーまで | する。レビュー済みのマーカーまで | しない。省略のマーカーを残す |
...
足止めは cwd のワークツリーで親を引く。止めている間、その親の cwd からの `Agent`（サブエージェントの
...
`chat` のフェーズは依頼という段を持たないので、人が端末で先へ進めるまで「レビュー準備中」のまま。
...
レビュー済みのマーカーを置き、足止めが解ける。未解決が残るなら一覧を返す。
...
| warn | `CCNAVI_TICKET_CONTROL=disable`（チケットの範囲も足止めも効かない） |
