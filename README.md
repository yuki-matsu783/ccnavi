...
しない提案、読めない提案）。範囲の超過では落とさない。承認も落とさないもので、そこは
「承認しても書けない」と行に添えて見せる。`--json` を足すと「承認の JSON」の形に `check` が付く。
...
| `check.reason` | 入らない理由の名前。`ok` / `refused`（絞りが通らない）/ `nothing-pending`（承認待ちが 1 件も無い）/ `rejected`（承認の対象にしない提案がある）/ `broken`（読めない提案がある） |
