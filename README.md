...
同じだけ守りが消えるため。場所の綴りはどの機械でも大文字小文字を区別せずに当てるが、
...
手順で、フェーズの種類の在りかは `ccnavi-ticket.sh --help` で、リスクの配点の綴りは承認の
ときの検査で、後工程の進め方は承認済みチケットが置かれたときに、それぞれ名指しされる。
...
- 大文字小文字は、どの機械でも区別しない。`docs/Design/*` は `docs/design/plan.md` にも
  ...
  `regex` で書いた範囲だけは、書いた人が意図を持てるので区別を残す
...
- Linux・macOS でも、`rules.yml` の `glob` が大文字小文字の違う綴りに当たる。`allow` も広がるので、
  `--test-samples` で確かめる
