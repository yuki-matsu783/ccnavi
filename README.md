...
| `CCNAVI_GUARD_UNWATCHED` | `enable`（既定）、`disable`。人にも classifier にも確認できないモード（`dontAsk` / `bypassPermissions`）で、ルールがどこも言及しない呼び出しを止めるか。`disable` なら判定を返さず、そのモードの取り決めに委ねる（読み切れなかった呼び出しは委ねない）。「ルールが言及していない呼び出し」。この門に `dry-run` は無く、それ以外の値は `enable` として動いて `--lint` が言う。**例外が 1 つ**: チケット制御が効いている間、`wip/proposals/todo/` への書き込みは組み込みの allow（`builtin-ticket-propose`）が言及するので、この門に届かず、`enable` でも通る（ADR-0058 の代償） |
...
ccnavi --approve --preview --check i0002 i0002-01   # 承認できる状態かを確かめるだけ（置かない）
```
...
**承認を頼む前に、エージェントが自分で確かめる。** `--approve --preview --check [<識別子>...]` は、
承認済みチケットを置かずに「いま `--approve` を打てば、その提案が承認の対象に入るか」を返す。
0 なら依頼してよく、1 なら理由が出る（承認待ちに無い識別子、承認待ちが 1 件も無い、承認の対象に
しない提案）。範囲の超過と読めない提案では落とさない。どちらも `--approve` が落とさないもので、
ここで落とすと確かめと承認の答えが割れる。落とさないが本文には出す。`--json` を足すと「承認の JSON」の形に `check` が付く。
`--preview` に相乗りさせてあるのは、組み込みの deny が免除するのが `--preview` の付いた
`--approve` だけだから。提案を `wip/proposals/todo/` に書くと、この確認を勧める文が
1 つの文脈につき 1 度だけ届く（組み込みの `builtin-ticket-propose`。止めはしない）。
...
実運用の判定に効く道を作らないため。守る対象（コアファイル）も差し替えを見ず、本来の場所を守る。
...
`overlap` / `requires` の指す先）。共通層の種類は今までどおり `--phases` で差し替える。効く経路と守る対象の扱いは
`--project-rules-file` と同じ。VS Code の拡張のフェーズ管理画面が、編集中の層の種類を保存する前に検証するために渡す。
...
ccnavi --approve --preview --check --json [<絞り>...]   # 承認できる状態かを確かめる（同上）
...
`--check` の答えは `--preview` の答えに `check` を足したもの。読むのはエージェントで、
終了コードが答えそのものになる（0 = そのまま承認の対象に入る、1 = 入らない）。

| 鍵 | 何 |
|---|---|
| `check.ok` | 真偽。`--approve` を打てば、指定したもの（指定が無ければ承認待ち全部）がそのまま承認の対象に入るか |
| `check.reason` | 入らない理由の名前。`ok` / `refused`（絞りが通らない）/ `nothing-pending`（承認待ちが 1 件も無い）/ `rejected`（承認の対象にしない提案がある）。読めない提案（`problems[]`）はここに出ない（落とさない。下記） |

`--preview` 自身は、対象にしない提案があっても 0 で返る（オーバーレイを出す側は 0 以外を
失敗として扱うので、そこで 1 を返すと一覧が出せなくなる）。答えが要るときだけ `--check` を足す。

`--check` が落とさないものが 2 つある。どちらも `--approve` が落とさないもので、ここで落とすと
「確かめでは 1、承認は 0」という食い違いになる。範囲の超過（`batch[].overflow`）と、読めない提案
（`problems[]`。走査は絞る前の全ツリーを見るので、他のセッションの書きかけ 1 本で落ちることになる）。
どちらも本文には出す。
